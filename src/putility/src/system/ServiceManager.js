...
const mkstatus = (name) => {
  ...
  c.name = `Status${name[0].toUpperCase() + name.slice(1)}`;
  return c;
};
...
class ServiceManager extends AdvancedBase {
  ...
  async maybe_init_(name) {
    ...
    for (const dependency of waiting_for) {
      /** @type Set */
      const waiting_set = this.waiting_[dependency] || (this.waiting_[dependency] = new Set());
      waiting_set.add(name);
    }

    entry.status = new this.constructor.StatusPending({ waiting_for });
  }
