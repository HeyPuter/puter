...
import { DEFAULT_REPORT_HOURS } from './TeamBillingService.ts';
...
describe('team billing events', () => {
    ...
    const makeWorkspace = async () =>
        service.createWorkspace(owner.id, {
            name: 'Billing Co',
            handle: freeHandle(),
        });
    ...
    /** Same, for a workspace whose owner is not the shared one. */
    ...
    beforeAll(async () => {
        // The cap has its own suite; these tests need many workspaces.
    ...
    it('emits account-created once, naming the workspace and the owner', async () => {
    ...
    it('emits one disabled event per seat plus a workspace event on delete', async () => {
        const team = await makeWorkspace();
    ...
    it('says nothing about an account no workspace pays for', async () => {
