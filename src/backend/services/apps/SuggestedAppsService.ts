...
const CODE_EXTS = new Set([
    'js', 'jsx', 'ts', 'tsx', 'json', 'json5', 'jsonl', 'css', 'scss',
    'sass', 'less', 'html', 'htm', 'xhtml', 'xml', 'svg', 'yaml', 'yml',
    'toml', 'ini', 'conf', 'cfg', 'env', 'sh', 'bash', 'zsh', 'fish',
    'bat', 'cmd', 'ps1', 'py', 'pyw', 'rb', 'php', 'pl', 'pm', 'lua',
    'java', 'kt', 'kts', 'scala', 'groovy', 'go', 'rs', 'c', 'h', 'cpp',
    'hpp', 'cc', 'cxx', 'cs', 'swift', 'r', 'jl', 'ex', 'exs', 'erl',
    'hrl', 'clj', 'cljs', 'hs', 'ml', 'mli', 'fs', 'fsi', 'fsx', 'dart',
    'sql', 'graphql', 'gql', 'proto', 'makefile', 'cmake', 'dockerfile',
    'tf', 'hcl', 'nix', 'vim', 'el', 'lisp', 'rkt', 'scm', 'asm',
    's', 'wasm', 'wat', 'v', 'vhd', 'vhdl', 'tcl',
]);

const IMAGE_EXTS = new Set(['jpg', 'jpeg', 'png', 'gif', 'webp', 'svg', 'bmp', 'ico', 'tiff', 'tif']);
const MEDIA_EXTS = new Set(['mp4', 'webm', 'mpg', 'mpeg', 'avi', 'mov', 'mkv', 'mp3', 'm4a', 'ogg', 'wav', 'flac', 'aac']);
...
export class SuggestedAppsService extends PuterService {
    async getSuggestedApps (
        entry: { name?: string; path?: string },
    ): Promise<Array<Record<string, unknown>>> {
        const name = entry.name ?? (entry.path ? pathPosix.basename(entry.path) : '');
