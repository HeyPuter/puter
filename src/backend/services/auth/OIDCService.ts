...
export class OIDCService extends PuterService {
    ...
    signState (payload: Record<string, unknown>): string {
        return this.services.token.sign('oidc-state', payload, { expiresIn: STATE_EXPIRY_SEC });
    }

    verifyState (token: string): Record<string, unknown> | null {
        try {
            return this.services.token.verify<Record<string, unknown>>('oidc-state', token);
        } catch {
            return null;
        }
    }
    ...
    signRevalidation (userId: number): string {
        return this.services.token.sign('oidc-state', {
            user_id: userId,
            purpose: 'revalidate',
    ...
    async exchangeCodeForTokens (
        ...
        if ( ! res.ok ) {
            console.warn('[oidc] token exchange failed', { status: res.status, body: await res.text() });
            return null;
        }
    ...
    async findUserByProviderSub (provider: string, providerSub: string): Promise<UserRow | null> {
        const link = await this.stores.oidc.getByProviderSub(provider, providerSub);
        if ( ! link ) return null;
    ...
    async createUserFromOIDC (
        ...
        if ( claims.email_verified === false ) {
            return { success: false, error: 'Provider did not verify this email address.' };
        }
        ...
        do {
            ...
            attempts++;
            if ( attempts > 20 ) return { success: false, error: 'Failed to generate unique username.' };
    ...
    async #fetchGoogleDiscovery (): Promise<Record<string, string> | null> {
        ...
        try {
            ...
            if ( ! res.ok ) return null;
            this.#googleDiscovery = await res.json() as Record<string, string>;
