...
export class PeerController extends PuterController {
    ...
    registerRoutes (router: PuterRouter): void {
        router.get('/peer/signaller-info', { subdomain: 'api' }, this.#signallerInfo);
        router.post('/peer/generate-turn', { subdomain: 'api' }, this.#generateTurn);
        router.post('/turn/ingest-usage', { subdomain: '*' }, this.#ingestUsage);
    }
    ...
    #generateTurn = async (req: Request, res: Response): Promise<void> => {
        ...
        if ( ! cfRes.ok ) {
            ...
            console.warn('[peer] Cloudflare TURN credential generation failed', cfRes.status, body);
    ...
    #ingestUsage = async (req: Request, res: Response): Promise<void> => {
        ...
        for ( const record of records ) {
            ...
            if ( userUuid ) {
                ...
                this.clients.event.emit('turn.egress', {
                    user_uuid: userUuid,
