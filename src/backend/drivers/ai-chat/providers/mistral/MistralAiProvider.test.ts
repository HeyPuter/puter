 ...
 *
 * The `@mistralai/mistralai` SDK is mocked so the provider never reaches
 * the network — we drive `chat.complete` / `chat.stream` directly to
 * assert how MistralAIProvider shapes requests (Mistral's camelCase
 * tool_calls/tool_call_id rewrite, route selection by stream flag),
 * surfaces non-streamed/streamed responses (with the camelCase usage
 * coercion and the `chunk.data` un-wrapping), maps errors, and reports
 * metering. The companion integration test
 * (MistralAiProvider.integration.test.ts) exercises the real Mistral
 * endpoint.
 */
...
const makeProvider = () => {
    ...
    const provider = new MistralAIProvider({ apiKey: 'test-key' }, metering);
