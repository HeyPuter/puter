 ...
 * we drive `chat.completions.create` directly to assert how ZAIProvider
 * shapes requests (custom GLM params, user_id derivation, cache_control
 * stripping), surfaces streamed/non-streamed responses, normalizes
 * GLM-specific `reasoning_content` to `reasoning`, maps errors, and
 * reports metering. The companion integration test
 * (ZAIProvider.integration.test.ts) exercises the real Z.AI endpoint.
 */
...
const makeProvider = (config: { apiKey?: string; apiBaseUrl?: string } = {}) => {
    ...
    const provider = new ZAIProvider(
        { apiKey: config.apiKey ?? 'test-key', ...(config.apiBaseUrl ? { apiBaseUrl: config.apiBaseUrl } : {}) },
        metering,
    );
