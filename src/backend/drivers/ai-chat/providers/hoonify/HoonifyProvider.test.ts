...
describe('HoonifyProvider model catalog', () => {
    ...
    it('does not alias qwen/qwen3.6-27b (owned by the Alibaba provider)', () => {
        // The driver registers Hoonify as an aggregator: a colliding alias
        // drops the whole model from the routing map, so this alias must
        // never be added while Alibaba claims the name.
        const { provider } = makeProvider();
        expect(provider.list()).not.toContain('qwen/qwen3.6-27b');
    });
});
