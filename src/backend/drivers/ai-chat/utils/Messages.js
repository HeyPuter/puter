...
export const normalize_single_message = (message, params = {}) => {
    ...
    if (message.role === 'tool') {
        const tool_use_id =
            message.tool_call_id || message.tool_use_id || message.id;
    ...
    for (let i = 0; i < message.content.length; i++) {
        ...
        if (
            typeof message.content[i].text === 'string' &&
            !message.content[i].type
        ) {
...
export const normalize_messages = (messages, params = {}) => {
    ...
    for (let i = 0; i < messages.length; i++) {
        ...
        if (can_merge) {
            merged_messages[merged_messages.length - 1].content.push(
                ...messages[i].content,
            );
        } else {
