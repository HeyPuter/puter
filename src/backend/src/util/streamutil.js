 ...
 * @param {*} offset
 */
const offset_write_stream = ({ originalDataStream, newDataStream, offset, replace_length = 0 }) => {
  ...
  const original_stream_on_data = (chunk) => {
    ...
    if (implied.state === STATE_ORIGINAL_STREAM && chunk.length >= remaining) {
      defer_buffer = chunk.slice(remaining);
      console.log('deferred:', defer_buffer.toString());
      chunk = chunk.slice(0, remaining);
    }

    if (implied.state === STATE_CONTINUE && replaced_bytes < replace_length) {
      ...
      if (defer_buffer.length > 0) {
        ...
        if (replaced_bytes < replace_length) {
          if (defer_buffer.length <= remaining_replacement) {
            ...
            console.log(
              'skipping deferred',
              defer_buffer.slice(0, remaining_replacement).toString()
            );
...
class StuckDetectorStream extends Transform {
  constructor(options, { timeout, on_stuck, on_unstuck }) {
    super(options);
...
const stuck_detector_stream = (source, { timeout, on_stuck, on_unstuck }) => {
...
async function* chunk_stream(stream, chunk_size = 1024 * 1024 * 5, expected_chunk_time) {
  let buffer = Buffer.alloc(chunk_size);
  let offset = 0;

  const chunk_time_ewma = expected_chunk_time !== undefined ? expected_chunk_time : null;
