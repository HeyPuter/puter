...
const programmer_errors = ['Assignment to constant variable.'];

const error_help_details = [
  {
    match: ({ message }) => message.startsWith('No suitable path found for'),
    apply(more) {
      more.references = [...reused.runtime_env_references];
    },
  },
  {
    match: ({ message }) => message.match(/^No (read|write) permission for/),
    apply(more) {
      more.solutions = [
        {
  ...
  {
    match: ({ message }) => message.startsWith('No valid config file found in path'),
    apply(more) {
      more.solutions = [
        {
  ...
  {
    match: ({ message }) => message === `config_name is required`,
    apply(more) {
      more.solutions = [
        'ensure config_name is present in your config file',
        'Seek help on ' + osclink('https://discord.gg/PQcx7Teh8u', 'our Discord server'),
      ];
    },
  },
  {
    match: ({ message }) => message == 'Assignment to constant variable.',
    apply(more) {
      more.references = [
        {
  ...
  {
    match: ({ message }) => programmer_errors.includes(message),
    apply(more) {
      more.notes = ['It looks like this might be our fault.'];
      more.solutions = [
        {
          title: `Check for an issue on ` + osclink('https://github.com/HeyPuter/puter/issues'),
        },
        {
          title:
            `If there's no issue, please ` +
            osclink('https://github.com/HeyPuter/puter/issues/new', 'create one') +
            '.',
        },
      ];
    },
  ...
  {
    match: ({ message }) => message.startsWith('Expected double-quoted property'),
    apply(more) {
      more.notes = ['There might be a trailing-comma in your config'];
    },
  },
];
...
const print_error_help = (err, out = process.stdout) => {
  ...
  const wrap_msg = (s) => `\x1B[31;1m┏━━ [ HELP:\x1B[0m ${quot(s)} \x1B[31;1m]\x1B[0m`;
  const wrap_list_title = (s) => `\x1B[36;1m${s}:\x1B[0m`;
  ...
  if (err.more.references?.length > 0) {
    ...
    for (const ref of err.more.references) {
      write(`  - ${vis('[', ']', ref.subject)} ` + `${vis('(', ')', ref.location)};\n`);
      write(`      ${ref.use}\n`);
      if (ref.url) {
