...
const PERMISSION_SCANNERS = [
        ...
        for (const implicator of _permission_implicators) {
          ...
          if (implied) {
            reading.push({
              ...
              by: implicator.id,
              data: implied,
              ...(!!actor.type.user ? { holder_username: actor.type.user.username } : {}),
            });
  ...
  },
  {
    ...
    async scan(a) {
      ...
      let sql_perm = permission_options
        .map((perm) => {
          return `\`permission\` = ?`;
        })
        .join(' OR ');
      ...
      const rows = await db.read(
        'SELECT * FROM `user_to_user_permissions` ' + 'WHERE `holder_user_id` = ? AND ' + sql_perm,
        [actor.type.user.id, ...permission_options]
      );
  ...
  },
  {
    ...
    async scan(a) {
      ...
      for (const issuer_username in hardcoded_user_group_permissions) {
        ...
        const issuer_groups = hardcoded_user_group_permissions[issuer_username];
        for (const group_uid in issuer_groups) {
          ...
          for (const permission of permission_options) {
            ...
            const issuer_reading = await a.icall('scan', issuer_actor, permission);
  ...
  },
  {
    ...
    async scan(a) {
      ...
      let sql_perm = permission_options.map((perm) => `p.permission = ?`).join(' OR ');
  ...
  },
  {
    ...
    async scan(a) {
      ...
      let sql_perm = permission_options.map(() => `\`permission\` = ?`).join(' OR ');
      ...
      const rows = await db.read(
        'SELECT * FROM `user_to_app_permissions` ' +
          ...
          sql_perm,
        [actor.type.user.id, actor.type.app.id, ...permission_options]
      );
  ...
  },
  {
    ...
    async scan(a) {
      ...
      // SELECT permission
      const rows = await db.read(
        'SELECT * FROM `dev_to_app_permissions` ' + 'WHERE `app_id` = ? AND ' + sql_perm,
        [actor.type.app.id, ...permission_options]
      );
