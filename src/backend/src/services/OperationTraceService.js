...
class OperationFrame {
  constructor({ parent, label, x }) {
    ...
    this.log = (x ?? Context)
      .get('services')
      .get('log-service')
      .create(`frame:${this.id}`, { concern: 'filesystem' });
  }
  ...
  set status(status) {
    ...
    this.log.info(
      `FRAME STATUS ${status.label} ` +
        (status !== this.effective_status_ ? `(effective: ${this.effective_status_.label}) ` : ''),
      {
        tags: this.tags,
        ...this.attributes,
      }
...
class OperationTraceService {
  ...
  add_frame_sync(label, x) {
    if (x) {
      this.log.noticeme('add_frame_sync() called with explicit context: ' + x.describe());
    }
...
class BaseOperation extends AdvancedBase {
  ...
  async run(values) {
    ...
    values.user = values.user ?? (values.actor ? values.actor.type.user : undefined);
