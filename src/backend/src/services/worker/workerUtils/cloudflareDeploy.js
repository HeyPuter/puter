...
async function getWorker(userData, authorization, workerId) {
  await cfFetch(
    `${WORKERS_BASE_URL}/scripts/${calculateWorkerName(userData.username, workerId)}`,
    'GET'
  );
}
async function createWorker(userData, authorization, workerId, body, PREAMBLE_LENGTH) {
  ...
  if (cfReturnCodes.success) {
    return JSON.stringify({
      success: true,
      errors: [],
      url: `${calculateWorkerName(userData.username, workerId)}.puter.work`,
    });
  } else {
    const parsedErrors = [];
