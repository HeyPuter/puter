...
class DevConsoleService extends BaseService {
  ...
  turn_on_the_warning_lights() {
     ...
     */
    this.add_widget(() => {
      return `\x1B[31;1m\x1B[5m *** ${Array(3).fill('WARNING').join(' ** ')} ***\x1B[0m`;
    });
  }
  ...
  update_() {
    ...
    const size_ok = () => process.stdout.rows - DESIRED_MIN_OUT > this.static_lines.length;
    let n_hidden = 0;
  ...
  async _init() {
    ...
    this._before_cmd = () => {
      ...
      console.log(`\x1B[33m` + this.generateSeparator(`[ Command Output ]`) + `\x1B[0m`);
    };
     ...
     */
    this._after_cmd = () => {
      console.log(`\x1B[33m` + this.generateEnd() + `\x1B[0m`);
    };
    ...
    this._post_write = () => {
      this.update_();
      // Draw separator bar
      process.stdout.write(`\x1B[36m` + this.generateSeparator() + `\x1B[0m\n`);

      // Input background disabled on Mac OS because it
      // has a - brace yourself - light-theme terminal 😱
      const drawInputBackground = process.platform !== 'darwin';
