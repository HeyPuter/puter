  ...
  ['__on_install.routes'](_, { app }) {
    ...
    Endpoint({
      ...
      handler: async (req, res) => {
        ...
        const can_see_all = actor && (await svc_permission.check(actor, PERM_SEE_ALL));
        const can_see_drivers = actor && (await svc_permission.check(actor, PERM_SEE_DRIVERS));
        ...
        for (const interface_name of col_interfaces.keys()) {
          ...
          interfaces[interface_name] = {
            spec: new Interface(iface, { name: interface_name }).serialize(),
            implementors: {},
          };
        }
        ...
        if (!can_see_all && !can_see_drivers) {
          ...
          for (const iface_name in interfaces) {
            for (const impl_name in interfaces[iface_name].implementors) {
              ...
              const can_see_this = actor && (await svc_permission.check(actor, perm));
              if (!can_see_this) {
