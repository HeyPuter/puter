...
class ThreadService extends BaseService {
    ...
    svc_permission.register_implicator(
      PermissionImplicator.create({
        ...
        checker: async ({ actor, permission }) => {
          ...
          if (thread.owner_user_id === actor.type.user.id && thread.parent_uid === null) {
            return {};
          }
  ...
  async init_event_listeners_() {
    ...
    svc_event.on('outer.thread.notify-subscribers', async (_, { uid, action, data }) => {
  ...
  install_threads_endpoints_({ router }) {
    ...
    Endpoint({
      ...
      handler: async (req, res) => {
        ...
        // Update thread
        await this.db.write('UPDATE `thread` SET text=? WHERE uid=?', [text, uid]);
    ...
    Endpoint({
      ...
      handler: async (req, res) => {
        ...
        // Update thread
        await this.db.write('DELETE FROM `thread` WHERE uid=?', [uid]);
  ...
  async get_thread({ uid }) {
    const [thread] = await this.db.read('SELECT * FROM `thread` WHERE uid=?', [uid]);
