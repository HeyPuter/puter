...
class AuthService extends BaseService {
  ...
  async authenticate_from_token(token) {
    ...
    if (decoded.type === 'actor-site') {
      ...
      const site = await svc_puterSite.get_subdomain_by_uid(site_uid);
  ...
  async check_session(cur_token, meta) {
    ...
    const user = await get_user({ uuid: is_legacy ? decoded.uuid : decoded.user_uid });
    if (!user) {
      return {};
    }
  ...
  async create_access_token(authorizer, permissions) {
    ...
    for (const permmission_spec of permissions) {
      ...
      const vals = Object.values(insert_object)
        .map((v) => '?')
        .join(', ');
      await this.db.write(
        'INSERT INTO `access_token_permissions` ' + `(${cols}) VALUES (${vals})`,
  ...
  async get_user_app_token_from_origin(origin) {
    ...
    const apps = await this.db.read('SELECT * FROM `apps` WHERE `uid` = ? LIMIT 1', [app_uid]);
