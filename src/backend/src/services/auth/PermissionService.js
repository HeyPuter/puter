...
class PermissionUtil {
  ...
  static split(permission) {
    return permission.split(':').map(PermissionUtil.unescape_permission_component);
  }

  static join(...components) {
    return components.map(PermissionUtil.escape_permission_component).join(':');
  }
  ...
  static reading_to_options(
    ...
    for (let finding of reading) {
      ...
      if (finding.$ === 'path') {
        ...
        const new_extras = finding.data ? [finding.data, ...extras] : [];
...
class PermissionService extends BaseService {
  ...
  async scan(actor, permission_options, _reserved, state) {
    const svc_trace = this.services.get('traceService');
    return await svc_trace.spanify(
      `permission:scan`,
      async () => {
  ...
  async scan_(actor, permission_options, _reserved, state) {
    ...
    await require('../../structured/sequence/scan-permission').call(this, {
      actor,
      permission_options,
      reading,
      state,
  ...
  async grant_user_app_permission(actor, app_uid, permission, extra = {}, meta) {
    ...
    await this.db.write(
      ...
      [actor.type.user.id, app_id, permission, JSON.stringify(extra), JSON.stringify(extra)]
    );
    ...
    const sql_cols = Object.keys(audit_values)
      .map((key) => `\`${key}\``)
      .join(', ');
    const sql_vals = Object.keys(audit_values)
      .map((key) => `?`)
      .join(', ');
  ...
  async grant_user_user_permission(actor, username, permission, extra = {}, meta) {
    ...
    await this.db.write(
      ...
      [user.id, actor.type.user.id, permission, JSON.stringify(extra), JSON.stringify(extra)]
    );
  ...
  async grant_user_group_permission(actor, gid, permission, extra = {}, meta) {
    ...
    await this.db.write(
      ...
      [actor.type.user.id, group.id, permission, JSON.stringify(extra), JSON.stringify(extra)]
    );
  ...
  async revoke_user_user_permission(actor, username, permission, meta) {
    ...
    await this.db.write(
      ...
      [user.id, actor.type.user.id, permission]
    );
  ...
  async revoke_user_group_permission(actor, gid, permission, meta) {
    ...
    await this.db.write(
      ...
      [actor.type.user.id, group.id, permission]
    );
