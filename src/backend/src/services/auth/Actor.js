...
const PRIVATE_UID_NAMESPACE = config.private_uid_namespace ?? require('crypto').randomUUID();
const PRIVATE_UID_SECRET =
  config.private_uid_secret ?? require('crypto').randomBytes(24).toString('hex');
...
class Actor extends AdvancedBase {
  ...
  get private_uid() {
    ...
    const hmac = this.modules.crypto
      .createHmac('sha256', PRIVATE_UID_SECRET)
      .update(this.uid)
      .digest('hex');
