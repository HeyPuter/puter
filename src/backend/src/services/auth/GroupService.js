...
class GroupService extends BaseService {
   ...
   */
  async get({ uid }) {
    const [group] = await this.db.read('SELECT * FROM `group` WHERE uid=?', [uid]);
    if (!group) return;
  ...
  async create({ owner_user_id, extra, metadata }) {
    ...
    await this.db.write(
      'INSERT INTO `group` ' +
        ...
        'VALUES (?, ?, ?, ?)',
      [uid, owner_user_id, JSON.stringify(extra), JSON.stringify(metadata)]
    );
  ...
  async list_public_groups() {
    ...
    groups = await this.db.read(
      'SELECT * FROM `group` WHERE uid IN (' + public_group_uids.map(() => '?').join(', ') + ')',
      public_group_uids
    );
    for (const group of groups) {
      group.extra = this.db.case({
  ...
  async add_users({ uid, users }) {
    const question_marks = '(' + Array(users.length).fill('?').join(', ') + ')';
    await this.db.write(
