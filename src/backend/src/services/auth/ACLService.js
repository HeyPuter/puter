...
class ACLService extends BaseService {
  ...
  async stat_user_user(issuer, holder, resource) {
    ...
    while (!(await perm_fsNode.get('is-root'))) {
      ...
      const these_permissions = await svc_perm.query_issuer_holder_permissions_by_prefix(
        issuer,
        holder,
        prefix
      );
  ...
  async _check_fsNode(actor, fsNode, mode) {
    ...
    if (actor.type instanceof AppUnderUserActorType) {
      ...
      if (
        ...
        ((await appdata_node.is(fsNode)) || (await appdata_node.is_above(fsNode)))
      ) {
