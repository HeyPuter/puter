...
class WSPushService extends BaseService {
  ...
  async _init() {
    ...
    this.svc_event.on('fs.storage.upload-progress', this._on_upload_progress.bind(this));
    this.svc_event.on('fs.storage.progress.*', this._on_upload_progress.bind(this));
    this.svc_event.on('puter-exec.submission.done', this._on_submission_done.bind(this));
    this.svc_event.on('outer.gui.*', this._on_outer_gui.bind(this));
  }
  ...
  async _on_upload_progress(key, data) {
    ...
    const ws_event_name = metadata.call_it_download ? 'download.progress' : 'upload.progress';
