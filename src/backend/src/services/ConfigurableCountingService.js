...
class ConfigurableCountingService extends BaseService {
  ...
  static sql_columns = {
    uint: ['value_uint_1', 'value_uint_2', 'value_uint_3'],
  };
  ...
  async increment({ service_name, service_type, values }) {
    ...
    // the keys.
    const pricing_category_str = counting_type.category
      .map((category) => {
    ...
    const duplicate_update_part = `count = count + 1${
    ...
    const sql =
      `INSERT INTO monthly_usage_counts (${Object.keys(required_data).join(
        ', '
      )}, count, ${custom_col_names.join(', ')}) ` +
      `VALUES (${Object.keys(required_data)
        .map(() => '?')
        .join(', ')}, 1, ${custom_col_values.map(() => '?').join(', ')}) ` +
      this.db.case({
        mysql: 'ON DUPLICATE KEY UPDATE ' + duplicate_update_part,
        sqlite: `ON CONFLICT(${identifying_keys
          .map((v) => `\`${v}\``)
          .join(', ')}) DO UPDATE SET ${duplicate_update_part}`,
      });
    const value_array = [
