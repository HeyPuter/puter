...
class ShareService extends BaseService {
    ...
    svc_event.on('user.email-confirmed', async (_, { user_uid, email }) => {
      ...
      for (const share of relevant_shares) {
        share.data = this.db.case({
          mysql: () => share.data,
          otherwise: () => JSON.parse(share.data ?? '{}'),
        })();
        ...
        for (const permission of share.data.permissions) {
          await svc_acl.set_user_user(issuer_actor, user.username, permission, undefined, {
            only_if_higher: true,
          });
        }

        await this.db.write('DELETE FROM share WHERE uid = ?', [share.uid]);
      }
    });
  }
  ...
  install_sharelink_endpoints({ app }) {
    ...
    Endpoint({
      ...
      handler: async (req, res) => {
        ...
        if (actor.type.user.email_confirmed && actor.type.user.email === share.recipient_email) {
        ...
        svc_notification.notify(UsernameNotifSelector(issuer_user.username), {
          source: 'sharing',
          title:
            `User ${actor.type.user.username} is ` +
  ...
  async get_share({ uid }) {
    const [share] = await this.db.read('SELECT * FROM share WHERE uid = ?', [uid]);
   ...
   */
  async create_share({ issuer, email, data }) {
    const require = this.require;
