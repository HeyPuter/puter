...
} = require('./auth/PermissionService');
...
class PuterSiteService extends BaseService {
      ...
      PermissionImplicator.create({
        ...
        checker: async ({ actor, permission }) => {
          ...
          const site_node = await svc_fs.node(
            new NodeInternalIDSelector('mysql', actor.type.site.root_dir_id)
          );
  ...
  async get_subdomain(subdomain) {
    ...
    const rows = await this.db.read(`SELECT * FROM subdomains WHERE subdomain = ? LIMIT 1`, [
      subdomain,
    ]);
  ...
  async get_subdomain_by_uid(uid) {
    const rows = await this.db.read(`SELECT * FROM subdomains WHERE uuid = ? LIMIT 1`, [uid]);
    if (rows.length === 0) return null;
    return rows[0];
  }
}
