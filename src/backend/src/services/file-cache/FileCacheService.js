...
class FileCacheService extends AdvancedBase {
  ...
  async try_get_(fsNode, opt_log) {
    ...
    if (tracker.phase === FileTracker.PHASE_PENDING) {
      Promise.race([tracker.p_ready, new Promise((resolve) => setTimeout(resolve, 2000))]);
    }
  ...
  async invalidate(fsNode_or_uid) {
    const key = typeof fsNode_or_uid === 'string' ? fsNode_or_uid : await fsNode_or_uid.get('uid');
  ...
  async _precache_evict(capacity_needed) {
    ...
    const sorted = Array.from(this.uid_to_tracker.values()).sort((a, b) => b.score - a.score);
