...
class FSLockService extends BaseService {
        ...
        handler: async (args, log) => {
          for (const path in this.locks) {
            ...
            if (this.locks[path].effective_mode === MODE_READ) {
              ...
              log.log(line);
            } else if (this.locks[path].effective_mode === MODE_WRITE) {
              line += 'WRITING';
              log.log(line);
            } else {
              line += 'UNKNOWN';
              ...
              const lines = JSON.stringify(this.locks[path], null, 2).split('\n');
  ...
  async lock_path(path, mode) {
    ...
    this.log.noticeme('WAITING FOR LOCK: ' + path + ' ' + mode.toString());
