...
export class DBKVStore {
    ...
    #meteringService;
    #global_config = {};
    constructor ({ sqlClient, meteringService, globalConfig }) {
        this.#db = sqlClient;
    ...
    async get ({ key }) {
        ...
        if ( Array.isArray(key) ) {
            ...
            const rows = app
                ...
                : await this.#db.read(`SELECT kkey, value, expireAt FROM kv WHERE user_id=? AND (app IS NULL OR app = '${GLOBAL_APP_KEY}') AND kkey_hash IN (${placeholders})`, params);
            const kvPairs = {};
            rows.forEach((row) => {
            ...
            return keys.map((key) => Object.prototype.hasOwnProperty.call(kvPairs, key) ? kvPairs[key] : null);
        }
        ...
        const kv = app
            ...
            : await this.#db.read(`SELECT * FROM kv WHERE user_id=? AND (app IS NULL OR app = '${GLOBAL_APP_KEY}') AND kkey_hash=? LIMIT 1`, [user.id, key_hash]);
        if ( kv[0] ) {
    ...
    async list ({ as }) {
        ...
        let rows = app
            ...
            : await this.#db.read(`SELECT kkey, value, expireAt FROM kv WHERE user_id=? AND (app IS NULL OR app = '${GLOBAL_APP_KEY}')`, [user.id]);
    ...
    async decr ({ key, pathAndAmountMap }) {
        return this.incr({ key, pathAndAmountMap: Object.fromEntries(Object.entries(pathAndAmountMap).map(([k, v]) => [k, -v])) });
    }
    async #expireat (key, timestamp) {
        const actor = Context.get('actor');
        const app = actor.type?.app ?? undefined;
...
//# sourceMappingURL=DBKVStore.js.map