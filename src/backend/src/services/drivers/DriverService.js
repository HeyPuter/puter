...
class DriverService extends BaseService {
  ...
  _init() {
    ...
    svc_apiError.register({
      ...
      method_not_found: {
        status: 404,
        message: ({ interface_name, method_name }) =>
          `Method not found: ${quot(method_name)} on interface ${quot(interface_name)}`,
      },
      no_implementation_available: {
        status: 502,
        message: ({ iface, interface_name, driver }) =>
          `No implementation available for ` + (iface ?? interface_name)
            ? 'interface'
            : 'driver' + ' ' + quot(iface ?? interface_name ?? driver) + '.',
      },
    });
  }
  ...
  async ['__on_registry.entries']() {
    ...
    await services.emit('driver.register.interfaces', { col_interfaces });
    await services.emit('driver.register.drivers', { col_drivers });
  }
  ...
  async _call({ driver, iface, method, args }) {
    ...
    const driver_service_exists = (() => {
      return this.services.has(driver) && this.services.get(driver).list_traits().includes(iface);
    })();
   ...
   */
  async call_new_({ actor, service, service_name, iface, method, args, skip_usage }) {
    if (!service) {
    ...
    if (!effective_policy) {
      throw new Error('policies with no effective user are not yet ' + 'supported');
    }
        ...
        },
        {
          ...
          on_return: async (result) => {
            if (result instanceof TypedValue) {
              ...
              let desired_type = method_spec.result_choices
  ...
  async _process_args(interface_name, method_name, args) {
    ...
    for (const [arg_name, arg_descriptor] of Object.entries(method.parameters)) {
      ...
      try {
        processed_args[arg_name] = await arg_behaviour.consolidate(ctx, arg_value, {
          arg_descriptor,
          arg_name,
        });
      } catch (e) {
