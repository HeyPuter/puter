...
class SqliteDatabaseAccessService extends BaseDatabaseAccessService {
  ...
  async _init() {
    ...
    const available_migrations = [
      ...
      [24, ['0027_emulator-app.dbmig.js']],
      [25, ['0028_clean-email.sql']],
      [26, ['0029_emulator_priv.sql']],
    ...
    const HIGHEST_VERSION = available_migrations[available_migrations.length - 1][0] + 1;
    /**
    ...
    svc_serverHealth.add_check('sqlite', async () => {
      ...
      if (user_version !== TARGET_VERSION) {
        throw new Error(
          `Database version mismatch: expected ${TARGET_VERSION}, ` + `got ${user_version}`
        );
      }
    });
  }
