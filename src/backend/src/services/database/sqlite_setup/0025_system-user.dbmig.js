...
if (existing_user) {
  ...
  for (;;) {
    [existing_user] = await read('SELECT username FROM `user` WHERE username=?', [replace_name]);
  ...
  await write(`UPDATE \`user\` SET username=? WHERE username='system' LIMIT 1`, [replace_name]);
}
