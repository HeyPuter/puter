  ...
  async start(args) {
    ...
    for (;;) {
      ...
      for (let i = 0; i < fsentries.length; i++) {
        ...
        await dbrw.promise().execute(`UPDATE fsentries SET path=? WHERE id=?`, [path, fsentry.id]);
      }
...
class Mig_FixTrash extends Job {
  ...
  async start(args) {
    ...
    for (const { name, uuid } of user_dirs) {
      ...
      log.info(
        `Fixing trash for user ${user.username} ${user.id} ${user_dir_uuid} ${trash_uuid} ${trash_ts}`
      );
