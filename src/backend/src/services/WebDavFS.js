...
// TODO DS: Move these to proper storage
/**
 * In-memory lock map. Key is filePath, value is lockToken and expiration.
 * @type {Object.<string, { path: string, expires: number }>}
 */
const lockMap = {}
/**
 * In-memory file lock map. Key is lockToken, value is { path, expires }
 ...
 * @returns
 */
const extractHeaderToken = (headerToken = '') => {
 ...
 * @returns {string | null}
 */
const getLockIfValid = (lockToken) => {
  const filePathAndExpiry = lockMap[lockToken]
  if (filePathAndExpiry && filePathAndExpiry.expires > Date.now()) {
    return filePathAndExpiry.path
  }
  // clear out expired lock
  delete lockMap[lockToken]
  fileLocksMap[filePathAndExpiry?.path]?.delete(lockToken)
  return null
}
 ...
 * @returns
 */
const createLock = (filePath) => {
  const lockToken = `urn:uuid:${crypto.randomUUID()}`
  const expires = Date.now() + 10000 // 10 seconds
  lockMap[lockToken] = { path: filePath, expires }
  if (!fileLocksMap[filePath]) {
    fileLocksMap[filePath] = new Set()
  } else {
    fileLocksMap[filePath].add(lockToken)
  }
  return lockToken
}
 ...
 * @returns
 */
const deleteLock = (lockToken, filePath) => {
  delete lockMap[lockToken]
  fileLocksMap[filePath]?.delete(lockToken)
  return lockToken
}
/**
 *
 ...
 * @returns
 */
const refreshLock = (lockToken, filePath) => {
  lockMap[lockToken] = { path: filePath, expires: Date.now() + 10000 }
  if (!fileLocksMap[filePath]) {
    fileLocksMap[filePath] = new Set()
  } else {
    fileLocksMap[filePath].add(lockToken)
  ...
  switch (req.method) {
    ...
    case 'PROPFIND':
      ...
      if (filePath === '/' || filePath === '') {
        ...
        res.end(
          convertMultipleToWebDAVPropfindXML(
            await operations.stat(rootNode),
            await operations.readdir(rootNode),
          ),
        )
        return
      }
...
<d:error xmlns:d="DAV:">
    <d:valid-resourcename>macOS metadata files not permitted</d:valid-resourcename>
        ...
        const contentLength =
          req.headers['content-length'] || req.headers['x-expected-entity-length'] // x-expected-entity-length is used by macOS Finder for some reason
        if (!contentLength) {
  ...
  <D:response>
    ...
    case 'LOCK':
      ...
      try {
        ...
        const { headerLockToken } = extractHeaderToken(ifHeader)
        const existingFileFromLock = getLockIfValid(headerLockToken)

        // Check if the resource exists
        if (!exists) {
          // handle non exsiting child folder if lock is present to refresh parent
          if (filePath.startsWith(existingFileFromLock)) {
            filePath = existingFileFromLock
          } else {
            res.status(404).end('Not Found')
            return
          }
        ...
        const locksOnFile = fileLocksMap[filePath]
        if (locksOnFile?.size && (!headerLockToken || existingFileFromLock !== `${filePath}`)) {
        ...
        const lockToken = headerLockToken
          ? refreshLock(headerLockToken, filePath)
          : createLock(filePath)
...
<D:prop xmlns:D="DAV:">
  <D:lockdiscovery>
    ...
    case 'UNLOCK':
      ...
      try {
        if (req.headers['x-litmus'].includes('21')) {
        ...
        if (existingFileFromLock) {
          if (existingFileFromLock === filePath) {
            deleteLock(headerLockToken, filePath)
