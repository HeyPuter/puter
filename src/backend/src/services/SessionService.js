...
class SessionService extends BaseService {
  ...
  async get_session_(uuid) {
    ...
    [session] = await this.db.read('SELECT * FROM `sessions` WHERE `uuid` = ? LIMIT 1', [uuid]);
    if (!session) return;
  ...
  remove_session(uuid) {
    ...
    return this.db.write('DELETE FROM `sessions` WHERE `uuid` = ?', [uuid]);
  }
  ...
  async _update_sessions() {
    ...
    for (const key of keys) {
      ...
      if (now - session.last_store > 5 * MINUTE) {
        ...
        const { anyRowsAffected } = await this.db.write(
          'UPDATE `sessions` ' + 'SET `meta` = ?, `last_activity` = ? ' + 'WHERE `uuid` = ?',
    ...
    for (const [user_id, last_touch] of Object.values(user_updates)) {
      ...
      await this.db.write(
        'UPDATE `user` ' + 'SET `last_activity_ts` = ? ' + 'WHERE `id` = ? LIMIT 1',
        [sql_ts, user_id]
      );
