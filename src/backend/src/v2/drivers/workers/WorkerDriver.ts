...
export class WorkerDriver extends PuterDriver {
    ...
    async create (args: Record<string, unknown>): Promise<unknown> {
        ...
        let workerName = String(args.workerName ?? '').toLowerCase();
        ...
        if ( ! WORKER_NAME_REGEX.test(workerName) ) {
            throw new HttpError(400, 'Worker name must be alphanumeric (plus _ and -)');
        }
        ...
        if ( existingWorkers.length >= MAX_WORKERS_PER_USER ) {
            throw new HttpError(403, `Worker limit reached (max ${MAX_WORKERS_PER_USER})`);
        }
        ...
        const cfResult = await this.#cfDeploy(workerName, authorization, preamble + sourceCode);
        return cfResult;
    }
    ...
    async getFilePaths (args: Record<string, unknown>): Promise<unknown[]> {
        ...
        return rows.map(r => {
            const name = String(r.subdomain ?? '').split('.').pop() ?? '';
            return {
                name,
                url: `https://${name}.puter.work`,
                file_path: r.root_dir_id ?? null,
                file_uid: r.root_dir_id ?? null,
                created_at: r.ts ? new Date(r.ts as string).toISOString() : null,
            };
        });
    }
    ...
    async #cfDeploy (workerName: string, authorization: string, code: string): Promise<Record<string, unknown>> {
        const cfg = this.#workerConfig();
        const metadata = JSON.stringify({
            ...
            bindings: [
                { type: 'secret_text', name: 'puter_auth', text: authorization },
                { type: 'plain_text', name: 'puter_endpoint', text: cfg.internetExposedUrl ?? 'https://api.puter.com' },
            ],
        });

        const form = new FormData();
        form.append('metadata', metadata);
        form.append('swCode', new Blob([code], { type: 'application/javascript' }));
        ...
        const errors = (json.errors ?? []).map(e => {
            ...
            const adjusted = lines.map(line => {
                if ( line.includes('at worker.js:') ) {
                    ...
                    positions[0] = String(Number(positions[0]) - preambleLineCount);
    ...
    async #handleFileWrite (data: unknown, meta: unknown): Promise<void> {
        ...
        for ( const row of matched ) {
            ...
            try {
                ...
                // Deploy
                const cfResult = await this.#cfDeploy(workerName, authorization, preamble + sourceCode) as { success?: boolean; errors?: unknown[]; url?: string };

                // Notify the user
                await this.#notifyUser(userId, workerName, cfResult);
            } catch ( err ) {
                console.warn(`[workers] hot-reload deploy failed for ${workerName}`, err);
    ...
    async #notifyUser (userId: number, workerName: string, result: { success?: boolean; errors?: unknown[]; url?: string }): Promise<void> {
        try {
