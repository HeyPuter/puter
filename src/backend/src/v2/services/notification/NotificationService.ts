...
export class NotificationService extends PuterService {
    ...
    override onServerStart (): void {
        ...
        this.clients.event.on('web.socket.user-connected', (_key: string, data: unknown) => {
            ...
            this.#connectTimeouts.set(userId, setTimeout(() => {
                this.#connectTimeouts.delete(userId);
                void this.#sendUnreads(userId).catch(err => {
                    console.warn('[notification] sendUnreads failed', err);
                });
            }, 2000));
        });
        ...
        this.clients.event.on('sent-to-user.notif.message', (_key: string, data: unknown) => {
            const d = data as { user_id?: number; response?: { uid?: string } } | undefined;
    ...
    async notify (
        ...
        this.clients.event.emit('outer.gui.notif.message', {
        ...
        const writePromise = (async () => {
            for ( const userId of userIds ) {
                try {
                    ...
                    console.warn(`[notification] persist failed for user ${userId}`, err);
                }
            }
        })();
        ...
        writePromise.then(() => {
            this.clients.event.emit('outer.gui.notif.persisted', {
                user_id_list: userIds,
                response: { uid },
            }, {});
        }).catch(() => { /* already logged per-user above */ });
    ...
    async #sendUnreads (userId: number): Promise<void> {
        ...
        this.clients.event.emit('outer.gui.notif.unreads', {
            user_id_list: [userId],
            response: { unreads },
        }, {});
    }
