...
import { get_app, get_user, id2path, id2uuid, is_empty, suggestedAppForFsEntry } from '../../helpers.js';
import { Actor, AppUnderUserActorType, UserActorType } from '../../services/auth/Actor.js';
...
import { getTracer, span } from '../../util/otelutil.js';
import { ECMAP } from './ECMAP.js';
import { NodeChildSelector, NodeInternalIDSelector, NodePathSelector, NodeRawEntrySelector, NodeUIDSelector, RootNodeSelector } from './node/selectors.js';
...
export const TYPE_FILE = { label: 'File' };
export const TYPE_DIRECTORY = { label: 'Directory' };
export default class FSNodeContext {
    ...
    async get (key, force) {
        ...
        */
        ...
        if ( key === 's3:location' ) {
            ...
            return {
                bucket: this.entry.bucket ?? config.s3_bucket ?? 'puter-local',
                bucket_region: this.entry.bucket_region ?? config.s3_region ?? config.region
                    ?? 'us-west-2',
                key: this.entry.uuid,
            };
        }
    ...
    async getSafeEntry (fetch_options = {}) {
        ...
        fsentry.dirname = dirname(fsentry.path);
