...
export async function createLock (
    ...
    // Per-token metadata
    await redis.set(`${LOCK_PREFIX}${lockToken}`, meta, 'EX', DAV_LOCK_TTL_SECONDS);
    ...
    await redis.set(`${LOCKS_PREFIX}${filePath}`, JSON.stringify(existing), 'EX', DAV_LOCK_TTL_SECONDS);

    return lockToken;
}

export async function deleteLock (redis: Cluster, lockToken: string): Promise<void> {
    ...
    if ( raw ) {
        ...
        if ( Object.keys(pathMap).length === 0 ) {
            ...
            await redis.set(`${LOCKS_PREFIX}${meta.path}`, JSON.stringify(pathMap), 'EX', DAV_LOCK_TTL_SECONDS);
        }
    }
    await redis.del(`${LOCK_PREFIX}${lockToken}`);
}

export async function refreshLock (redis: Cluster, lockToken: string): Promise<boolean> {
    const raw = await redis.get(`${LOCK_PREFIX}${lockToken}`);
    if ( ! raw ) return false;
    const meta = JSON.parse(raw) as { path: string };

    // Re-set with fresh TTL
    await redis.set(`${LOCK_PREFIX}${lockToken}`, raw, 'EX', DAV_LOCK_TTL_SECONDS);
    // Refresh the path map TTL too
    const pathRaw = await redis.get(`${LOCKS_PREFIX}${meta.path}`);
    if ( pathRaw ) {
        await redis.set(`${LOCKS_PREFIX}${meta.path}`, pathRaw, 'EX', DAV_LOCK_TTL_SECONDS);
    }
    return true;
}
 ...
 */
export async function getFileLocks (redis: Cluster, filePath: string): Promise<LockInfo[]> {
    ...
    for ( ;; ) {
        ...
        for ( const [token, info] of Object.entries(map) ) {
            results.push({
                lockToken: token,
                lockScope: (info as { lockScope: 'exclusive' | 'shared' }).lockScope,
 ...
 */
export async function getLockIfValid (redis: Cluster, lockToken: string): Promise<LockInfo | null> {
    const raw = await redis.get(`${LOCK_PREFIX}${lockToken}`);
    if ( ! raw ) return null;
    const meta = JSON.parse(raw) as { path: string; lockScope: 'exclusive' | 'shared'; lockType: 'write' };
...
async function getPathLockMap (redis: Cluster, filePath: string): Promise<Record<string, { lockScope: string; lockType: string }>> {
    const raw = await redis.get(`${LOCKS_PREFIX}${filePath}`);
    if ( ! raw ) return {};
    try {
        return JSON.parse(raw) as Record<string, { lockScope: string; lockType: string }>;
    } catch {
        return {};
    }
}
