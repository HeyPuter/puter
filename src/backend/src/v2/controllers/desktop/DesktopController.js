...
export class DesktopController {
    ...
    registerRoutes (router) {
        ...
        router.post('/set-desktop-bg', {
            ...
            if ( url !== undefined ) {
                if ( url !== null && typeof url !== 'string' ) {
                    throw new HttpError(400, '`url` must be a string or null');
                }
            ...
            if ( fit !== undefined ) {
                if ( fit !== null && typeof fit !== 'string' ) {
                    throw new HttpError(400, '`fit` must be a string or null');
                }
        ...
        router.post('/update-taskbar-items', {
            subdomain: 'api',
            requireUserActor: true,
        }, async (req, res) => {
            const { items } = req.body ?? {};
            if ( ! Array.isArray(items) ) {
                throw new HttpError(400, 'Missing or invalid `items` array');
            }
        ...
        router.post('/set_layout', {
            subdomain: 'api',
            requireUserActor: true,
        }, async (req, res) => {
            const { item_uid, item_path, layout } = req.body ?? {};
            if ( !layout || !ALLOWED_LAYOUTS.includes(layout) ) {
                throw new HttpError(400, `\`layout\` must be one of: ${ALLOWED_LAYOUTS.join(', ')}`);
            }
            await this.#updateFSEntry(req.actor.user.id, { item_uid, item_path }, { layout });
            res.json({});
        });
        ...
        router.post('/set_sort_by', {
            ...
            const { item_uid, item_path, sort_by, sort_order } = req.body ?? {};
            if ( !sort_by || !ALLOWED_SORT_BY.includes(sort_by) ) {
                throw new HttpError(400, `\`sort_by\` must be one of: ${ALLOWED_SORT_BY.join(', ')}`);
            }
            const resolvedOrder = sort_order ?? 'asc';
            if ( ! ALLOWED_SORT_ORDER.includes(resolvedOrder) ) {
                throw new HttpError(400, `\`sort_order\` must be one of: ${ALLOWED_SORT_ORDER.join(', ')}`);
            }
            await this.#updateFSEntry(req.actor.user.id, { item_uid, item_path }, {
                sort_by,
