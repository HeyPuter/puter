...
import { get_user } from '../../../helpers.js';
import APIErrorService from '../../../modules/web/APIErrorService.js';
import { Actor, UserActorType } from '../../../services/auth/Actor.js';
import { BaseOperation } from '../../../services/OperationTraceService.js';
import { Context } from '../../../util/context.js';
import FSNodeContext from '../FSNodeContext.js';
...
const typeCheckForRead = async (fsNode) => {
    if ( await fsNode.get('type') === FSNodeContext.TYPE_DIRECTORY ) {
        throw APIError.create('cannot_read_a_directory');
    }
};

export class LLRead extends BaseOperation {
    ...
    async _run ({ fsNode, no_acl, actor, offset, length, range, version_id } = {}) {
        ...
        if ( ! await fsNode.exists() ) {
            throw APIErrorService.create('subject_does_not_exist');
        }
        ...
        while ( type === FSNodeContext.TYPE_SYMLINK ) {
