...
class Metric {
  ...
  update_() {
    ...
    this.windowAverage = this.window.reduce((sum, v) => sum + v) / this.window.length;
...
class PerformanceMonitor extends BaseService {
  ...
  logMonitorContext(ctx) {
    if (!this.performanceMetrics.hasOwnProperty(ctx.name)) {
      this.performanceMetrics[ctx.name] = new Metric(config.windowSize ?? 30);
    }
    ...
    // Update averaging metrics
    {
      ...
      for (const stamp of ctx.stamps) {
        ...
        metricsToUpdate[stamp.name] = (metricsToUpdate[stamp.name] ?? 0) + (end - start);
      }
  ...
  updateMetric_(key, value) {
    const metric = this.performanceMetrics[key] ?? (this.performanceMetrics[key] = new Metric(30));
  ...
  async recordMetrics_() {
    ...
    try {
      ...
      console.error('Failed to send metrics to CloudWatch', e);
    }
  }
}
