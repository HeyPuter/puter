  ...
  ({ fs, path_ }) => ({
    ...
    skip_if_not_in_repo: ({ path }) => {
      const exists = fs.existsSync(
        path_.join(path, "../../.is_puter_repository")
      );
      return exists;
    },
    ...
    require_write_permission: ({ path }) => {
      try {
        fs.writeFileSync(
          path_.join(path, ".tmp_test_write_permission"),
          "test"
        );
...
const mod_paths =
  ...
  ({ path_ }) => [
    ...
    },
    {
      get path() {
        return path_.join(
          path_.dirname(entry_path || require.main.filename),
          "../mods"
        );
      },
      checks: [path_checks.skip_if_not_exists],
...
class RuntimeEnvironment extends AdvancedBase {
  ...
  init_() {
    ...
    if (!using_config || owrite_config) {
      ...
      generated_values.private_uid_secret = crypto
        .randomBytes(24)
        .toString("hex");
    ...
    environment.config_path = path_.join(
      config_path_entry.path,
      config_to_load
    );
