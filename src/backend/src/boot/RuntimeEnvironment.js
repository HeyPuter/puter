...
const path_checks =
  ...
  ({ fs, path_ }) => ({
    ...
    contains_config_file: ({ path }) => {
      const valid_config_names = ['config.json', 'config.json5'];
  ...
  ({ path_ }) => [
    {
      ...
      checks: [path_checks.require_if_not_undefined],
    },
    {
      path: '/etc/puter',
...
const runtime_paths =
  ...
  ({ path_ }) => [
    ...
    {
      path: '/var/puter',
      checks: [path_checks.skip_if_not_exists, path_checks.env_not_set('NO_VAR_RUNTIME')],
    },
    {
      get path() {
...
const mod_paths =
  ...
  ({ path_ }) => [
    ...
    {
      path: '/var/puter/mods',
      checks: [path_checks.skip_if_not_exists, path_checks.env_not_set('NO_VAR_MODS')],
    },
    {
      get path() {
...
class RuntimeEnvironment extends AdvancedBase {
  ...
  init_() {
    ...
    if (!using_config || owrite_config) {
      ...
      if (using_config) {
        this.logger.info(
          `Overwriting ${quot(using_config)} because ` + `${hl('--overwrite-config')} is set`
        );
        // make backup
        fs.copyFileSync(
    ...
    if (!config.mod_directories) {
      config.mod_directories = ['{source}/../mods/mods_enabled', '{source}/../extensions'];
    }
  ...
  get_first_suitable_path_(meta, paths, last_checks) {
    for (const entry of paths) {
      ...
      this.logger.info(`Checking path ${quot(entry.label ?? entry.path)} for ${meta.pathFor}...`);

      let checks_pass = true;
      for (const check of checks) {
        this.logger.info(`-> doing ${quot(check.name)} on path ${quot(entry.path)}...`);
        const result = check(entry);
        if (result === false) {
          this.logger.info(`-> ${quot(check.name)} doesn't like this path`);
          checks_pass = false;
          break;
        }
      }
      ...
      this.logger.info(`${hl('USING')} ${quot(entry.path)} for ${meta.pathFor}.`);
