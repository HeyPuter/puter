  ...
  static METHODS = {
    ...
    async upsert(entity, extra) {
      if (await app_name_exists(await entity.get('name'))) {
        ...
        const is_name_change =
          !old_entity || (await old_entity.get('name')) !== (await entity.get('name'));
      ...
      if (extra.old_entity) {
        await this.db.write(`DELETE FROM app_filetype_association WHERE app_id = ?`, [insert_id]);
      }
      ...
      const has_new_icon =
        !extra.old_entity || (await entity.get('icon')) !== (await extra.old_entity.get('icon'));
      ...
      const has_new_name =
        extra.old_entity && (await entity.get('name')) !== (await extra.old_entity.get('name'));
...

      {
        ...
        const full_entity = old_entity ? await (await old_entity.clone()).apply(entity) : entity;
        // Update app cache
        const raw_app = {
    ...
    async read_transform(entity) {
      ...
      const stats = await svc_appInformation.get_stats(await entity.get('uid'), {
        period: Context.get('es_params')?.stats_period,
        grouping: Context.get('es_params')?.stats_grouping,
        created_at: await entity.get('created_at'),
      });
      ...
      entity.set(
        ...
        await (async () => {
          ...
          try {
            ...
            return expected_uid === (await entity.get('uid')) ? origin : null;
          } catch (e) {
    ...
    async maybe_insert_subdomain_(entity) {
      ...
      if (await entity.get('source_directory')) {
        await (await entity.get('source_directory')).fetchEntry();
