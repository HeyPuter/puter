...
class SQLES extends BaseES {
  ...
  static METHODS = {
    ...
    async read(uid) {
      const [stmt_where, where_vals] = await (async () => {
        if (typeof uid !== 'object') {
          ...
          let id_col = id_prop.descriptor.sql?.column_name ?? id_prop.name;
        ...
        if (!uid.hasOwnProperty('predicate')) {
          throw new Error(
            'SQLES.read does not understand this input: ' + 'object with no predicate property'
          );
        }
      ...
      const stmt = `SELECT * FROM ${this.om.sql.table_name}${stmt_where}`;
    ...
    async select({ predicate, limit, offset }) {
      ...
      let stmt = `SELECT * FROM ${this.om.sql.table_name}${stmt_where}`;
    ...
    async upsert(entity, extra) {
      ...
      for (const prop of Object.values(this.om.properties)) {
        ...
        const values = [];
        let stmt = `SELECT COUNT(*) FROM ${this.om.sql.table_name} WHERE ${col_name} = ?`;
        values.push(value);
    ...
    async delete(uid, extra) {
      ...
      const stmt = `DELETE FROM ${this.om.sql.table_name} WHERE ${id_col} = ?`;
    ...
    async create_(entity) {
      ...
      const sql_placeholders = Object.keys(sql_data)
        .map(() => '?')
        .join(', ');
      const execute_vals = Object.values(sql_data);

      const stmt = `INSERT INTO ${this.om.sql.table_name} (${sql_cols}) VALUES (${sql_placeholders})`;
    ...
    async update_(entity, old_entity) {
      ...
      const sql_assignments = Object.keys(sql_data)
        .map((col_name) => {
          return `${col_name} = ?`;
        })
        .join(', ');
      ...
      const id_prop = this.om.properties[this.om.primary_identifier];
      const id_col = id_prop.descriptor.sql?.column_name ?? id_prop.name;

      const stmt = `UPDATE ${this.om.sql.table_name} SET ${sql_assignments} WHERE ${id_col} = ?`;
    ...
    async om_to_sql_condition_(om_query) {
      ...
      if (om_query instanceof And) {
        ...
        const sql = child_raw_conditions
          .map((sql) => {
            return `(${sql})`;
          })
          .join(' AND ');
