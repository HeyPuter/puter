...
class WebServerService extends BaseService {
  ...
  async ['__on_start.webserver']() {
    ...
    globalThis.deployment_type =
      config.http_port === 5101 ? 'green' : config.http_port === 5102 ? 'blue' : 'not production';
    ...
    this.startup_widget = () => {
      ...
      const lines = [`Puter is now live at: ${link}`, `Type web:dismiss to un-stick this message`];
      const lengths = [`Puter is now live at: `.length + url.length, lines[1].length];
  ...
  async _init() {
    ...
    // Instrument logging to use our log service
    {
      ...
      const stream = {
        write: (message) => {
          ...
          fields.url = safe_url;
          // re-write message
          message = [fields.method, fields.url, fields.status, fields.responseTime].join(' ');
    ...
    app.use(function (req, res, next) {
      req.co_isolation_enabled =
        ['Chrome', 'Edge'].includes(req.ua.browser.name) && Number(req.ua.browser.major) >= 110;
      next();
    });
    ...
    app.use(function (req, res, next) {
      ...
      const is_site =
        req.hostname.endsWith(config.static_hosting_domain) || req.hostname === 'docs.puter.com';
      ...
      const co_isolation_okay =
        (!is_popup || is_parent_co) && (is_app || !is_site) && req.co_isolation_enabled;
      if (req.path === '/signup' || req.path === '/login') {
        res.setHeader('Access-Control-Allow-Origin', origin ?? '*');
      }
      // Website(s) to allow to connect
      if (config.experimental_no_subdomain || req.subdomains[req.subdomains.length - 1] === 'api') {
        res.setHeader('Access-Control-Allow-Origin', origin ?? '*');
      }
