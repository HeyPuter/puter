...
const {
  surrounding_box,
  es_import_promise,
} = require("../../fun/dev-console-ui-utils.js");
...
class WebServerService extends BaseService {
  ...
  async ["__on_start.webserver"]() {
    ...
    globalThis.deployment_type =
      config.http_port === 5101
        ? "green"
        : config.http_port === 5102
          ? "blue"
          : "not production";
    ...
    const setupJustCompleted = config.setup_just_completed === true;
    if (setupJustCompleted) {
      this.log.info("Setup was just completed, forcing port reassignment");
      // Clear the flag so we don't do this again on the next restart
      config.setup_just_completed = false;

      // Force port to be reassigned by setting to 'auto'
      config.http_port = "auto";
    }
    ...
    // If setup was just completed, try to ensure we don't get the same port
    if (setupJustCompleted && !process.env.PORT) {
      // Try to avoid the port that was just used by the setup wizard
      // by moving it to the end of the list
      const firstPort = ports_to_try[0];
      ports_to_try.shift();
      ports_to_try.push(firstPort);
      this.log.info(
        `Moved port ${firstPort} to end of list to avoid conflict with setup wizard`
      );
    }
    ...
    this.log.info(`Server is running on port ${config.http_port}`);
