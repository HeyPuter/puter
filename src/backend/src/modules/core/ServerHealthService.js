    ...
    promise.asyncSafeSetInterval(
      async () => {
        ...
        for (const { name, fn, chainable } of this.checks_) {
          ...
          try {
            await Promise.race([fn(), p_timeout]);
            ...
            svc_alarm.create('health-check-failure', `Health check ${name} failed`, { error: err });
      ...
      null,
      {
        onBehindSchedule: (drift) => {
          svc_alarm.create('health-checks-behind-schedule', 'Health checks are behind schedule', {
            drift,
          });
        },
      }
