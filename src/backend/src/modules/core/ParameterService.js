...
class ParameterService extends BaseService {
  ...
  createParameters(serviceName, parameters, opt_instance) {
    for (const parameter of parameters) {
      ...
      if (opt_instance) {
        this.bindToInstance(`${serviceName}:${parameter.id}`, opt_instance, parameter.id);
      }
    }
  }
    ...
    commands.registerCommands('params', [
      ...
      },
      {
        ...
        handler: async (args, log) => {
          ...
          if (prefix) {
            parameters = parameters.filter((p) => p.spec_.id.startsWith(prefix));
          }
          log.log(`available parameters${prefix ? ` (starting with: ${prefix})` : ''}:`);
          for (const parameter of parameters) {
