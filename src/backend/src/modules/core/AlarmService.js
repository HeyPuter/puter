...
class AlarmService extends BaseService {
    ...
    if (this.global_config.env === 'dev') {
      ...
      this.alarm_widget = () => {
        ...
        for (const alarm of Object.values(this.alarms)) {
          const line =
            `\x1B[31;1m [alarm]\x1B[0m ` + `${alarm.id_string}: ${alarm.message} (${alarm.count})`;
  ...
  handle_alarm_repeat_(alarm) {
    this.log.warn(`REPEAT ${alarm.id_string} :: ${alarm.message} (${alarm.count})`, alarm.fields);
  ...
  handle_alarm_on_(alarm) {
    this.log.error(`ACTIVE ${alarm.id_string} :: ${alarm.message} (${alarm.count})`, alarm.fields);
  ...
  handle_alarm_off_(alarm) {
    this.log.info(`CLEAR ${alarm.id} :: ${alarm.message} (${alarm.count})`, alarm.fields);
  }
    ...
    commands.registerCommands('alarm', [
      ...
      },
      {
        ...
        handler: async (args, log) => {
          ...
          if (!alarm) {
            log.log(
              `no alarm with id ${id}; ` + `but calling clear(${JSON.stringify(id)}) anyway.`
            );
          }
          this.clear(id);
        },
