...
class LogContext {
  ...
  cache(isCacheHit, identifier, fields = {}) {
    this.log(LOG_LEVEL_DEBU, isCacheHit ? 'cache_hit' : 'cache_miss', { identifier, ...fields });
  }
  ...
  log(log_level, message, fields = {}, objects = {}) {
    ...
    for (const k in fields) {
      if (whatis(fields[k]) === 'object' && typeof fields[k].toLogFields === 'function')
    ...
    this.logService.log_(log_level, this.crumbs, message, fields, objects);
  }
...
class CustomLogger {
  ...
  async onLogMessage(log_lvl, crumbs, message, fields, ...a) {
    ...
    const { log_lvl: _log_lvl, crumbs: _crumbs, message: _message, fields: _fields, args } = ret;
  ...
  ['__on_boot.consolidation']() {
    ...
    commands.registerCommands('logs', [
      ...
      },
      {
        ...
        handler: async (args, log) => {
          globalThis.dev_console_indent_on = !globalThis.dev_console_indent_on;
        },
      },
    ]);
  }
  ...
  async _init() {
    ...
    if (config.logger) {
      ...
      const output_lvl = Object.values({
        ...
        return (
          lvl.label === config.logger.level.toUpperCase() ||
