...
class AIChatService extends BaseService {
  ...
  async ['__on_boot.consolidation']() {
    ...
    for (const provider of this.providers) {
      const delegate = this.services.get(provider.service_name).as('puter-chat-completion');
  ...
  static IMPLEMENTS = {
    ['driver-capabilities']: {
      supports_test_mode(iface, method_name) {
        return iface === 'puter-chat-completion' && method_name === 'complete';
      },
    },
    /**
    ...
    ['puter-chat-completion']: {
      ...
      async complete(parameters) {
        ...
        if (parameters.messages) {
          parameters.messages = Messages.normalize_messages(parameters.messages);
        }
        ...
        const max_allowed_output_amount = available - approximate_input_cost;

        const max_allowed_output_tokens = max_allowed_output_amount / model_output_cost;
        ...
        try {
          ...
          while (!!error) {
            ...
            const { fallback_service_name, fallback_model_name } = fallback;
        ...
        if (
          ...
          TypeSpec.adapt({ $: 'ai-chat-intermediate' }).equals(ret.result.type)
        ) {
          (async () => {
        ...
        if (parameters.response?.normalize) {
          ret.result.message = Messages.normalize_single_message(ret.result.message);
          ret.result = {
  ...
  async check_usage_({ actor, service, model }) {
    ...
    const [row] = await this.db.read(
      'SELECT SUM(`cost`) AS sum FROM `ai_usage` ' + 'WHERE `user_id` = ? AND `created_at` >= ?',
    ...
    for (const msg of messages) {
      ...
      if (typeof msg.content === 'string') texts.push(msg.content);
        ...
        if (Array.isArray(msg.content)) {
          texts.push(
            ...msg.content
              .filter((o) => (!o.type && o.hasOwnProperty('text')) || o.type === 'text')
              .map((o) => o.text)
          );
