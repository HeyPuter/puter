...
class OpenAICompletionService extends BaseService {
  ...
  async _init() {
    ...
    let apiKey =
      this.config?.services?.openai?.apiKey ?? this.global_config?.services?.openai?.apiKey;

    // Fallback to the old format for backward compatibility
    if (!apiKey) {
      apiKey = this.config?.openai?.secret_key ?? this.global_config?.openai?.secret_key;
   ...
   */
  async complete(messages, { stream, moderation, model, tools, temperature, max_tokens }) {
    // Validate messages
        ...
        promises.push(
          (async () => {
            if ((await task.node.get('size')) > MAX_FILE_SIZE) {
              ...
              task.contentPart.text =
                `{error: input file exceeded maximum of ${MAX_FILE_SIZE} bytes; ` +
            ...
            if (mimeType.startsWith('image/')) {
              ...
              task.contentPart.type = 'text';
              task.contentPart.text =
                `{error: input file has unsupported MIME type; ` +
                `the user did not write this message}`; // "poor man's system prompt"
            }
          })()
        );
