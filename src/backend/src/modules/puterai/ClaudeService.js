  ...
  static IMPLEMENTS = {
    ['puter-chat-completion']: {
      ...
      async complete({ messages, stream, model, tools, max_tokens, temperature }) {
        ...
        console.log(
          '\x1B[26;1m ===== SDK PARAMETERS',
          require('util').inspect(sdk_params, undefined, Infinity)
        );
        ...
        if (stream) {
          ...
          const init_chat_stream = async ({ chatStream }) => {
            ...
            for await (const event of completion) {
              const input_tokens = (event?.usage ?? event?.message?.usage)?.input_tokens;
              const output_tokens = (event?.usage ?? event?.message?.usage)?.output_tokens;
