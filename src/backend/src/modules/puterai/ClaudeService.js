    ...
    ["puter-chat-completion"]: {
      ...
      async complete({
        ...
        [system_prompts, messages] =
          Messages.extract_and_remove_system_messages(messages);
        ...
        if (stream) {
          ...
          const init_chat_stream = async ({ chatStream }) => {
            ...
            for await (const event of completion) {
              ...
              const input_tokens = (event?.usage ?? event?.message?.usage)
                ?.input_tokens;
              const output_tokens = (event?.usage ?? event?.message?.usage)
                ?.output_tokens;
