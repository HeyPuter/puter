...
class AWSPollyService extends BaseService {
  ...
  static IMPLEMENTS = {
    ...
    ['puter-tts']: {
      ...
      async synthesize({ text, voice, ssml, language, engine = 'standard', test_mode }) {
        if (test_mode) {
  ...
  _get_client(region) {
    if (!region) {
      region = this.config.aws?.region ?? this.global_config.aws?.region ?? 'us-west-2';
    }
  ...
  async get_default_voice_for_engine_(engine = 'standard') {
    ...
    for (const voice_name of preferred_voices) {
      const voice = voices.Voices.find(
        (v) => v.Id === voice_name && v.SupportedEngines && v.SupportedEngines.includes(engine)
      );
      if (voice) {
        return voice.Id;
      }
    ...
    const fallback_voice = voices.Voices.find(
      (voice) => voice.SupportedEngines && voice.SupportedEngines.includes(engine)
    );
