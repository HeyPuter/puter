...
class DefaultUserService extends BaseService {
  ...
  async ['__on_ready.webserver']() {
    ...
    const is_default_password = await bcrypt.compare(tmp_password, user.password);
  ...
  async create_default_user_() {
    ...
    await db.write(`UPDATE user SET password = ? WHERE id = ?`, [password_hashed, user.id]);
  ...
  async force_tmp_password_(user) {
    const db = this.services.get('database').get(DB_WRITE, 'terminal-password-reset');
    const actor = await Actor.create(UserActorType, { user });
    return await Context.get()
      .sub({ actor })
      .arun(async () => {
