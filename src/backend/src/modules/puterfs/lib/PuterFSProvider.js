...
class PuterFSProvider extends putility.AdvancedBase {
  ...
  async stat({ selector, options, controls, node }) {
    ...
    const { traceService, fsEntryService, fsEntryFetcher, resourceService } =
  ...
  async copy_tree({ context, source, parent, target_name }) {
    return await this.copy_tree_({ context, source, parent, target_name });
  }
  async copy_tree_({ context, source, parent, target_name }) {
    ...
    const raw_fsentry = {
      ...
      // an error
      ...(source.entry.thumbnail ? { thumbnail: source.entry.thumbnail } : {}),
  ...
  async rmnode_({ context, node, options }) {
    ...
    svc_size.change_usage(await node.get('user_id'), -1 * (await node.get('size')));
