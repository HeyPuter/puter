...
class DatabaseFSEntryInsert extends AbstractDatabaseFSEntryOperation {
  static requiredForCreate = ['uuid', 'parent_uid'];
...
class DatabaseFSEntryService extends BaseService {
  ...
  async fast_get_direct_descendants(uuid) {
    return (
      uuid === PuterPath.NULL_UUID
        ? await this.db.read(`SELECT uuid FROM fsentries WHERE parent_uid IS NULL`)
        : await this.db.read(`SELECT uuid FROM fsentries WHERE parent_uid = ?`, [uuid])
    ).map((x) => x.uuid);
  }
  ...
  async get(uuid, fetch_entry_options) {
    ...
    if (answer.is_diff) {
      ...
      const base_entry = await fsEntryFetcher.find(new NodeUIDSelector(uuid), fetch_entry_options);
  ...
  async get_descendants(uuid) {
    return uuid === PuterPath.NULL_UUID
      ? await this.db.read(`SELECT uuid FROM fsentries WHERE parent_uid IS NULL`, [uuid])
      : await this.db.read(`SELECT uuid FROM fsentries WHERE parent_uid = ?`, [uuid]);
  }
  ...
  async enqueue_(op) {
    ...
    const state =
      this.status === this.constructor.STATUS_READY ? this.currentState : this.deferredState;
  ...
  async exec_() {
    ...
    this.log.info(`Executing ${queue.length} operations...`);
    ...
    this.log.info(`Finished ${queue.length} operations.`);
