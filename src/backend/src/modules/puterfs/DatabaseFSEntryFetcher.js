...
module.exports = class DatabaseFSEntryFetcher extends BaseService {
  ...
  async find(selector, fetch_entry_options) {
    ...
    if (selector instanceof NodeChildSelector) {
      ...
      if (typeof id !== 'number') {
        throw new Error('unexpected type for id value', typeof id, id);
      }
  ...
  async findByPath(path, fetch_entry_options = {}) {
    ...
    const resultColsSql = this.defaultProperties.join(', ') + (thumbnail ? `, thumbnail` : '');

    result = await this.db.read(
      `SELECT ` + resultColsSql + ` FROM fsentries WHERE path=? LIMIT 1`,
      [path]
    );
    ...
    const loop = async () => {
      for (let i = 0; i < parts.length; i++) {
        ...
        if (parent_uid === null) {
          result = await this.db.read(
            `SELECT ` + colsSql + ` FROM fsentries WHERE parent_uid IS NULL AND name=? LIMIT 1`,
            [part]
          );
        } else {
          result = await this.db.read(
            `SELECT ` + colsSql + ` FROM fsentries WHERE parent_uid=? AND name=? LIMIT 1`,
            [parent_uid, part]
          );
        }
