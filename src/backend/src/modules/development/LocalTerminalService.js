...
class LocalTerminalService extends BaseService {
  ...
  get_profiles() {
    return {
      ['api-test']: {
        cwd: path_.join(__dirname, '../../../../../', 'tools/api-tester'),
        shell: ['/usr/bin/env', 'node', 'apitest.js', '--config=config.yml'],
        allow_args: true,
      },
    };
  }
  ['__on_install.routes'](_, { app }) {
    ...
    Endpoint({
      ...
      handler: async (req, res) => {
        ...
        const can_access = actor && (await svc_permission.check(actor, PERM_LOCAL_TERMINAL));
        ...
        // stdout to websocket
        {
          ...
          proc.stdout.on('data', (data) => {
            ...
            svc_socketio.send({ room: req.user.id }, 'local-terminal.stdout', {
              term_uuid,
              base64,
            });
          });
          proc.stderr.on('data', (data) => {
            const base64 = data.toString('base64');
            console.log('---------------------- CHUNK?', base64);
            svc_socketio.send({ room: req.user.id }, 'local-terminal.stderr', {
              term_uuid,
              base64,
            });
          });
        ...
        proc.on('exit', () => {
          ...
          const svc_socketio = req.services.get('socketio');
          svc_socketio.send({ room: req.user.id }, 'local-terminal.exit', {
            term_uuid,
          });
        });
  ...
  async _init() {
    ...
    svc_event.on('web.socket.user-connected', async (_, { socket, user }) => {
      const svc_permission = this.services.get('permission');
