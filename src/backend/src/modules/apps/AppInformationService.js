...
class AppInformationService extends BaseService {
  ...
  async get_stats(app_uid, options = {}) {
    ...
    if (stats_grouping) {
      ...
      if (!timeFormat) {
        throw new Error(
          `Invalid stats_grouping: ${stats_grouping}. Supported values are: hour, day, week, month, year`
        );
      }
      ...
      if (global.clickhouseClient) {
        ...
        const completeOpenStats = this.mergeWithGeneratedPeriods(
          processedOpenRows,
          allPeriods,
          stats_grouping
        );
        const completeUserStats = this.mergeWithGeneratedPeriods(
          processedUserRows,
          allPeriods,
          stats_grouping
        );
        ...
        return {
          ...
          referral_count:
            period === 'all' ? await kv.get(`apps:referral_count:uid:${app_uid}`) : null,
        };
      } else {
  ...
  async _refresh_app_stats() {
    ...
    for (const app of apps) {
      ...
      const { open_count } = (
        await db.read(`SELECT COUNT(_id) AS open_count FROM app_opens WHERE app_uid = ?`, [app.uid])
      )[0];
  ...
  async _refresh_app_stat_referrals() {
    ...
    for (const app of apps) {
      ...
      const { referral_count } = (
        await db.read(`SELECT COUNT(id) AS referral_count FROM user WHERE referrer LIKE ?`, [
          origin + '%',
        ])
      )[0];
  ...
  async delete_app(app_uid, app) {
    ...
    if (!app) {
      app = (await db.read(`SELECT * FROM apps WHERE uid = ?`, [app_uid]))[0];
    }
    ...
    await db.write(`DELETE FROM apps WHERE uid = ? LIMIT 1`, [app_uid]);
