...
class OldAppNameService extends BaseService {
  ...
  async check_app_name(name) {
    const rows = await this.db.read('SELECT * FROM `old_app_names` WHERE `name` = ?', [name]);
    ...
    if (age > n_ms) {
      // Remove record
      await this.db.write('DELETE FROM `old_app_names` WHERE `id` = ?', [row.id]);
      // Return undefined
      return;
    }
  ...
  async remove_name(id) {
    await this.db.write('DELETE FROM `old_app_names` WHERE `id` = ?', [id]);
  }
}
