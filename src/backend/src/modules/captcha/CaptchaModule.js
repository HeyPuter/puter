...
class CaptchaModule extends AdvancedBase {
  async install(context) {
    ...
    try {
      ...
      console.log(
        `Captcha service registered and ${captchaService.enabled ? 'enabled' : 'disabled'}`
      );
      console.log(
        'TOKENS_TRACKING: Retrieved CaptchaService instance with ID:',
        captchaService.serviceId
      );
    } catch (error) {
