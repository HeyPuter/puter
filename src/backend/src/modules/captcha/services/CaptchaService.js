...
class CaptchaService extends BaseService {
    ...
    try {
      ...
      app.get('/api/captcha/test-lifecycle', (req, res) => {
        try {
          ...
          console.log(
            'TOKENS_TRACKING: Test token stored. New token count:',
            this.captchaTokens.size
          );
          ...
          console.log(
            'TOKENS_TRACKING: Verification with correct answer result:',
            correctVerification
          );
          ...
          console.log(
            'TOKENS_TRACKING: Second test token stored. Token count:',
            this.captchaTokens.size
          );
  ...
  generateCaptcha() {
    ...
    console.log(
      'TOKENS_TRACKING: Static test token exists:',
      this.captchaTokens.has('test-static-token')
    );
    ...
    console.log(
      'TOKENS_TRACKING: Token map size before storing new token:',
      this.captchaTokens.size
    );
    ...
    console.log(
      'TOKENS_TRACKING: Token map size after storing new token:',
      this.captchaTokens.size
    );
  ...
  verifyCaptcha(token, userAnswer) {
    ...
    console.log(
      'TOKENS_TRACKING: Trying to verify token:',
      token ? token.substring(0, 8) + '...' : 'undefined'
    );

    console.log(
      'verifyCaptcha called with token:',
      token ? token.substring(0, 8) + '...' : 'undefined'
    );
    ...
    console.log(
      'TOKENS_TRACKING: Token map size after removing used token:',
      this.captchaTokens.size
    );
  ...
  _checkServiceHealth() {
    console.log(
      'TOKENS_TRACKING: Service health check. ID:',
      this.serviceId,
      'Token count:',
      this.captchaTokens.size
    );
  ...
  cleanupExpiredTokens() {
    ...
    for (const [token, data] of this.captchaTokens.entries()) {
      ...
      console.log(
        `TOKENS_TRACKING: Token ${token.substring(0, 8)}... expires: ${new Date(data.expiresAt).toISOString()}, expired: ${isExpired}`
      );
    ...
    if (expiredCount > 0) {
      console.log(
        `TOKENS_TRACKING: Found ${expiredCount} expired tokens to remove and ${validCount} valid tokens to keep`
      );
