...
class MkTree extends HLFilesystemOperation {
  ...
  async create_branch_({ parent_node, tree, parent_exists }) {
    ...
    const dirs = trunk === '.' ? [] : trunk.split('/').filter(Boolean);
...
class QuickMkdir extends HLFilesystemOperation {
  async _run() {
    ...
    const dirs = path === '.' ? [] : path.split('/').filter(Boolean);
...
class HLMkdir extends HLFilesystemOperation {
  ...
  static FEATURES = [new OtelFeature(['_get_existing_parent', '_create_parents'])];
  ...
  async _run() {
    ...
    const all_nodes = [...this.parent_directories_created, this.created];
  ...
  async _get_existing_parent({ parent_node }) {
    ...
    const dirs = target_dirname === '.' ? [] : target_dirname.split('/').filter(Boolean);
  ...
  async _create_top_parent({ top_parent }) {
    ...
    const maybe_path_selector = top_parent.get_selector_of_type(NodePathSelector);
