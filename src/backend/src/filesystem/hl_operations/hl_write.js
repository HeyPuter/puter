...
class WriteCommonFeature {
  install_in_instance(instance) {
    instance._verify_size = async function () {
      if (this.values.file && this.values.file.size > config.max_file_size) {
        throw APIError.create('file_too_large', null, {
          max_size: config.max_file_size,
        });
      }

      if (this.values.thumbnail && this.values.thumbnail.size > config.max_thumbnail_size) {
...
class HLWrite extends HLFilesystemOperation {
  ...
  static FEATURES = [new WriteCommonFeature()];
  ...
  async _run() {
    ...
    if (!(await parent.exists()) || (await parent.get('type')) !== TYPE_DIRECTORY) {
      destination = parent;
    ...
    {
      const node_to_check = dest_exists && overwrite && !dedupe_name ? destination : parent;
    ...
    if (dest_exists) {
      ...
      if (dedupe_name) {
        ...
        for (let i = 1; ; i++) {
          ...
          const exists = await fsEntryFetcher.nameExistsUnderParent(parent.uid, try_new_name);
          if (!exists) {
      ...
      (async () => {
        const reason = await (async () => {
          ...
          const thumb_file = (() => {
            ...
            const [replace_stream, thumbnail_stream] = pausing_tee(values.file.stream, 2);
