...
class HLMove extends HLFilesystemOperation {
  ...
  async _run() {
    ...
    const new_metadata =
      typeof values.new_metadata === 'string'
        ? values.new_metadata
        : JSON.stringify(values.new_metadata);
    ...
    if (src_user_id !== par_user_id) {
      ...
      if (source_user.id !== par_user_id) dest_user = await get_user({ id: par_user_id });
      else dest_user = source_user;
    ...
    if (await dest.exists()) {
      ...
      if (values.dedupe_name) {
        ...
        for (let i = 1; ; i++) {
          ...
          const exists = await svc_fsEntryFetcher.nameExistsUnderParent(parent.uid, try_new_name);
          if (!exists) {
