...
class HLNameSearch extends HLFilesystemOperation {
  async _run() {
    ...
    const db = services.get('database').get(DB_READ, 'fs.namesearch');
    ...
    const results = await db.read(
      `SELECT uuid FROM fsentries WHERE name LIKE ? AND ` + `user_id = ? LIMIT 50`,
      [term, actor.type.user.id]
    );

    const uuids = results.map((v) => v.uuid);

    const fsnodes = await Promise.all(
