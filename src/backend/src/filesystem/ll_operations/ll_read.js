...
class LLRead extends LLFilesystemOperation {
    ...
    static METHODS = {
        async _run({ fsNode, no_acl, context, actor, offset, length, range, version_id }){
            ...
            if ( maybe_buffer ) {
                ...
                const meteringService = context.get('services').get('meteringService');

                // return cached stream
                if ( has_range && (length || offset) ) {
                    meteringService.incrementUsage(ownerActor, "filesystem:ingress:bytes", length);
                    return buffer_to_stream(maybe_buffer.slice(offset, offset + length));
                }
                meteringService.incrementUsage(ownerActor, "filesystem:ingress:bytes", await fsNode.get('size'));
            ...
            meteringService.incrementUsage(ownerActor, "filesystem:ingress:bytes", size);
