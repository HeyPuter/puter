      ...
      [
        ...
        async function calculate_has_range(a) {
          ...
          const has_range =
            ...
            (length !== undefined && length != (await fsNode.get('size')));
          a.set('has_range', has_range);
        },
        async function update_accessed(a) {
          const context = a.iget('context');
          const db = context.get('services').get('database').get(DB_WRITE, 'filesystem');
        ...
        async function check_for_cached_copy(a) {
          ...
          if (maybe_buffer) {
            ...
            if (has_range) {
              return a.stop(buffer_to_stream(maybe_buffer.slice(offset, offset + length)));
            }
            return a.stop(buffer_to_stream(maybe_buffer));
          }
