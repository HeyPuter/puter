...
class LLWriteBase extends LLFilesystemOperation {
  ...
  async _storage_upload({ uuid, bucket, bucket_region, file, tmp }) {
...
class LLOWrite extends LLWriteBase {
  async _run() {
    const { node, actor, immutable, file, tmp, fsentry_tmp, message } = this.values;
    ...
    (async () => {
      ...
      if (store_version_id) {
        ...
        db.write(
          ...
          [actor.type.user.id, new_item.id, new_item.uuid, store_version_id, message ?? null, ts]
        );
      }
    })();
