...
const {
  ...
  PermissionExploder,
} = require('../services/auth/PermissionService.js');
...
class FilesystemService extends BaseService {
  ...
  old_constructor(args) {
    ...
    info
      .given('fs.fsentry')
      .provide('fs.fsentry:path')
      .addStrategy('entry-or-delegate', async (entry) => {
        if (entry.path) return entry.path;
        return await info.with('fs.fsentry:uuid').obtain('fs.fsentry:path').exec(entry.uuid);
      });
  }
  ...
  async _init() {
    ...
    svc_permission.register_rewriter(
      PermissionRewriter.create({
        matcher: (permission) => {
    ...
    svc_permission.register_implicator(
      PermissionImplicator.create({
        id: 'is-owner',
        matcher: (permission) => {
    ...
    svc_permission.register_exploder(
      PermissionExploder.create({
        id: 'fs-access-levels',
        matcher: (permission) => {
          return permission.startsWith('fs:') && PermissionUtil.split(permission).length >= 3;
        },
        exploder: async ({ permission }) => {
          ...
          if (rules.hasOwnProperty(specified_mode)) {
            permissions.push(
              ...rules[specified_mode].map((mode) =>
                PermissionUtil.join(parts[0], parts[1], mode, ...parts.slice(3))
              )
            );
          }
  ...
  async node(selector) {
    ...
    if (typeof selector === 'object' && selector.constructor.name === 'Object') {
      if (selector.path) {
