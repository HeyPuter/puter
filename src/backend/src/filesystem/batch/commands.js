...
class BatchCommand extends AdvancedBase {
  static FEATURES = [new AsyncProviderFeature()];
...
class MkdirCommand extends BatchCommand {
  async run(executor, parameters) {
    ...
    const meta = parameters.parent ? executor.pathResolver.getMeta(parameters.parent) : undefined;
    ...
    if (meta?.conflict_free) {
      ...
      if (parameters.as) {
        executor.pathResolver.putSelector(parameters.as, q_mkdir.created.selector, {
          conflict_free: true,
        });
      }
    ...
    const response = await hl_mkdir.run({
      ...
      create_missing_parents:
        parameters.create_missing_ancestors ?? parameters.create_missing_parents ?? false,
    ...
    if (parameters.as) {
      executor.pathResolver.putSelector(
        ...
        hl_mkdir.used_existing ? undefined : { conflict_free: true }
      );
    }
...
class WriteCommand extends BatchCommand {
  async run(executor, parameters) {
    ...
    const destinationOrParent = await fs.node(
      await executor.pathResolver.awaitSelector(parameters.path)
    );
...
class ShortcutCommand extends BatchCommand {
  async run(executor, parameters) {
    ...
    const shortcut_to = await fs.node(
      await executor.pathResolver.awaitSelector(parameters.shortcut_to)
    );
...
class DeleteCommand extends BatchCommand {
  async run(executor, parameters) {
    ...
    const target = await fs.node(await executor.pathResolver.awaitSelector(parameters.path));
