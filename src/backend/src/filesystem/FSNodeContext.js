...
module.exports = class FSNodeContext {
   ...
   */
  constructor({ services, selector, provider, fs }) {
  ...
  async fetchShares(force) {
    ...
    const permissions = await svc_permission.query_issuer_permissions_by_prefix(
      actor.type.user,
      `fs:${await this.get('uid')}:`
    );

    this.entry.shares = { users: [], apps: [] };

    for (const user_perm of permissions.users) {
      const access = PermissionUtil.split(user_perm.permission).slice(-1)[0];
    ...
    for (const app_perm of permissions.apps) {
      const access = PermissionUtil.split(app_perm.permission).slice(-1)[0];
  ...
  async fetchSuggestedApps(user, force) {
    ...
    this.entry.suggested_apps = await suggest_app_for_fsentry(this.entry, { user });
  }
  ...
  async get(key) {
    ...
    if (this.found === false) {
      throw new Error(
        `Tried to get ${key} of non-existent fsentry: ` + this.selector.describe(true)
      );
    }
    ...
    if (key === 'entry') {
      ...
      if (this.found === false) {
        throw new Error(
          `Tried to get entry of non-existent fsentry: ` + this.selector.describe(true)
        );
      }
      return this.entry;
    }
    ...
    if (key === 'path') {
      ...
      if (this.found === false) {
        throw new Error(
          `Tried to get path of non-existent fsentry: ` + this.selector.describe(true)
        );
      }
      if (!this.path) {
  ...
  async getSafeEntry(fetch_options = {}) {
    ...
    fsentry.shortcut_to = res.shortcut_to ? await id2uuid(res.shortcut_to) : undefined;
    try {
      fsentry.shortcut_to_path = res.shortcut_to ? await id2path(res.shortcut_to) : undefined;
    } catch (e) {
    ...
    if (res.file_request_token && res.file_request_token !== '') {
      fsentry.file_request_url = config.origin + '/upload?token=' + res.file_request_token;
    }
