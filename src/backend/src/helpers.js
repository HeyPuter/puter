...
} = require("./services/database/BaseDatabaseAccessService.js");
...
const chkperm = spanify(
  "chkperm",
  async (target_fsentry, requester_user_id, action) => {
    // basic cases where false is the default response
    if (!target_fsentry) return false;
    ...
    // special case: owner of entry has shared at least one entry with requester and requester is asking for the owner's root directory: /[owner_username]
      ...
      target_fsentry.parent_uid === null &&
      (await has_shared_with(target_fsentry.user_id, requester_user_id)) &&
      action !== "write"
    )
      return true;
    else return false;
...
function validate_fsentry_name(name) {
  if (!name) throw { message: "Name can not be empty." };
  else if (!isString(name)) throw { message: "Name can only be a string." };
    ...
    throw {
      message: `Name can not be longer than ${config.max_fsentry_name_length} characters`,
    };
  else return true;
}
...
async function id2uuid(id) {
  ...
  let fsentry = await db.requireRead(
    "SELECT `uuid`, immutable FROM `fsentries` WHERE `id` = ? LIMIT 1",
    [id]
  );

  if (!fsentry[0]) return null;
  else return fsentry[0].uuid;
}
...
async function df(user_id) {
  ...
  if (!fsentry[0] || !fsentry[0].total) return 0;
  else return fsentry[0].total;
}

/**
 * Get user by any of a variety of identifying properties
 *
 * @param {User} userObj - A user object containing fields from which to retrieve the user
 * @returns {Promise<User>}
 */
async function get_user(options) {
  // Ensure services are available
  if (!services || !services.get) {
    console.error("Services not available for get_user function");
    return null;
  }

  try {
    const getUserService = services.get("get-user");
    if (!getUserService) {
      console.error("get-user service not available");
      return null;
    }
    return await getUserService.get_user(options);
  } catch (error) {
...
async function refresh_apps_cache(options, override) {
  ...
  // refresh only apps that are approved for listing
    ...
    let apps = await db.read(
      "SELECT * FROM apps WHERE approved_for_listing = 1"
    );
    for (let index = 0; index < apps.length; index++) {
      const app = apps[index];
      kv.set("apps:name:" + app.name, app);
...
async function get_app(options) {
  ...
  if (options.uid) {
    ...
    if (!app[0]) {
      ...
      app = await db.read("SELECT * FROM `apps` WHERE `uid` = ? LIMIT 1", [
        options.uid,
      ]);
    }
    ...
    if (!app[0]) {
      ...
      app = await db.read("SELECT * FROM `apps` WHERE `name` = ? LIMIT 1", [
        options.name,
      ]);
    }
  } else if (options.id) {
    ...
    if (!app[0]) {
      ...
      app = await db.read("SELECT * FROM `apps` WHERE `id` = ? LIMIT 1", [
        options.id,
      ]);
    }
  }
...
async function app_exists(options) {
  ...
  let app;
  if (options.uid)
    app = await db.read("SELECT `id` FROM `apps` WHERE `uid` = ? LIMIT 1", [
      options.uid,
    ]);
  else if (options.name)
    app = await db.read("SELECT `id` FROM `apps` WHERE `name` = ? LIMIT 1", [
      options.name,
    ]);
  else if (options.id)
    app = await db.read("SELECT `id` FROM `apps` WHERE `id` = ? LIMIT 1", [
      options.id,
    ]);
...
async function change_username(user_id, new_username) {
  ...
  // update username
  await db.write("UPDATE `user` SET username = ? WHERE `id` = ? LIMIT 1", [
    new_username,
    user_id,
  ]);
  ...
  await services
    .get("filesystem")
    .update_child_paths(`/${old_username}`, `/${new_username}`, user_id);
...
async function uuid2fsentry(uuid, return_thumbnail) {
  ...
  if (!fsentry[0]) return false;
  else return fsentry[0];
}
...
async function convert_path_to_fsentry(path) {
  ...
  if (fsentry_names.length === 0) return false;
  ...
  for (let i = 0; i < fsentry_names.length; i++) {
    ...
    if (result[0]) {
      ...
      if (result[0].is_public === null) result[0].is_public = is_public;
      else is_public = result[0].is_public;
    } else {
      return false;
    }
    final_res = result;
...
const get_descendants_0 = async (
  ...
  if (split_path.length === 2 && split_path[0] === "") username = split_path[1];
  ...
  // -------------------------------------
    ...
    if (temp_children.length > 0) {
      for (let i = 0; i < temp_children.length; i++) {
        ...
        if (await chkperm(tchild, user.id)) children.push(tchild);
      }
    }
  }
  ...
  for (let i = 0; i < children.length; i++) {
    ...
    ret.push({
      ...
      shortcut_to: children[i].shortcut_to
        ? await id2uuid(children[i].shortcut_to)
        : undefined,
      shortcut_to_path: children[i].shortcut_to
    ...
    if (
      ...
      ret.push(
        await get_descendants(path + "/" + children[i].name, user, depth)
      );
    }
  }
  return ret.flat();
      ...
      result = await db.read(
        `
                WITH RECURSIVE cte AS (
                    ...
                    SELECT e.uuid, e.parent_uid, e.name, ${db.case({
...
async function is_ancestor_of(ancestor_uid, descendant_uid) {
  ...
  if (ancestor_uid === null) return true;
  // root is never a descendant to any FSEntries
  if (descendant_uid === null) return false;
  ...
  let parent = await db.read(
    "SELECT `uuid`, `parent_uid` FROM `fsentries` WHERE `uuid` = ? LIMIT 1",
    [descendant_uid]
  );
  if (parent[0] === undefined)
    parent = await db.pread(
      "SELECT `uuid`, `parent_uid` FROM `fsentries` WHERE `uuid` = ? LIMIT 1",
      [descendant_uid]
    );
  if (
    parent[0].uuid === ancestor_uid ||
    parent[0].parent_uid === ancestor_uid
  ) {
    return true;
  }
  // keep checking as long as parent of parent is not root
  while (parent[0].parent_uid !== null) {
    parent = await db.read(
      "SELECT `uuid`, `parent_uid` FROM `fsentries` WHERE `uuid` = ? LIMIT 1",
      [parent[0].parent_uid]
    );
    if (parent[0] === undefined) {
...
async function jwt_auth(req) {
  ...
  // Auth token in URL
  else if (req.query && req.query.auth_token) token = req.query.auth_token;
  // Socket
  else if (req.handshake && req.handshake.auth && req.handshake.auth.auth_token)
    token = req.handshake.auth.auth_token;

  if (!token || token === "null") throw "No auth token found";
  else if (typeof token !== "string") throw "token must be a string.";
  ...
  try {
    ...
    if (!actor.type?.constructor?.name === "UserActorType") {
      throw {
        message: APIError.create("token_unsupported").serialize(),
      };
    }
...
async function ancestors(fsentry_id) {
  ...
  while (parent[0].parent_uid !== null) {
    ...
    parent = await db.read("SELECT * FROM `fsentries` WHERE `id` = ? LIMIT 1", [
      parent_fsentry.id,
    ]);
...
async function username_exists(username) {
  ...
  if (rows[0].username_exists) return true;
}
...
async function app_name_exists(name) {
  ...
  let rows = await db.read(
    `SELECT EXISTS(SELECT 1 FROM apps WHERE apps.name=?) AS app_name_exists`,
    [name]
  );
  if (rows[0].app_name_exists) return true;
...
async function suggest_app_for_fsentry(fsentry, options) {
  ...
  // Draw
  //---------------------------------------------
  if (fsname.endsWith(".bmp") || content_type.startsWith("image/")) {
  ...
  for (const app_id of apps) {
    ...
    if (
      third_party_app.approved_for_opening_items ||
      (options !== undefined &&
        options.user !== undefined &&
        options.user.id === third_party_app.owner_user_id)
    )
...
function validate_signature_auth(url, action, options = {}) {
  ...
  if (!query.get("uid"))
    ...
    throw {
      message: "`action` is required for signature-based authentication.",
    };
  else if (!query.get("expires"))
    throw {
      message: "`expires` is required for signature-based authentication.",
    };
  else if (!query.get("signature"))
    throw {
      message: "`signature` is required for signature-based authentication.",
    };
  ...
  const expired =
    query.get("expires") && query.get("expires") < Date.now() / 1000;

  // expired?
  if (expired) throw { message: "Authentication failed. Signature expired." };
  ...
  if (
    !expired &&
    query.get("signature") ===
      sha256(`${uid}/write/${secret}/${query.get("expires")}`)
  )
    return true;
  // if not, check specific actions
    ...
    !expired &&
    query.get("signature") ===
      sha256(`${uid}/${action}/${secret}/${query.get("expires")}`)
  )
    return true;
  // auth failed
  else throw { message: "Authentication failed" };
}
