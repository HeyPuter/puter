...
function validate_fsentry_name(name) {
  ...
  else if (name.includes('/')) throw { message: "Name can not contain the '/' character." };
  else if (name === '.') throw { message: "Name can not be the '.' character." };
  else if (name === '..') throw { message: "Name can not be the '..' character." };
...
async function refresh_apps_cache(options, override) {
  ...
  // if options is provided, refresh only the app specified
    ...
    if (options.name) app = await db.pread('SELECT * FROM apps WHERE name = ?', [options.name]);
    else if (options.uid) app = await db.pread('SELECT * FROM apps WHERE uid = ?', [options.uid]);
    else if (options.id) app = await db.pread('SELECT * FROM apps WHERE id = ?', [options.id]);
    else {
...
async function convert_path_to_fsentry(path) {
  ...
  result = await db.read(`SELECT * FROM fsentries WHERE path=? LIMIT 1`, ['/' + path]);
...
const get_descendants_0 = async (path, user, depth, return_thumbnail = false) => {
  ...
  for (let i = 0; i < children.length; i++) {
    ...
    if (children[i].is_dir && (depth === undefined || (depth !== undefined && depth > 0))) {
...
async function deleteUser(user_id) {
  ...
  for (let offset = 0; true; offset += 5000) {
    let files = await db.read(
      `SELECT uuid, bucket, bucket_region FROM fsentries WHERE user_id = ? AND is_dir = 0 LIMIT 5000 OFFSET ` +
        offset,
      [user_id]
    );
...
async function jwt_auth(req) {
  let token;
  // HTTML Auth header
  if (req.header && req.header('Authorization')) token = req.header('Authorization');
  // Cookie
  else if (req.cookies && req.cookies[config.cookie_name]) token = req.cookies[config.cookie_name];
  // Auth token in URL
...
async function get_taskbar_items(user, { icon_size, no_icons } = {}) {
  ...
  for (let index = 0; index < taskbar_items_from_db.length; index++) {
    ...
    let item = {};
    if (taskbar_item_from_db.name) item = await get_app({ name: taskbar_item_from_db.name });
    else if (taskbar_item_from_db.id) item = await get_app({ id: taskbar_item_from_db.id });
    else if (taskbar_item_from_db.uid) item = await get_app({ uid: taskbar_item_from_db.uid });
...
function validate_signature_auth(url, action, options = {}) {
  ...
  if (!query.get('uid')) throw { message: '`uid` is required for signature-based authentication.' };
  else if (!action) throw { message: '`action` is required for signature-based authentication.' };
  ...
  if (
    !expired &&
    query.get('signature') === sha256(`${uid}/write/${secret}/${query.get('expires')}`)
  )
    return true;
  // if not, check specific actions
  else if (
    !expired &&
    query.get('signature') === sha256(`${uid}/${action}/${secret}/${query.get('expires')}`)
  )
    return true;
  // auth failed
