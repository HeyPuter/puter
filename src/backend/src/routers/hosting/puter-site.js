...
class PuterSiteMiddleware extends AdvancedBase {
  ...
  async run(req, res, next) {
    if (!req.hostname.endsWith(config.static_hosting_domain) && req.subdomains[0] !== 'devtest')
      return next();
  ...
  async run_(req, res, next) {
    ...
    const resolved_url_path = this.modules.path.resolve('/', path);
    ...
    if (site.protected) {
      ...
      const get_site_actor_from_token = async () => {
        ...
        let site_actor;
        try {
          site_actor = await svc_auth.authenticate_from_token(site_token);
        } catch (e) {
          failed = true;
        }
        ...
        // the last request
        if (Date.now() - site_actor.type.iat * 1000 > 1000 * 30) {
      ...
      const make_site_actor_from_app_token = async () => {
        ...
        const app_actor = await svc_auth.authenticate_from_token(token);

        const user_actor = app_actor.get_related_actor(UserActorType);
  ...
  respond_error_({ req, res, e }) {
    ...
    res.redirect(
      `${config.origin}?${e.querystringize({
        ...(req.query['puter.app_instance_id']
          ? {
