...
module.exports = eggspress(
  ...
  async (req, res, next) => {
    ...
    if (owner_user.suspended) return res.status(401).send({ error: 'Account suspended' });
      ...
      async get_dest_node() {
        ...
        try {
          return await new FSNodeParam('dest_path').consolidate({
            req,
            getParam: () => req.body.dest_path ?? req.body.destination_uid,
          });
        } catch (e) {
