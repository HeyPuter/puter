...
const {
  is_temp_users_disabled: lazy_temp_users,
...
module.exports = eggspress(
  ...
  async (req, res, next) => {
    ...
    if (
      require('../helpers').subdomain(req) !== 'api' &&
      require('../helpers').subdomain(req) !== ''
    )
      next();
    ...
    if (!req.body.is_temp && req.body.p102xyzname !== '') return res.send();
    ...
    // check if username is of proper length
      ...
      return res
        .status(400)
        .send(`Username cannot be longer than ${config.username_max_length} characters.`);
    ...
    else if (!req.body.is_temp && !req.body.email) return res.status(400).send('Email is required');
    ...
    if (pseudo_user === undefined) {
      ...
      await svc_group.add_users({
        uid: req.body.is_temp ? config.default_temp_group : config.default_user_group,
    ...
    if ((!req.body.is_temp && email_confirmation_required) || user.requires_email_confirmation) {
      ...
      else send_email_verification_token(user.email_confirm_token, user.email, user.uuid);
    }
    ...
    let referral_code;
    if (pseudo_user === undefined) {
      const svc_referralCode = Context.get('services').get('referral-code', { optional: true });
      if (svc_referralCode) {
