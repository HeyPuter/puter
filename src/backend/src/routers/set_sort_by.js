...
router.post('/set_sort_by', auth, express.json(), async (req, res, next) => {
  // check subdomain
  if (require('../helpers').subdomain(req) !== 'api') next();

  // check if user is verified
  if (
    (config.strict_email_verification_required || req.user.requires_email_confirmation) &&
    !req.user.email_confirmed
  )
    return res
      .status(400)
      .send({ code: 'account_is_not_verified', message: 'Account is not verified' });

  // validation
  if (req.body.item_uid === undefined && req.body.item_path === undefined)
    return res.status(400).send('`item_uid` or `item_path` is required');
  else if (req.body.sort_by === undefined) return res.status(400).send('`sort_by` is required');
  else if (
    req.body.sort_by !== 'name' &&
    req.body.sort_by !== 'size' &&
    req.body.sort_by !== 'modified' &&
    req.body.sort_by !== 'type'
  )
  ...
  //get dir
  let item;
  if (req.body.item_uid) item = await uuid2fsentry(req.body.item_uid);
  else if (req.body.item_path) item = await convert_path_to_fsentry(req.body.item_path);
  ...
  // must be dir
  if (!item.is_dir) return res.status(400).send(`must be a directory`);
  ...
  // set sort_by
  await db.write(`UPDATE fsentries SET sort_by = ? WHERE id = ?`, [req.body.sort_by, item.id]);
