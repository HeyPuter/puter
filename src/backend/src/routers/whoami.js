...
const WHOAMI_GET = eggspress(
  ...
  async (req, res, next) => {
    ...
    const details = {
      ...
      email_confirmed: req.user.email_confirmed || req.user.username === 'admin',
      ...
      taskbar_items: await get_taskbar_items(req.user, {
        ...(req.query.icon_size ? { icon_size: req.query.icon_size } : { no_icons: true }),
      }),
...
WHOAMI_POST.post('/whoami', auth, express.json(), async (req, response, next) => {
  ...
  if (
    req.query.return_desktop_items === 1 ||
    req.query.return_desktop_items === '1' ||
    req.query.return_desktop_items === 'true'
  ) {
        ...
        for (let i = 0; i < desktop_items.length; i++) {
          if (desktop_items[i].id !== null) {
            ...
            if (!desktop_items[i].is_dir) {
              desktop_items[i].suggested_apps = await suggest_app_for_fsentry(desktop_items[i], {
                user: req.user,
              });
            }
            // is_shared
