...
router.post('/setItem', auth, express.json(), async (req, res, next) => {
  ...
  else if (typeof req.body.key !== 'string') return res.status(400).send('`key` must be a string');
  else if (!req.body.value) return res.status(400).send('`value` is required');
  ...
  if (Buffer.byteLength(req.body.key, 'utf8') > config.kv_max_key_size)
    return res
      .status(400)
      .send('`key` is too large. Max size is ' + byte_format(config.kv_max_key_size) + '.');
  else if (Buffer.byteLength(req.body.value, 'utf8') > config.kv_max_value_size)
    return res
      .status(400)
      .send('`value` is too large. Max size is ' + byte_format(config.kv_max_value_size) + '.');
