...
router.post('/suggest_apps', auth, express.json(), async (req, res, next) => {
  ...
  // by uid
  if (req.body.uid) fsentry = await uuid2fsentry(req.body.uid);
  // by path
  else {
    fsentry = await convert_path_to_fsentry(req.body.path);
    if (fsentry === false) return res.status(400).send('Path not found.');
  }

  const services = Context.get('services');
  const fs = services.get('filesystem');
  const node = await fs.node(
    new NodeInternalIDSelector('mysql', fsentry.id, {
      source: 'suggest_apps',
    })
  );
  ...
  if (!(await svc_acl.check(actor, node, 'read'))) {
    (await svc_acl.get_safe_acl_error(actor, node, 'read')).write(res);
    return;
  }
