...
router.post(
  ...
  async (req, res, next) => {
    ...
    else if (req.query.path.trim() === '') return res.status(400).send('path cannot be empty');
    ...
    if (path === '/') return res.status(400).send('Cannot download a directory.');
    ...
    // stream data from S3
    try {
      const esc_filename = (await fsnode.get('name')).replace(/[^a-zA-Z0-9-_\.]/g, '_');
