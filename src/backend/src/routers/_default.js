...
router.all('*', async function (req, res, next) {
  ...
  // --------------------------------------
    ...
    return res.sendFile(
      _path.join(__dirname, config.defaultjs_asset_path, 'puter.js/alpha.js'),
      function (err) {
  ...
  // --------------------------------------
    ...
    return res.sendFile(
      _path.join(__dirname, config.defaultjs_asset_path, 'puter.js/v1.js'),
      function (err) {
        if (err && err.statusCode) {
    ...
    return res.sendFile(
      _path.join(__dirname, config.defaultjs_asset_path, 'puter.js/v2.js'),
      function (err) {
        if (err && err.statusCode) {
          return res.status(err.statusCode).send('Error /puter.js');
        }
  ...
  // --------------------------------------
    ...
    if (path === '/putility/v1') {
      return res.sendFile(
        _path.join(__dirname, config.defaultjs_asset_path, 'putility.js/v1.js'),
        function (err) {
          if (err && err.statusCode) {
  ...
  // --------------------------------------
    ...
    if (path === '/robots.txt') {
      ...
      if (req.query.user_uuid === undefined)
        ...
        // more validation
        if (!user) h += '<p style="text-align:center; color:red;">User not found.</p>';
        else if (user.unsubscribed === 1)
          h += '<p style="text-align:center; color:green;">You are already unsubscribed.</p>';
        // mark user as confirmed
        else {
          await db.write('UPDATE `user` SET `unsubscribed` = 1 WHERE id = ?', [user.id]);
    ...
    // ------------------------
      ...
      const manifest = _fs.existsSync(_path.join(config.assets.gui, 'puter-gui.json'))
        ? (() => {
  ...
  // --------------------------------------
    ...
    subdomain === 'camera' ||
    subdomain === 'recorder' ||
    subdomain === 'dev-center' ||
    subdomain === 'terminal' ||
    subdomain === 'developer'
  ) {
    let root = PathBuilder.add(__dirname)
  ...
  //------------------------------------------
    ...
    return res.redirect(
      ...
      req.protocol +
        '://' +
        req.get('host').replace(config.domain, config.static_hosting_domain) +
        req.originalUrl
    );
  }
});
