  ...
  (req, res, next) => {
    ...
    console.log('LOGIN REQUEST RECEIVED with captcha data:', {
      ...
      captchaToken: req.body.captchaToken
        ? req.body.captchaToken.substring(0, 8) + '...'
        : undefined,
  ...
  async (req, res, next) => {
    ...
    try {
      ...
      if (user.username === 'system' && config.allow_system_login !== true) {
        return res.status(400).send(req.body.username ? 'Username not found.' : 'Email not found.');
      }
      // is user suspended?
      if (user.suspended) return res.status(401).send('This account is suspended.');
      // pseudo user?
      // todo make this better, maybe ask them to create an account or send them an activation link
      if (user.password === null) return res.status(400).send('Incorrect password.');
      // check password
      if (await bcrypt.compare(req.body.password, user.password)) {
        ...
        if (user.otp_enabled) {
          ...
          const otp_jwt_token = svc_token.sign(
            'otp',
            {
              user_uid: user.uuid,
            },
...
router.post(
  ...
  express.json(),
  body_parser_error_handler,
  requireCaptcha({ strictMode: true, eventType: 'login_otp' }),
  async (req, res, next) => {
    // either api. subdomain or no subdomain
    if (
      require('../helpers').subdomain(req) !== 'api' &&
...
router.post(
  ...
  express.json(),
  body_parser_error_handler,
  requireCaptcha({ strictMode: true, eventType: 'login_recovery' }),
  async (req, res, next) => {
    // either api. subdomain or no subdomain
    if (
      require('../helpers').subdomain(req) !== 'api' &&
