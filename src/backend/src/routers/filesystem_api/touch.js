...
router.post('/touch', auth, express.json(), async (req, res, next) => {
  // check subdomain
  if (require('../../helpers.js').subdomain(req) !== 'api') next();
  ...
  // validation
  if (req.body.path === undefined) return res.status(400).send('path is required');
  // path must be a string
  else if (typeof req.body.path !== 'string') return res.status(400).send('path must be a string.');
  else if (req.body.path.trim() === '') return res.status(400).send('path cannot be empty');
  ...
  if (dirpath === '/') return res.status(400).send('Can not touch in root.');
  ...
  // dirpath not found
  if (parent === false) return res.status(400).send('Target path not found');
  ...
  const existing_fsentry = await convert_path_to_fsentry(
    _path.resolve('/', dirpath + '/' + target_name)
  );
