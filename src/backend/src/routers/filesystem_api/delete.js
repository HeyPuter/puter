...
module.exports = eggspress(
  ...
  async (req, res, next) => {
    ...
    if (paths === undefined) return res.status(400).send('paths is required');
    else if (!Array.isArray(paths)) return res.status(400).send('paths must be an array');
    else if (paths.length === 0) return res.status(400).send('paths cannot be empty');
