...
router.get('/file', async (req, res, next) => {
  ...
  if (download === 'true' || download === '1' || download === true) download = true;

  // retrieve FSEntry from db
  const fsentry = await db.read(`SELECT * FROM fsentries WHERE uuid = ? LIMIT 1`, [uid]);

  // FSEntry not found
  if (!fsentry[0]) return res.status(400).send({ message: 'No entry found with this uid' });

  // check if item owner is suspended
  const user = await get_user({ id: fsentry[0].user_id });
  if (user.suspended) return res.status(401).send({ error: 'Account suspended' });
  ...
  if (fsentry[0].is_dir) {
    ...
    if (children.length > 0) {
      for (const child of children) {
        // sign file
        const signed_child = await sign_file(child, can_write ? 'write' : 'read');
  ...
  // force download?
  if (download) res.attachment(fsentry[0].name);
  ...
  // update `accessed`
  db.write('UPDATE fsentries SET accessed = ? WHERE `id` = ?', [Date.now() / 1000, fsentry[0].id]);
  ...
  if (!range) {
    ...
    try {
      ...
      return res
        ...
        .status(500)
        .send({ message: 'There was an internal problem reading the file.' });
    }
  }
  //--------------------------------------------------
  // Range
  //--------------------------------------------------
    ...
    if (
      user_agent &&
      user_agent.toLowerCase().includes('safari') &&
