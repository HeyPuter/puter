...
router.get('/itemMetadata', async (req, res, next) => {
  ...
  // Validation
  if (!req.query.uid) return res.status(400).send('`uid` is required');
  ...
  else if (!is_valid_uuid4(req.query.uid)) return res.status(400).send('uid must be a valid uuid');
  ...
  if (req.query.return_versions) {
    ...
    let versions = await db.read(`SELECT * FROM fsentry_versions WHERE fsentry_id = ?`, [item.id]);
