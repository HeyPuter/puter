...
router.post('/save_account', auth, express.json(), async (req, res, next) => {
  ...
  // validation
  if (req.user.password !== null) return res.status(400).send('User account already saved.');
  else if (!req.body.username) return res.status(400).send('Username is required');
  // username must be a string
    ...
    return res
      .status(400)
      .send(`Username cannot have more than ${config.username_max_length} characters.`);
  ...
  else if (!req.body.email) return res.status(400).send('Email is required');
  // email must be a string
  else if (typeof req.body.email !== 'string')
    return res.status(400).send('email must be a string.');
  else if (!validator.isEmail(req.body.email))
    return res.status(400).send('Please enter a valid email address.');
  else if (!req.body.password) return res.status(400).send('Password is required');
  // password must be a string
    ...
    return res
      .status(400)
      .send(`Password must be at least ${config.min_pass_length} characters long.`);
  ...
  return svc_lock.lock(
    [`save-account:username:${req.body.username}`, `save-account:email:${req.body.email}`],
    { timeout: 5 * SECOND },
    async () => {
      ...
      if (req.body.username !== req.user.username && (await username_exists(req.body.username)))
        return res
          .status(400)
          .send('This username already exists in our database. Please use another one.');
      // duplicate email check (pseudo-users don't count)
      let rows2 = await db.read(
        `SELECT EXISTS(SELECT 1 FROM user WHERE email=? AND password IS NOT NULL) AS email_exists`,
        [req.body.email]
      );
      if (rows2[0].email_exists)
        return res
          .status(400)
          .send('This email already exists in our database. Please use another one.');
      // get pseudo user, if exists
      let pseudo_user = await db.read(`SELECT * FROM user WHERE email = ? AND password IS NULL`, [
        req.body.email,
      ]);
      ...
      if (pseudo_user === undefined) {
        ...
        await db.write(`UPDATE fsentries SET name = ? WHERE user_id = ? and parent_uid IS NULL`, [
          // name
          req.body.username,
          // id
          req.user.id,
        ...
        await filesystem.update_child_paths(
          `/${req.user.username}`,
          `/${req.body.username}`,
          req.user.id
        );
        ...
        else send_email_verification_token(email_confirm_token, req.body.email, user_uuid);
      }
