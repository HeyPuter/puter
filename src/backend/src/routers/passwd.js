...
router.post('/passwd', auth, express.json(), async (req, res, next) => {
  ...
  if (!req.body.old_pass) return res.status(401).send('old_pass is required');
  // old_pass must be a string
  else if (typeof req.body.old_pass !== 'string')
    return res.status(400).send('old_pass must be a string.');
  else if (!req.body.new_pass) return res.status(401).send('new_pass is required');
  ...
  try {
    ...
    if (!isMatch) return res.status(400).send('old_pass does not match your current password.');
