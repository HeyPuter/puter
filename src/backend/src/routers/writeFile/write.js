...
module.exports = async function writeFile_handle_write({ req, res, actor, node }) {
  ...
  try {
    dirname = ((await node.get('type')) !== TYPE_DIRECTORY ? _path.dirname.bind(_path) : (a) => a)(
      await node.get('path')
    );
  } catch (e) {
    console.log(e);
  ...
  for (const uploaded_file of req.files) {
    try {
      ...
      const ret_obj = await hl_write.run({
        ...
        specified_name:
          (await node.get('type')) === TYPE_DIRECTORY ? req.body.name : await node.get('name'),
