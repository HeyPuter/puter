...
router.post('/remove-site-dir', auth, express.json(), async (req, res, next) => {
  ...
  // validation
  if (req.body.dir_uuid === undefined) return res.status(400).send('dir_uuid is required');
  ...
  if (item !== false) {
    ...
    // if site_uuid is undefined, disassociate all websites from this directory
      ...
      await db
        .promise()
        .execute(`UPDATE subdomains SET root_dir_id = NULL WHERE user_id = ? AND root_dir_id =?`, [
          user.id,
          item.id,
        ]);
