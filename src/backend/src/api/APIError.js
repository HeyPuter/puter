...
module.exports = class APIError {
  static codes = {
    ...
    disallowed_value: {
      status: 400,
      message: ({ key, allowed }) =>
        `value of ${quot(key)} must be one of: ` + allowed.map((v) => quot(v)).join(', '),
    },
    invalid_token: {
      status: 400,
    ...
    disallowed_thing: {
      ...
      message: ({ thing_type, accepted }) =>
        ...
        (accepted ? '; ' + 'accepted types are: ' + accepted.map((v) => quot(v)).join(', ') : '') +
        '.',
    },
    ...
    field_only_valid_with_other_field: {
      status: 400,
      message: ({ key, other_key }) =>
        `Field ${quot(key)} is only valid when field ${quot(other_key)} is specified.`,
    },
    invalid_id: {
      status: 400,
      message: ({ id }) => {
    ...
    field_too_long: {
      status: 400,
      message: ({ key, max_length }) =>
        `Field ${quot(key)} is too long. Max length is ${max_length}.`,
    },
    field_too_short: {
      status: 400,
      message: ({ key, min_length }) =>
        `Field ${quot(key)} is too short. Min length is ${min_length}.`,
    },
    already_in_use: {
      status: 409,
    ...
    invalid_file_name: {
      status: 400,
      message: ({ name, reason }) =>
        `Invalid file name: ${quot(name)}${reason ? `; ${reason}` : '.'}`,
    },
    ...
    internal_error: {
      status: 500,
      message: ({ message }) =>
        message ? 'An internal error occurred: ' + quot(message) : 'An internal error occurred.',
    },
    response_timeout: {
      status: 504,
    ...
    subdomain_limit_reached: {
      status: 400,
      message: ({ limit }) =>
        `You have exceeded the number of subdomains under your current plan (${limit}).`,
    },
    subdomain_reserved: {
      status: 400,
    ...
    invalid_username_or_email: {
      status: 400,
      message: ({ value }) => `The value ${quot(value)} is not a valid username or email.`,
    },
    invalid_path: {
      status: 400,
      message: ({ value }) => `The value ${quot(value)} is not a valid path.`,
    },
    future: {
      status: 400,
    ...
    field_errors: {
      status: 400,
      message: ({ key, errors }) =>
        `The value for ${quot(key)} has the following errors: ` + errors.join('; '),
    },
    share_expired: {
      status: 422,
    ...
    email_must_be_confirmed: {
      status: 422,
      message: ({ action }) => `Email must be confirmed to ${action ?? 'apply a share'}.`,
    },
    no_need_to_request: {
      status: 422,
    ...
    invalid_engine: {
      status: 400,
      message: ({ engine, valid_engines }) =>
        `Invalid engine: ${quot(engine)}. Valid engines are: ${valid_engines.map(quot).join(', ')}.`,
    },
  };
  ...
  static create(status, source, fields = {}) {
    ...
    if (typeof source === 'object' && source instanceof Error) {
  ...
  write(res) {
    const message = typeof this.message === 'function' ? this.message(this.fields) : this.message;
  ...
  get message() {
    const message =
      typeof this._message === 'function' ? this._message(this.fields) : this._message;
    return message;
  }
