...
class ExtensionServiceState extends AdvancedBase {
  ...
  register_route_handler_(path, handler, options = {}) {
    ...
    if (!options.noauth) {
      const auth_conf = typeof options.auth === 'object' ? options.auth : {};
...
class ExtensionService extends BaseService {
  ...
  async _init(args) {
    ...
    this.state.values.set(
      'log_context',
      this.services.get('log-service').create(this.state.extension.name)
    );
    ...
    svc_event.on_all(async (key, data, meta = {}) => {
      ...
      const promises = [];
      promises.push(this.state.extension.emit(`core.${key}`, data, meta));
      promises.push(this.state.extension.emit(key, data, meta));
