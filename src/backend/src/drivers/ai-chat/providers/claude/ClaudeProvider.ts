...
import type { AIChatStream, AIChatTextStream, AIChatToolUseStream } from '../../utils/Streaming.js';
...
export class ClaudeProvider implements IChatProvider {
    ...
    async complete ({ messages, stream, model, tools, max_tokens, temperature, reasoning, reasoning_effort }: ICompleteArguments): Promise<IChatCompleteResult> {
        ...
        if (
            ...
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
            system_prompts[0].content = system_prompts[0].content.map((prompt: any) => {
        ...
        messages = messages.map((message: any) => {
            ...
            return {
                role: 'user',
                content: [{ type: 'tool_result', tool_use_id: toolUseId, content: contentValue }],
            };
        });
        ...
        const modelUsed = this.models().find(m => [m.id, ...(m.aliases || [])].includes(model))
        ...
        const resolvedTemperature = isOpus47
            ? undefined
            : (thinkingConfig ? 1 : (temperature ?? 0));
        ...
        const sdkParams: MessageCreateParams = {
            ...
            max_tokens: Math.floor(max_tokens
                || ((model === 'claude-3-5-sonnet-20241022' || model === 'claude-3-5-sonnet-20240620')
                    ? 8192
                    // eslint-disable-next-line @typescript-eslint/no-explicit-any
                    : (this.models().filter(e => ((e as any).name === model || e.aliases?.includes(model)))[0]?.max_tokens || 4096))),
        ...
        if ( stream ) {
            const init_chat_stream = async ({ chatStream }: { chatStream: AIChatStream }) => {
                const completion = await this.anthropic.messages.stream(sdkParams);
                ...
                for await ( const event of completion ) {
                    if ( event.type === 'message_delta' ) {
                        const meteredData = this.#usageFormatterUtil((event?.usage ?? {}) as Usage | BetaUsage);
                    ...
                    if ( event.type === 'content_block_start' ) {
                        ...
                        if ( event.content_block.type === 'tool_use' ) {
                            ...
                            contentBlock = message!.contentBlock({ type: 'text' });
                        } else {
                            contentBlock = message!.contentBlock({ type: event.content_block.type });
                        }
                        continue;
                    }
                    ...
                    if ( event.type === 'content_block_delta' ) {
                        if ( event.delta.type === 'input_json_delta' ) {
                            ...
                            (contentBlock as AIChatTextStream)!.addReasoning((event.delta as { thinking: string }).thinking);
                        }
                        // signature_delta — ignored
                    }
                }
                const finalUsage = await completion.finalMessage()
                    .then(msg => this.#usageFormatterUtil(msg.usage as Usage | BetaUsage))
                    .catch(() => null);
                if ( finalUsage ) {
    ...
    #buildThinkingConfig ({ modelId, reasoningEffort, maxTokens }: {
        modelId?: string;
        ...
        const requestedBudget = { low: 1024, medium: 4096, high: 8192 }[reasoningEffort];
