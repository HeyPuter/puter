...
module.exports = new Sequence([
  ...
  function initialize_result_object(a) {
    a.set('result', {
      ...
      $version: 'v0.0.0',
      status: null,
      recipients: Array(a.get('req_recipients').length).fill(null),
      shares: Array(a.get('req_shares').length).fill(null),
      serialize() {
        const result = this;
  ...
  function abort_on_error_if_mode_is_strict(a) {
    ...
    if (result.recipients.some((v) => v !== null) || result.shares.some((v) => v !== null)) {
      result.serialize();
  ...
  function early_return_on_dry_run(a) {
    ...
    for (const item of recipients_work.list()) {
      result.recipients[item.i] = { $: 'api:status-report', status: 'success' };
    }
  ...
  async function grant_permissions_to_existing_users(a) {
    const { req, result, recipients_work, shares_work } = a.values();
    ...
    for (const recipient_item of recipients_work.list()) {
      ...
      for (const share_item of shares_work.list()) {
        ...
        for (const perm of permissions) {
          if (perm.startsWith('fs:')) {
            await svc_acl.set_user_user(actor, username, perm, undefined, { only_if_higher: true });
          } else {
            await svc_permission.grant_user_user_permission(actor, username, perm);
          }
        }
      }
      ...
      svc_notification.notify(UsernameNotifSelector(username), {
        ...
        text:
          `The user ${quot(req.user.username)} shared ` +
          `${files.length} ` +
      ...
      result.recipients[recipient_item.i] = { $: 'api:status-report', status: 'success' };
    }
  },
  async function email_the_email_recipients(a) {
    ...
    for (const recipient_item of recipients_work.list()) {
      ...
      const email_link = `${config.origin}?share_token=${share_token}`;
