  ...
  [
    function validate_share_types(a) {
      ...
      for (const item of shares_work.list()) {
        ...
        if (thing.$ === 'error') {
          item.invalid = true;
          result.shares[i] = APIError.create('format_error', null, {
        ...
        if (!allowed_things.includes(thing.$)) {
          item.invalid = true;
          result.shares[i] = APIError.create('disallowed_thing', null, {
            thing: thing.$,
    ...
    function create_file_share_intents(a) {
      ...
      for (const item of shares_work.list()) {
        ...
        if (errors.length) {
          item.invalid = true;
          result.shares[item.i] = APIError.create('field_errors', null, {
            key: `shares[${item.i}]`,
            errors,
          });
          continue;
    ...
    function create_app_share_intents(a) {
      ...
      for (const item of shares_work.list()) {
        ...
        const app_selector = thing.uid ? `uid#${thing.uid}` : thing.name;

        item.share_intent = {
          $: 'share-intent:app',
          permissions: [PermissionUtil.join('app', app_selector, 'access')],
        };
        continue;
      }
    },
    ...
    async function fetch_apps_for_app_shares(a) {
      ...
      for (const item of shares_work.list()) {
        ...
        const app = await get_app(thing.uid ? { uid: thing.uid } : { name: thing.name });
        if (!app) {
          item.invalid = true;
          result.shares[item.i] =
            ...
            APIError.create('entity_not_found', null, {
              identifier: thing.uid ? { uid: thing.uid } : { id: { name: thing.name } },
            });
        }
    ...
    async function add_subdomain_permissions(a) {
      ...
      for (const item of shares_work.list()) {
        ...
        const [subdomain] = await db.read(
          `SELECT * FROM subdomains WHERE associated_app_id = ? ` + `AND user_id = ? LIMIT 1`,
    ...
    async function add_appdata_permissions(a) {
      ...
      for (const item of shares_work.list()) {
        ...
        const appdatadir = `/${app_owner.username}/AppData/${item.app.uid}`;
        const appdatadir_perm = PermissionUtil.join('fs', appdatadir, 'write');
