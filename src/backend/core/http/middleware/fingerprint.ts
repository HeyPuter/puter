 ...
 * or service (via the ALS `Context.get('req')`) can read them without
 * recomputing:
 *
 * - `req.networkFingerprint` — always set; a coarse IP+headers hash (the anchor
 *   of the rate limiter's default key). Server-derived, so it can't be forged
