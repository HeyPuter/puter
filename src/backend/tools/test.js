...
class TestLogger {
  ...
  info(...args) {
    console.log('\x1B[36;1m[TESTKERNEL/INFO]\x1B[0m', ...args);
  }
  error(...args) {
    console.log('\x1B[31;1m[TESTKERNEL/ERROR]\x1B[0m', ...args);
  }
}
...
TestKernel.prototype._create_mod_context = Kernel.prototype._create_mod_context;
...
const main = async () => {
  ...
  const service_names =
    process.argv.length > 2 ? process.argv.slice(2) : Object.keys(k.services.instances_);
  ...
  console.log(
    `\x1B[36;1m<===\x1B[0m ` + 'ASSERTION OUTPUTS ARE REPEATED BELOW' + ` \x1B[36;1m===>\x1B[0m`
  );
