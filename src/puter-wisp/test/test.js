...
const { NewVirtioFrameStream, NewWispPacketStream, WispPacket } = require('../src/exports');
...
const BASH_TEST_BYTES = [
  ...
  10, 0, 0, 0, 2, 1, 0, 0, 0, 40, 110, 111, 110, 101, 10, 0, 0, 0, 2, 1, 0, 0, 0, 41, 58, 47, 35,
  32, 7, 0, 0, 0, 2, 1, 0, 0, 0, 13, 10, 14, 0, 0, 0, 2, 1, 0, 0, 0, 27, 91, 63, 50, 48, 48, 52,
  108, 13, 17, 0, 0, 0, 2, 1, 0, 0, 0, 27, 91, 63, 50, 48, 48, 52, 104, 40, 110, 111, 110, 11, 0, 0,
  0, 2, 1, 0, 0, 0, 101, 41, 58, 47, 35, 32,
];
...
const runit = async () => {
  ...
  await test('bash prompt chunking', async () => {
    const byteStream = NewTestChunkedByteStream([
      ...
      // "(none"
      Uint8Array.from([10, 0, 0, 0, 2, 1, 0, 0, 0, 40, 110, 111, 110, 101]),
      // "):/# "
      Uint8Array.from([10, 0, 0, 0, 2, 1, 0, 0, 0, 41, 58, 47, 35, 32]),
    ]);
    const virtioStream = NewVirtioFrameStream(byteStream);
    const wispStream = NewWispPacketStream(virtioStream);
    ...
    const expected = [40, 110, 111, 110, 101, 41, 58, 47, 35, 32];
