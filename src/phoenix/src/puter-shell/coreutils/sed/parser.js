...
class DelimiterParser extends Parser {
  ...
  _parse(stream) {
    ...
    if (this.first) {
      if (this.character && this.character !== value) return UNRECOGNIZED;
      // Backslash and newline are disallowed as delimiters.
      if (value === '\n' || value === '\\') return UNRECOGNIZED;
...
export const parseScript = (script_string, options) => {
  ...
  const parser = grammar_context.define_parser(
    {
      script: (a) =>
        a.repeat(a.optional(a.symbol('command')), a.firstMatch(a.literal('\n'), a.literal(';'))),
      command: (a) =>
        a.sequence(
          a.symbol('whitespace'),
      ...
      address_range: (a) =>
        a.sequence(
          a.optional(
            a.sequence(
              a.symbol('address'),
              a.optional(a.sequence(a.literal(','), a.symbol('address')))
            )
          ),
          a.optional(a.sequence(a.symbol('whitespace'), a.literal('!')))
        ),
      address: (a) =>
        a.firstMatch(
      ...
      regex: (a) =>
        a.sequence(
          a.firstMatch(
            ...
            a.sequence(a.literal('\\'), a.delimiter({ first: true }))
          ),
      ...
      '{': (a) => a.literal('{'),
      '}': (a) => a.literal('}'),
      ':': (a) => a.sequence(a.literal(':'), a.symbol('label')),
      '=': (a) => a.literal('='),
      a: (a) => a.sequence(a.literal('a\\\n'), a.symbol('text')),
      b: (a) =>
        a.sequence(
          a.literal('b'),
          a.optional(a.sequence(a.symbol('whitespace'), a.symbol('label')))
        ),
          ...
          a.optional(
            a.repeat(
              a.firstMatch(
                ...
                a.literal('p'),
                a.symbol('decimal_number'),
                a.sequence(a.literal('w'), a.symbol('whitespace'), a.symbol('filename'))
              )
            )
          )
        ),
    ...
    },
    {
      script: (script) => {
        ...
        for (const command of commands) {
          ...
          if (command instanceof GroupEndCommand) {
            if (group_depth < 1) throw new Error('Unexpected "}": no open groups');
            group_depth--;
          }
        }

        if (group_depth !== 0) throw new Error(`${group_depth} groups left open`);
      ...
      address_range: (address_range) => {
        if (address_range.length === 0) return new AddressRange();
      ...
      address: (address) => {
        if (address instanceof RegExp) return new Address(address);
        ...
        if (flag_values && flag_values.value.length) {
          for (const flag of flag_values.value) {
            if (flag.value instanceof Array) {
              // It's a 'w'
              if (flags.writeToFile) throw new Error(`Multiple 'w' flags given to s command`);
              flags.writeToFile = flag.value[1].value;
            } else if (flag.value === 'g') {
              if (flags.global) throw new Error(`Multiple 'g' flags given to s command`);
              flags.global = true;
            } else if (flag.value === 'p') {
              if (flags.print) throw new Error(`Multiple 'p' flags given to s command`);
              flags.print = true;
            } else {
              // Should be a number
