...
export class BranchCommand extends Command {
  ...
  async run(context) {
    if (typeof this.substitutionCondition === 'boolean') {
      if (context.substitutionResult !== this.substitutionCondition) return JumpLocation.None;
    }
...
export class ReplaceCommand extends Command {
  ...
  dump(indent) {
    return (
      `${makeIndent(indent)}REPLACE-TEXT:\n` +
      this.addressRange.dump(indent + 1) +
      `${makeIndent(indent + 1)}CONTENTS: '${this.text}'\n`
    );
  }
...
export class DeleteCommand extends Command {
  ...
  async run(context) {
    if (this.firstLine) {
      ...
      if (rest === undefined) return JumpLocation.EndOfCycle;
  ...
  dump(indent) {
    return (
      `${makeIndent(indent)}DELETE: ${this.firstLine ? 'LINE' : 'ALL'}\n` +
      this.addressRange.dump(indent + 1)
    );
  }
}
...
export class GetCommand extends Command {
  ...
  dump(indent) {
    return (
      `${makeIndent(indent)}GET-HELD: ${this.append ? 'APPEND' : 'ALL'}\n` +
      this.addressRange.dump(indent + 1)
    );
  }
}
...
export class HoldCommand extends Command {
  ...
  dump(indent) {
    return (
      `${makeIndent(indent)}HOLD: ${this.append ? 'APPEND' : 'ALL'}\n` +
      this.addressRange.dump(indent + 1)
    );
  }
}
...
export class PrintCommand extends Command {
  ...
  dump(indent) {
    return (
      `${makeIndent(indent)}PRINT: ${this.firstLine ? 'LINE' : 'ALL'}\n` +
      this.addressRange.dump(indent + 1)
    );
  }
}
...
export class TransliterateCommand extends Command {
  ...
  dump(indent) {
    return (
      `${makeIndent(indent)}TRANSLITERATE:\n` +
      this.addressRange.dump(indent + 1) +
