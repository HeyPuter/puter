    ...
    const parser = grammar_context.define_parser(
      {
        ...
        array: (a) =>
          a.sequence(
            a.literal('['),
            a.firstMatch(
              a.repeat(a.symbol('element'), a.literal(','), { trailing: false }),
              a.optional(a.symbol('whitespace'))
            ),
            a.literal(']')
          ),
        ...
        object: (a) =>
          a.sequence(
            a.literal('{'),
            a.firstMatch(
              a.repeat(a.symbol('member'), a.literal(','), { trailing: false }),
              a.optional(a.symbol('whitespace'))
            ),
            a.literal('}')
          ),
      ...
      },
      {
        ...
        array: (it) => {
          ...
          return (it[1].value || []).filter((it) => it.$ === 'element').map((it) => it.value);
        },
        member: (it) => {
          const [name_part, value_part] = it.filter(
            (it) => it.$ === 'string' || it.$ === 'element'
          );
