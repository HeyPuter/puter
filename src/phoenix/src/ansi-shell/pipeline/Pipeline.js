...
export class PreparedCommand {
  static async createFromAST(ctx, ast) {
    ...
    const command = cmd ? await commandProvider.lookup(cmd, { ctx }) : command_token;

    if (command === undefined) {
      throw new ConcreteSyntaxError(`no command: ${JSON.stringify(cmd)}`, command_token.$cst);
    }
  ...
  async execute() {
    ...
    try {
      ...
      if (e instanceof Exit) {
        ...
        await ctx.externs.err.write('\x1B[31;1m' + command.name + ': ' + e.message + '\x1B[0m\n');
        exit_code = -1;
      } else {
    ...
    for (let i = 0; i < this.outputRedirects.length; i++) {
      ...
      const dest_path =
        outputRedirect instanceof Token ? await outputRedirect.resolve(this.ctx) : outputRedirect;
