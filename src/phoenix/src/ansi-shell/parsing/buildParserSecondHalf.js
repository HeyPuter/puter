...
class ShellConstructsPStratumImpl {
  static states = [
    ...
    },
    {
      ...
      next({ node, value, lexer }) {
        ...
        if (node.quote === null && (value.$ === 'whitespace' || value.$ === 'op.close')) {
          this.pop();
          return;
        }
  ...
  consolidateTokens(tokens) {
    ...
    if (types.includes('op.pipe')) {
      const components = splitTokens(tokens, (t) => t.$ === 'op.pipe').map((tokens) =>
        this.consolidateTokens(tokens)
      );
    ...
    for (const token of tokens) {
      if (state === states.STATE_REDIRECT) {
        const arry = state.direction === 'out' ? outputRedirects : inputRedirects;
        arry.push({
