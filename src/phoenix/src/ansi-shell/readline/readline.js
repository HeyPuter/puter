  ...
  builder
    ...
    .state('start', async (ctx) => {
      ...
      if (locals.byte === consts.CHAR_TAB) {
        ...
        const completions = await completer.getCompletions(externs.commandCtx, inputState);
      ...
      if (locals.byte === consts.CHAR_DEL) {
        ...
        vars.result = vars.result.slice(0, vars.cursor - 1) + vars.result.slice(vars.cursor);
      ...
      if (vars.cursor === vars.result.length) {
        ...
        // update buffer
        vars.result = vars.result.slice(0, vars.cursor) + part + vars.result.slice(vars.cursor);
        // update cursor
        vars.cursor += part.length;
      }
    })
    ...
    .action('ESC-CSI.post', async (ctx) => {
      ...
      if (ctx.locals.doWrite) {
        externs.out.write(
          new Uint8Array([ctx.consts.CHAR_ESC, ctx.consts.CHAR_CSI, ...controlSequence, finalByte])
        );
      }
    })
    .build();
