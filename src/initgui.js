...
window.initgui = async function () {
  ...
  if (isMobile.phone) $("body").attr("class", "device-phone");
  else if (isMobile.tablet) $("body").attr("class", "device-tablet");
  else $("body").attr("class", "device-desktop");
  ...
  if (url_paths[0]?.toLocaleLowerCase() === "app" && url_paths[1]) {
    ...
    for (let [key, value] of url_query_params) {
      if (!key.startsWith("puter.")) app_query_params[key] = value;
    }
  }
  ...
  //--------------------------------------------------------------------------------------
  if (
    url_query_params.has("puter.fullpage") &&
    (url_query_params.get("puter.fullpage") === "false" ||
      url_query_params.get("puter.fullpage") === "0")
  ) {
    window.is_fullpage_mode = false;
  } else if (
    url_query_params.has("puter.fullpage") &&
    (url_query_params.get("puter.fullpage") === "true" ||
      url_query_params.get("puter.fullpage") === "1")
  ) {
  ...
  //--------------------------------------------------------------------------------------
  if (
    url_query_params.has("embedded_in_popup") &&
    (url_query_params.get("embedded_in_popup") === "true" ||
      url_query_params.get("embedded_in_popup") === "1")
  ) {
  ...
  if (url_query_params.has("r")) {
    ...
    if (window.first_visit_ever) window.show_referral_notice = true;
  }
  ...
  if (window.embedded_in_popup && openerOrigin) {
    ...
    if (
      logged_in_users.length > 0 &&
      (!userAppToken || url_query_params.get("request_auth"))
    ) {
      await UIWindowSessionList({
        reload_on_success: false,
        draggable_body: false,
      ...
      // -------------------------------------------------------------------------------------
        ...
        //--------------------------------------------------------------------------------------
          ...
          window.addEventListener("message", async (event) => {
            if (event.data.msg !== "showSaveFilePickerPopup") return;
              ...
              onSaveFileDialogSave: async function (
                ...
                while (item_with_same_name_already_exists) {
                  ...
                  try {
                    ...
                    let file_signature = await puter.fs.sign(app_uid, {
                      uid: res.uid,
                      action: "write",
                    });
                    ...
                    if (err.code === "item_with_same_name_exists") {
                      const alert_resp = await UIAlert({
                        ...
                        parent_uuid:
                          $(el_filedialog_window).attr("data-element_uuid"),
                      });
  ...
  $(document).bind("mousedown touchstart", function (e) {
    ...
    if (e.type === "touchstart" && !isMobile.phone && !isMobile.tablet) return;
  ...
  $(document).bind("keydown", async function (e) {
    ...
    if (e.which === 37 || e.which === 38 || e.which === 39 || e.which === 40) {
      ...
      if ($(".launch-popover").length > 0) {
        ...
        if (
          $(".launch-popover .start-app-card.launch-app-selected").length ===
            0 &&
          e.which === 40
        ) {
          $(".launch-popover .start-app-card:visible")
            .first()
            .addClass("launch-app-selected");
          // blur search input
        ...
        // if search input is focused and left or right arrow is pressed, return false
          ...
          $(".launch-popover .launch-search").is(":focus") &&
          ...
          let selected_item = $(
            ".launch-popover .start-app-card.launch-app-selected"
          ).get(0);
          ...
          let selected_item_row_count = Math.ceil(
            $(".launch-popover .start-app-card:visible").length / 5
          );
          ...
          if (e.which === 38) {
            ...
            if (selected_item_row === 0) {
              ...
              $(
                ".launch-popover .start-app-card.launch-app-selected"
              ).removeClass("launch-app-selected");
              // bring cursor to the end of the search input
              $(".launch-popover .launch-search").val(
                $(".launch-popover .launch-search").val()
              );
          ...
          new_selected_item_index =
            new_selected_item_row * selected_item_col_count +
            new_selected_item_col;
          new_selected_item = $(".launch-popover .start-app-card:visible").get(
            new_selected_item_index
          );
          ...
          let selected_item_bottom =
            new_selected_item.getBoundingClientRect().bottom;
          let isVisible =
            selected_item_top >= popover_scroll_top &&
            selected_item_bottom <= popover_scroll_top + popover_height;
      ...
      // ----------------------------------------------
        ...
        // if no item is selected and down arrow is pressed, select the first item
        if (
          $(".context-menu-active .context-menu-item-active").length === 0 &&
          e.which === 40
        ) {
          let selected_item = $(".context-menu-active .context-menu-item").get(
            0
          );
        ...
        // if no item is selected and up arrow is pressed, select the last item
          ...
          let selected_item = $(".context-menu .context-menu-item").get(
            $(".context-menu .context-menu-item").length - 1
          );
          select_ctxmenu_item(selected_item);
          return false;
        }
        // if an item is selected and down arrow is pressed, select the next enabled item
          ...
          let selected_item = $(
            ".context-menu-active .context-menu-item-active"
          ).get(0);
          let selected_item_index = $(
            ".context-menu-active .context-menu-item"
          ).index(selected_item);
          let new_selected_item_index = selected_item_index + 1;
          let new_selected_item = $(
            ".context-menu-active .context-menu-item"
          ).get(new_selected_item_index);
        ...
        // if right arrow is pressed, open the submenu by triggering a mouseover event
          ...
          if ($(selected_item).hasClass("context-menu-item-submenu") === true) {
            ...
            select_ctxmenu_item(
              $('.context-menu[data-is-submenu="true"] .context-menu-item').get(
                0
              )
            );
          }
        ...
        // if left arrow is pressed on a submenu, close the submenu
          ...
          $('.context-menu-active[data-is-submenu="true"]').length > 0 &&
          e.which === 37
        ) {
          // get parent menu
          let parent_menu_id = $(
            '.context-menu-active[data-is-submenu="true"]'
          ).data("parent-id");
          let parent_menu = $(
            '.context-menu[data-element-id="' + parent_menu_id + '"]'
          );
          // remove the submenu
          ...
          let selected_item = $(
            ".context-menu-active .context-menu-item-active-blurred"
          ).get(0);
      ...
      // ----------------------------------------------
        ...
        // if Shift key is pressed and ONE item is already selected, pick that item
          ...
          $(active_item_container).find(".item-selected").length === 1 &&
          e.shiftKey
        ) {
          selected_item = $(active_item_container)
            .find(".item-selected")
            .get(0);
        }
        // if Shift key is pressed and MORE THAN ONE item is selected, pick the latest active item
          ...
          selected_item = $(active_element).hasClass("item")
            ? active_element
            : $(active_element).closest(".item").get(0);
        }
        ...
        let active_el_pos = $(selected_item).hasClass("item")
          ? selected_item.getBoundingClientRect()
          : $(selected_item).closest(".item").get(0).getBoundingClientRect();
        ...
        let elements_at_next_pos = document.elementsFromPoint(
          x_nxtpos,
          y_nxtpos
        );
        let next_item;
        for (let index = 0; index < elements_at_next_pos.length; index++) {
          ...
          if (
            $(elem_at_next_pos).hasClass("item") &&
            $(elem_at_next_pos)
              .closest(".item-container")
              .is(active_item_container)
          ) {
        ...
        if (next_item) {
          ...
          if (e.which === 38 || e.which === 40) next_item.scrollIntoView(false);
        }
      }
    }
    //-----------------------------------------------------------------------
    ...
    //-----------------------------------------------------------------------
      ...
      ($(".window-active").hasClass("window-filedialog") ||
        $(".window-active").hasClass("window-alert")) &&
      ...
      ((!$(focused_el).is("input") && !$(focused_el).is("textarea")) ||
        $(focused_el).hasClass("savefiledialog-filename"))
    ) {
    ...
    //-----------------------------------------------------------------------
      ...
      e.which === 27 &&
      $(focused_el).hasClass("window-navbar-path-input")
    ) {
      $(focused_el).blur();
      $(focused_el).val($(focused_el).closest(".window").attr("data-path"));
      $(focused_el).attr(
        "data-path",
        $(focused_el).closest(".window").attr("data-path")
      );
    }
  ...
  $(document).bind("keydown", async function (e) {
    ...
    //-----------------------------------------------------------------------
    if (
      (e.keyCode === 46 && e.shiftKey) ||
      (e.altKey && e.metaKey && e.keyCode === 8)
    ) {
      let $selected_items = $(active_element)
        .closest(`.item-container`)
        .find(`.item-selected`);
    ...
    if (e.keyCode === 46 || (e.keyCode === 8 && (e.ctrlKey || e.metaKey))) {
      ...
      let $selected_items = $(active_element)
        .closest(`.item-container`)
        .find(`.item-selected[data-path^="${trash_path + "/"}"]`);
      if ($selected_items.length > 0) {
        const alert_resp = await UIAlert({
          message: i18n("confirm_delete_multiple_items"),
          buttons: [
        ...
        if (alert_resp === "Delete") {
          ...
          if (trash.is_empty) {
            ...
            $(`.item[data-path="${html_encode(trash_path)}" i]`)
              .find(".item-icon > img")
              .attr("src", window.icons["trash.svg"]);
            $(`.window[data-path="${html_encode(trash_path)}"]`)
      ...
      // regular delete?
        ...
        $selected_items = $(active_element)
          .closest(".item-container")
          .find(".item-selected");
        if ($selected_items.length > 0) {
          // Only delete the items if we're not renaming one.
          if (
            $selected_items.children(".item-name-editor-active").length === 0
          ) {
    ...
    if (
      ...
      let matches = [];
      const selected_items = $(active_item_container)
        .find(`.item-selected`)
        .not(".item-disabled")
        .first();
      ...
      if (
        ...
        $(selected_items)
          .attr("data-name")
          .toLowerCase()
          .startsWith(keypress_item_seach_term)
      ) {
        return false;
      }

      // search for matches
      let haystack = $(active_item_container)
        .find(`.item`)
        .not(".item-disabled");
      ...
      if (matches.length > 0) {
        ...
        update_explorer_footer_selected_items_count(
          $(active_element).closest(".window")
        );
      }
    ...
    //-----------------------------------------------------------------------
      ...
      let matches = [];
      const selected_items = $(".context-menu")
        .find(`.context-menu-item-active`)
        .first();
      ...
      if (matches.length > 0) {
        ...
        $(".context-menu")
          .find(`.context-menu-item-active`)
          .removeClass("context-menu-item-active");
        // select matching item
  ...
  $(document).bind("keyup keydown", async function (e) {
    ...
    if (
      ...
      if ($parent_container) {
        ...
        update_explorer_footer_selected_items_count(
          $parent_container.closest(".window")
        );
      }
    ...
    if (
      ...
      !$(focused_el).is("input") &&
      !$(focused_el).is("textarea") &&
      Date.now() - last_enter_pressed_to_rename_ts > 200 &&
      ...
      // ---------------------------------------------
        ...
        if ($('.context-menu[data-is-submenu="true"]').length > 0) {
          let selected_item = $(
            '.context-menu[data-is-submenu="true"] .context-menu-item'
          ).get(0);
          select_ctxmenu_item(selected_item);
        }
      ...
      if (parent_container) {
        ...
        if (
          (target_path === trash_path ||
            target_path.startsWith(trash_path + "/")) &&
          clipboard_op !== "move"
        )
          return;
        // execute clipboard operation
        if (clipboard_op === "copy") copy_clipboard_items(target_path);
  ...
  $(document).mousemove(function (event) {
    ...
    if (
      (mouseX < 150 && mouseY < toolbar_height + 20) ||
      (mouseX < 20 && mouseY < 150)
    )
    ...
    // mouse in right edge of screen
      ...
      mouseX > desktop_width - 20 &&
      mouseY >= 150 &&
      mouseY < desktop_height - 150
    )
      current_active_snap_zone = "e";
    // mouse in top-right corner of screen
    else if (
      (mouseX > desktop_width - 150 && mouseY < toolbar_height + 20) ||
      (mouseX > desktop_width - 20 && mouseY < 150)
    )
    ...
    // mouse in top edge of screen
      ...
      mouseY < toolbar_height + 20 &&
      mouseX >= 150 &&
      mouseX < desktop_width - 150
    )
      current_active_snap_zone = "n";
    // not in any snap zone
    else current_active_snap_zone = undefined;
    ...
    if (item_containers.length > 0) {
      ...
      for (let i = 0; i < item_containers.length; i++) {
        ...
        if (
          ...
          let active_container_zindex = parseInt(
            $(item_containers[i]).closest(".window").css("z-index")
          );
          if (
            !isNaN(active_container_zindex) &&
            active_container_zindex >= highest_window_zindex
          ) {
  ...
  $(document).on("paste", function (event) {
    ...
    if (!(items instanceof DataTransferItemList)) return;
...
$(window).on("resize", function () {
  ...
  if (window.embedded_in_popup) return;
