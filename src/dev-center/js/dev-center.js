...
function refresh_app_list(show_loading = false) {
  if (show_loading) puter.ui.showSpinner();
  // get apps
  setTimeout(
    function () {
...
async function create_app(title, source_path = null, items = null) {
  ...
  puter.apps
    ...
    .then(async (app) => {
      ...
      // ----------------------------------------------------
      app_dir = await puter.fs.mkdir(`/${authUsername}/AppData/${dev_center_uid}/${app.uid}`, {
        overwrite: true,
        recursive: true,
        rename: false,
      });
      ...
      .then(async (app) => {
        ...
        puter.fs
          ...
          .then(async (stat) => {
            ...
            puter.fs.delete(`/${authUsername}/AppData/${dev_center_uid}/${app_uid}`, {
              recursive: true,
            });
          });
      })
  ...
  h += `
        ...
        <div class="section-tab" data-tab="info">
            <form style="clear:both; padding-bottom: 50px;">
               ...
               <textarea id="edit-app-filetype-associations"  placeholder="Paste multiple extensions like: .txt, .doc, .pdf, application/json">${JSON.stringify(
                 app.filetype_associations.map((item) => ({ value: item })),
                 null,
                 app.filetype_associations.length
               ).replace(/</g, '\\u003c')}</textarea>
...
function hasChanges() {
  ...
  return (
    ...
    $('#edit-app-fullpage-on-landing').is(':checked') !==
      originalValues.checkboxes.fullPageOnLanding
  );
}
...
async function edit_app_section(cur_app_name, tab = 'deploy') {
  ...
  const filetype_association_input = document.querySelector(
  ...
  if (tagify) {
    ...
    tagify.DOM.input.addEventListener('keydown', function (e) {
      if (e.key === ',' && tagify.DOM.input.textContent.trim()) {
        ...
        if ((text.startsWith('.') || text.includes('/')) && tagify.settings.pattern.test(text)) {
...
$('.jip-submit-btn').on('click', async function (e) {
  ...
  if (first_name === '' || last_name === '' || paypal === '') error = `All fields are required.`;
  ...
  else if (paypal.length > 100) error = `<strong>Paypal</strong> cannot be longer than ${100}.`;
  // check if email is valid
  else if (!validateEmail(paypal)) error = `Paypal email must be a valid email address.`;
...
$(document).on('click', '.edit-app-save-btn', async function (e) {
  ...
  else if (title.length > 60) error = `<strong>Title</strong> cannot be longer than ${60}.`;
  else if (name === '') error = `<strong>Name</strong> is required.`;
  else if (name.length > 60) error = `<strong>Name</strong> cannot be longer than ${60}.`;
  ...
  else if (!is_valid_url(index_url)) error = `<strong>Index URL</strong> must be a valid url.`;
  else if (
  ...
  else if (isNaN(height)) error = `<strong>Window Height</strong> must be a number.`;
  // height must be greater than 0
  else if (height <= 0) error = `<strong>Window Height</strong> must be greater than 0.`;
  // width must be a number
  else if (isNaN(width)) error = `<strong>Window Width</strong> must be a number.`;
  // width must be greater than 0
  else if (width <= 0) error = `<strong>Window Width</strong> must be greater than 0.`;
  // top must be a number
  else if (top && isNaN(top)) error = `<strong>Window Top</strong> must be a number.`;
  // left must be a number
  else if (left && isNaN(left)) error = `<strong>Window Left</strong> must be a number.`;
  ...
  if ($('#edit-app-social-image').attr('data-base64')) {
    socialImageUrl = await handleSocialImageUpload(
      name,
      $('#edit-app-social-image').attr('data-base64')
    );
  ...
  puter.apps
    .update(currently_editing_app.name, {
      title: title,
      name: name,
      indexURL: index_url,
      icon: icon,
...
window.deploy = async function (app, items) {
  ...
  $('.drop-area').html(
    deploying_spinner + ' <div>Deploying <span class="deploy-percent">(0%)</span></div>'
  );
  ...
  // --------------------------------------------------------------------
    ...
    puter.fs
      .upload(items, `/${authUsername}/AppData/${dev_center_uid}/${currently_editing_app.uid}`, {
        dedupeName: false,
        overwrite: false,
...
function setRootDirTree(tree, items) {
  ...
  if (
    ...
    let newItems = [];
    for (let item of items) {
      if (item.fullPath) item.finalPath = item.fullPath.replace(rootKeys[0], '');
...
$(document).on('change', '.app-checkbox', function (e) {
  ...
  if ($('.app-checkbox:checked').length > 0) $('.delete-apps-btn').removeClass('disabled');
  ...
  if (resp === 'delete') {
    ...
    for (let app of apps) {
      ...
      try {
        ...
        // delete app directory
        await puter.fs.delete(`/${authUsername}/AppData/${dev_center_uid}/${app_uid}`, {
          recursive: true,
        });
        count_apps();
      } catch (err) {
...
async function initializeAssetsDirectory() {
  try {
    ...
    if (!existingURL) {
      ...
      const assetsDir = await puter.fs.mkdir(`/${authUsername}/AppData/${dev_center_uid}/assets`, {
        overwrite: false,
      });
...
async function handleSocialImageUpload(app_name, socialImageData) {
  ...
  try {
    ...
    // Upload new image
    await puter.fs.upload(new File([blob], `${app_name}.png`, { type: 'image/png' }), assetsDir, {
      overwrite: true,
    });
...
async function render_analytics(period) {
  ...
  // Create new canvas
  const container = $(
    '<div class="analytics-container" style="width:100%; height:400px; margin-top:30px;"></div>'
  );
