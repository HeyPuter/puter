...
window.onload = async function () {
  const modules = {};
  modules.bench = (
    await WebAssembly.instantiateStreaming(fetch('./static/bench.wasm'))
  ).instance.exports;
  ...
  var emulator = (window.emulator = new V86({
    ...
    cmdline:
      'rw root=/dev/sda init=/sbin/init rootfstype=ext4 puterusername=' +
      (await puter.getUser()).username,
  ...
  class PTY {
    ...
    send(data) {
      ...
      data = new DataBuilder({ leb: true }).uint8(0x02).uint32(this.streamId).cat(data).build();
      const packet = new WispPacket({ data, direction: WispPacket.SEND });
      this.client.send(packet);
    }
  ...
  ptyMgr = new PTYManager({
    client: new WispClient({
      ...
      sendFn: (packet) => {
        ...
        emulator.bus.send('virtio-console0-input-bytes', virtioframe);
      },
    }),
  });
  ptyMgr.init();
