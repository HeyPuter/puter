...
async function handle_resp(
  ...
  // success
    ...
    if (resp.success === false && resp.error?.code === "permission_denied") {
      let perm = await puter.ui.requestPermission({
...
function setupXhrEventHandlers(
  xhr,
  success_cb,
  error_cb,
  resolve_func,
...
async function driverCall_(
  ...
  xhr.onreadystatechange = () => {
    if (xhr.readyState === 2) {
      if (xhr.getResponseHeader("Content-Type") !== "application/x-ndjson")
        return;
      is_stream = true;
