...
class AI {
  ...
  img2txt = async (...args) => {
    ...
    return await utils
      .make_driver_method(['source'], 'puter-ocr', 'aws-textract', 'recognize', {
        test_mode: testMode ?? false,
        transform: async (result) => {
          let str = '';
          for (let i = 0; i < result?.blocks?.length; i++) {
            if ('text/textract:LINE' === result.blocks[i].type) str += result.blocks[i].text + '\n';
          }
          return str;
        },
      })
  ...
  txt2speech = async (...args) => {
    ...
    return await utils
      .make_driver_method(['source'], 'puter-tts', 'aws-polly', 'synthesize', {
        responseType: 'blob',
        test_mode: testMode ?? false,
        transform: async (result) => {
  ...
  txt2speech = Object.assign(this.txt2speech, {
    ...
    listEngines: async () => {
      return await utils
        .make_driver_method(['source'], 'puter-tts', 'aws-polly', 'list_engines', {
          responseType: 'text',
        })
        .call(this, {});
    },
    ...
    listVoices: async (engine) => {
      ...
      return utils
        .make_driver_method(['source'], 'puter-tts', 'aws-polly', 'list_voices', {
          responseType: 'text',
        })
        .call(this, params);
    },
  ...
  chat = async (...args) => {
    ...
    // ai.chat(prompt, imageURL/File, testMode)
      ...
      (typeof args[1] === 'string' || args[1] instanceof File)
    ) {
    ...
    if (
      requestParams.model === 'claude-sonnet-4' ||
      requestParams.model === 'claude-sonnet-4-latest'
    ) {
    ...
    if (!requestParams.model || requestParams.model.startsWith('gpt-')) {
      ...
      requestParams.model === 'meta-llama/Meta-Llama-3.1-8B-Instruct-Turbo' ||
      requestParams.model === 'meta-llama/Meta-Llama-3.1-70B-Instruct-Turbo' ||
      requestParams.model === 'meta-llama/Meta-Llama-3.1-405B-Instruct-Turbo' ||
      requestParams.model === `google/gemma-2-27b-it`
    ) {
      driver = 'together-ai';
    } else if (
      requestParams.model.startsWith('mistral-') ||
      requestParams.model.startsWith('codestral-') ||
      requestParams.model.startsWith('pixtral-') ||
      requestParams.model.startsWith('magistral-') ||
      requestParams.model.startsWith('devstral-') ||
      requestParams.model.startsWith('mistral-ocr-') ||
      requestParams.model.startsWith('open-mistral-')
    ) {
      driver = 'mistral';
    } else if (
      [
    ...
    return await utils
      .make_driver_method(['messages'], 'puter-chat-completion', driver, 'complete', {
        test_mode: testMode ?? false,
        transform: async (result) => {
          result.toString = () => {
  ...
  txt2img = async (...args) => {
    ...
    // Call the original chat.complete method
    return await utils
      .make_driver_method(['prompt'], 'puter-image-generation', undefined, 'generate', {
        responseType: 'blob',
        test_mode: testMode ?? false,
