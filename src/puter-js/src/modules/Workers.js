export class WorkersHandler {
  ...
  async create(workerName, filePath) {
    ...
    const driverCall = await puter.drivers.call('workers', 'worker-service', 'create', {
    ...
    if (!driverCall.success || !driverResult.success) {
      throw new Error(
        driverResult?.errors || 'Driver failed to execute, do you have the necessary permissions?'
      );
    }
  ...
  async delete(workerName) {
    const driverCall = await puter.drivers.call('workers', 'worker-service', 'destroy', {
      authorization: puter.authToken,
      workerName,
    });
