...
const upload = async function (items, dirPath, options = {}) {
  return new Promise(async (resolve, reject) => {
    ...
    if (
      ...
      items instanceof DataTransferItem ||
      items[0] instanceof DataTransferItem ||
      options.parsedDataTransferItems
    ) {
    ...
    // FileList/File
      ...
      items instanceof File ||
      items[0] instanceof File ||
      items instanceof FileList ||
      items[0] instanceof FileList
    ) {
    ...
    for (let i = 0; i < entries.length; i++) {
      ...
      if (entries[i].isDirectory)
        dirs.push({
          path: path.join(
            dirPath,
            entries[i].finalPath ? entries[i].finalPath : entries[i].fullPath
          ),
        });
      // also files
      else {
        ...
        let [dirLevel, fileName] = [
          fileItem?.slice(0, fileItem?.lastIndexOf('/')),
          fileItem?.slice(fileItem?.lastIndexOf('/') + 1),
        ];
    ...
    for (let i = 0; i < dirs.length; i++) {
      ...
      for (let j = 0; j < files.length; j++) {
        ...
        if (
          !files[j].puter_path_param &&
          path.join(dirPath, files[j].filepath).startsWith(dirs[i].path + '/')
        ) {
    ...
    for (let i = 0; i < files.length; i++) {
      fd.append(
        ...
        JSON.stringify({
          ...
          path:
            (files[i].puter_path_param && path.dirname(files[i].puter_path_param ?? '')) ||
            (files[i].filepath && path.join(dirPath, path.dirname(files[i].filepath))) ||
            '',
    ...
    xhr.upload.addEventListener('progress', function (e) {
      ...
      let op_progress = (
        ((bytes_uploaded_to_cloud + bytes_uploaded_to_server) / total_size) *
        100
      ).toFixed(2);
