      ...
      window.addEventListener('message', (event) => {
        if (event.data.msg === 'messageToApp') {
          ...
          if (event.data.targetAppInstanceID !== this.appInstanceID) {
            console.error(
              `AppConnection received message intended for wrong app! appInstanceID=${this.appInstanceID}, target=${event.data.targetAppInstanceID}`
            );
            return;
          }
...
class UI extends EventListener {
  ...
  #ipc_stub = async function ({ callback, method, parameters }) {
    let p, resolve;
  ...
  constructor(context, { appInstanceID, parentInstanceID }) {
    const eventNames = ['localeChanged', 'themeChanged', 'connection'];
    super(eventNames);
    ...
    globalThis.document &&
      window.addEventListener('message', async (e) => {
        ...
        // a callback function for this message? if answer is yes to both then execute the callback
          ...
          e.data.original_msg_id !== undefined &&
        ...
        // Item Watch response
          ...
          if (
            ...
            typeof this.itemWatchCallbackFunctions[e.data.data.uid] === 'function'
          )
  ...
  getEntriesFromDataTransferItems = async function (dataTransferItems, options = { raw: false }) {
    const checkErr = (err) => {
      ...
      const infoMsg =
        `${err.name} occurred within datatransfer-files-promise module\n` +
  ...
  menubar = function () {
    ...
    window.addEventListener('blur', function (e) {
      ...
      document
        ...
        .forEach((el) => el.classList.remove('menubar-item-open'));
    });
    ...
    document.querySelectorAll('.--puter-menubar .menubar-item').forEach((el) =>
