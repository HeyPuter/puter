  ...
  return new Promise(async (res, rej) => {
    try {
      ...
      if (parsedURL.protocol === 'http:') {
        socket = new puter.net.Socket(parsedURL.hostname, parsedURL.port || 80);
      } else if (parsedURL.protocol === 'https:') {
        socket = new puter.net.tls.TLSSocket(parsedURL.hostname, parsedURL.port || 443);
      } else {
        rej(`Failed to fetch. URL scheme "${parsedURL.protocol}" is not supported.`);
      }
