...
window.fsTests = [
  {
    ...
    test: async function () {
      try {
        ...
        if (puter.debugMode) console.log(error);
  ...
  },
  {
    ...
    description:
      'Test writing to an existing file with overwrite and dedupe disabled - should fail',
    test: async function () {
      try {
        let randName = puter.randName();
        await puter.fs.write(randName, 'testValue');
  ...
  },
  {
    ...
    description:
      'Test writing to an existing file with overwrite disabled but dedupe enabled - should create new file',
    test: async function () {
      try {
        let randName = puter.randName();
        await puter.fs.write(randName, 'testValue');
        ...
        try {
          ...
          fail(
            'testFSWriteToExistingFileWithoutOverwriteButWithDedupe failed to delete file:',
            error
          );
        }
      } catch (error) {
  ...
  },
  {
    ...
    description:
      'Test writing to an existing file with overwrite enabled but dedupe disabled - should overwrite',
    test: async function () {
      try {
        let randName = puter.randName();
        await puter.fs.write(randName, 'testValue');
        ...
        try {
          ...
          fail(
            'testFSWriteToExistingFileWithOverwriteButWithoutDedupe failed to delete file:',
            error
          );
        }
      } catch (error) {
  ...
  },
  {
    ...
    description:
      'Test attempting to delete a non-existent file and verify it returns a valid response',
    test: async function () {
      try {
        let randName = puter.randName();
        const result = await puter.fs.delete(randName);
        ...
        else pass('testFSDeleteNonExistentFile passed');
      }
    },
  },
  {
    ...
    description:
      'Test attempting to read from a non-existent file and verify it returns a valid response',
    test: async function () {
      try {
        let randName = puter.randName();
        ...
        else pass('testFSReadNonExistentFile passed');
      }
    },
  },
  {
    ...
    description:
      'Test writing text content to a file with special characters and verify it returns a valid UID',
    test: async function () {
      let randName;
      try {
  ...
  },
  {
    ...
    description:
      'Test reading text content from a file with special characters and verify it matches the written content',
    test: async function () {
      try {
  ...
  },
  {
    ...
    description:
      'Test reading large text content from a file and verify it matches the written content',
    test: async function () {
      try {
        let randName = puter.randName();
    ...
    test: async function () {
      try {
        ...
        try {
          ...
          else pass('testFSRenameFile passed');
        }
      } catch (error) {
    ...
    test: async function () {
      try {
        ...
        try {
          ...
          else pass('testFSMoveFile passed');
        }
      } catch (error) {
  ...
  },
  {
    ...
    description:
      'Test copying a file to a new directory with a new name and verify the old file is still there',
    test: async function () {
      try {
        let randName = puter.randName();
        let randName2 = puter.randName();
  ...
  },
  {
    ...
    description:
      'Test attempting to get metadata from a non-existent file or directory and verify it returns a valid response',
    test: async function () {
      try {
        let randName = puter.randName();
        let result = await puter.fs.stat(randName);
        ...
        if (error.code !== 'subject_does_not_exist') fail('testFSStatNonExistent failed:', error);
        else pass('testFSStatNonExistent passed');
      }
    },
  },
  {
    ...
    description:
      'Test deleting a directory containing multiple files and verify it no longer exists',
    test: async function () {
      try {
        let randName = puter.randName();
        await puter.fs.mkdir(randName);
  ...
  },
  {
    ...
    test: async function () {
      try {
        ...
        assert(
          result.owner !== undefined && typeof result.owner === 'object',
          'Failed to stat directory (owner)'
        );
        assert(
          result.dirname !== undefined && typeof result.dirname === 'string',
          'Failed to stat directory (dirname)'
        );
        assert(
          result.parent_id !== undefined && typeof result.parent_id === 'string',
          'Failed to stat directory (parent_id)'
        );
        // todo this will fail for now until is_dir is turned into boolean
        assert(
          result.is_dir !== undefined &&
            typeof result.is_dir === 'boolean' &&
            result.is_dir === true,
          'Failed to stat directory (is_dir)'
        );
        assert(
          result.is_empty !== undefined && typeof result.is_empty === 'boolean',
          'Failed to stat directory (is_empty)'
        );
  ...
  },
  {
    ...
    description:
      'Test writing text content to files with names from naughtyStrings and verify it returns a valid UID',
    test: async function () {
      try {
        let randName = puter.randName();
        for (let i = 0; i < naughtyStrings.length; i++) {
          ...
          assert(
            ...
            'Failed to write to file with naughty name: ' + filename
          );
          // delete the file
          try {
    ...
    test: async function () {
      try {
        ...
        for (let i = 0; i < binaryData.length; i++) {
          assert(
            readBinaryData[i] === binaryData[i],
            `Binary data mismatch at byte ${i}: expected ${binaryData[i]}, got ${readBinaryData[i]}`
          );
        }
  ...
  },
  {
    ...
    description:
      'Test that filesystem operations are properly sandboxed to the app directory and cannot access files outside of it',
    test: async function () {
      try {
        ...
        try {
          ...
          if (error.code !== 'subject_does_not_exist') {
            fail(
              'testFSAppDirectoryIsolation failed: Wrong error code for Desktop directory access: ' +
                error.code
            );
          }
        }
        ...
        try {
          ...
          if (
            ...
            fail(
              'testFSAppDirectoryIsolation failed: Wrong error code for absolute path access: ' +
                error.code
            );
          }
        }
        ...
        try {
          ...
          fail(
            'testFSAppDirectoryIsolation failed: Should not be able to write outside app directory'
          );
        } catch (error) {
          if (error.code !== 'subject_does_not_exist') {
            fail(
              'testFSAppDirectoryIsolation failed: Wrong error code for writing outside directory: ' +
                error.code
            );
          }
        }
        ...
        try {
          ...
          fail(
            'testFSAppDirectoryIsolation failed: Should not be able to create directory outside app directory'
          );
        } catch (error) {
          if (error.code !== 'subject_does_not_exist') {
            fail(
              'testFSAppDirectoryIsolation failed: Wrong error code for creating directory outside: ' +
                error.code
            );
          }
        }
        ...
        try {
          ...
          fail(
            'testFSAppDirectoryIsolation failed: Should not be able to read from home directory'
          );
        } catch (error) {
          if (
            error.code !== 'access_denied' &&
            ...
            fail(
              'testFSAppDirectoryIsolation failed: Wrong error code for home directory access: ' +
                error.code
            );
          }
        }
