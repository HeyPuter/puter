...
const testTxt2SpeechBasicCore = async function () {
  ...
  assert(
    ...
    `toString() should return a string, got: ${typeof toStringValue} with value: ${toStringValue}`
  );
  assert(
    typeof valueOfValue === 'string',
    `valueOf() should return a string, got: ${typeof valueOfValue} with value: ${valueOfValue}`
  );
  ...
  assert(
    toStringValue === srcValue,
    `toString() should return the same as src. toString(): ${toStringValue}, src: ${srcValue}`
  );
  assert(
    valueOfValue === srcValue,
    `valueOf() should return the same as src. valueOf(): ${valueOfValue}, src: ${srcValue}`
  );
};
...
const testTxt2SpeechWithParametersCore = async function () {
  ...
  const result = await puter.ai.txt2speech(
    'Hello, this is a test with parameters.',
    'en-US',
    'Brian'
  );
  ...
  assert(
    result.src !== basicResult.src,
    'different parameters should produce different audio URLs'
  );
};
...
const testTxt2SpeechWithTestModeCore = async function () {
  ...
  assert(
    typeof toStringValue === 'string',
    `toString() should return a string in test mode, got: ${typeof toStringValue} with value: ${toStringValue}`
  );
  assert(
    typeof valueOfValue === 'string',
    `valueOf() should return a string in test mode, got: ${typeof valueOfValue} with value: ${valueOfValue}`
  );
  ...
  assert(
    toStringValue === srcValue,
    `toString() should return the same as src in test mode. toString(): ${toStringValue}, src: ${srcValue}`
  );
  assert(
    valueOfValue === srcValue,
    `valueOf() should return the same as src in test mode. valueOf(): ${valueOfValue}, src: ${srcValue}`
  );
};
