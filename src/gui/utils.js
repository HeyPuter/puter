...
async function build(options) {
  ...
  for (let i = 0; i < lib_paths.length; i++) {
    ...
    if (file.endsWith('.js') && !file.endsWith('.min.js')) {
      let minified_code = await uglifyjs.minify(fs.readFileSync(file).toString(), {
        mangle: false,
      });
      if (minified_code && minified_code.code) {
        js += minified_code.code;
        if (options?.verbose) console.log('minified: ', file);
      }
    } else {
      js += fs.readFileSync(file);
      if (options?.verbose) console.log('skipped minification: ', file);
    }
  ...
  for (let i = 0; i < css_paths.length; i++) {
    ...
    else css += fs.readFileSync(path.join(__dirname, 'src', css_paths[i]));
