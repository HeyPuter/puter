 ...
 */
export const privacy_aware_path = (world) =>
  function privacy_aware_path(fspath) {
    // e.g. /my_username/test.txt -> ~/test.txt
    if (fspath.startsWith('~/')) return fspath;
    // e.g. /my_username/test.txt -> ~/test.txt
    else if (
      fspath.startsWith(
        world.window.home_path.endsWith('/') ? world.window.home_path : world.window.home_path + '/'
      )
    )
      return fspath.replace(world.window.home_path, '~');
    // e.g. /other_username/test.txt -> /other_username/test.txt
    else if (fspath.startsWith('/') && !fspath.startsWith(world.window.home_path)) return fspath;
    // e.g. test.txt -> ~/test.txt
    else if (!fspath.startsWith('/')) return '~/' + fspath;
