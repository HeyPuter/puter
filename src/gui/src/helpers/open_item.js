...
Please try recreating the link.`);
  ...
  //----------------------------------------------------------------
    ...
    UIAlert(
      `This item can't be opened because it's in the trash. To use this item, first drag it out of the Trash.`
    );
  }
  //----------------------------------------------------------------
  ...
  //----------------------------------------------------------------
    ...
    try {
      ...
      let $filedialog_parent_app_window = $(
        `.window[data-element_uuid="${filedialog_parent_uid}"]`
      );
      ...
      let res = await puter.fs.sign(window.host_app_uid ?? parent_window_app_uid, {
        uid: uid,
        action: 'write',
      });
      res = res.items;
      ...
      const return_to_parent_window =
        $el_parent_window.attr('data-return_to_parent_window') === 'true';
      if (return_to_parent_window) {
        ...
        const target_iframe = $(`.window[data-element_uuid="${parent_uuid}"]`)
  ...
  //----------------------------------------------------------------
    ...
    UIWindow({
      ...
      height: options.maximized
        ? `calc(100% - ${window.taskbar_height + window.toolbar_height + 1}px)`
        : undefined,
