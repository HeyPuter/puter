...
const item_icon = async (fsentry) => {
  ...
  // --------------------------------------------------
    ...
    if (fsentry.associated_app.icon) return { image: fsentry.associated_app.icon, type: 'icon' };
    else return { image: window.icons['app.svg'], type: 'icon' };
  }
  // --------------------------------------------------
  // Trash
  // --------------------------------------------------
    ...
    let trash_img = $(
      `.item[data-path="${html_encode(window.trash_path)}" i] .item-icon-icon`
    ).attr('src');
    ...
    if (!trash_img) {
      ...
      else trash_img = window.icons['trash-full.svg'];
    }
  ...
  // --------------------------------------------------
    ...
    // regular directories
    else return { image: window.icons['folder.svg'], type: 'icon' };
  }
  // --------------------------------------------------
