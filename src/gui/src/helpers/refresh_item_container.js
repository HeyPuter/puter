...
const refresh_item_container = function (el_item_container, options) {
  ...
  if (options.fadeInItems) $(el_item_container).css('opacity', '0');
  ...
  let loading_timeout = setTimeout(function () {
    ...
    if ($(loading_spinner).closest('.item-container').attr('data-path') !== container_path) return;

    // show the loading spinner
    $(loading_spinner).show();
    setTimeout(function () {
      $(loading_spinner)
        .find('.explorer-loading-spinner-msg')
        .html('Taking a little longer than usual. Please wait...');
    }, 3000);
  }, 1000);
  ...
  puter.fs.stat(container_path, function (fsentry) {
    ...
    // update layout
    if (el_window && el_window.length > 0) window.update_window_layout(el_window, fsentry.layout);
    //
  ...
  puter.fs
    ...
    .then((fsentries) => {
      ...
      if ($(el_item_container).attr('data-path') !== container_path) return;
      ...
      setTimeout(
        async function () {
          ...
          for (let index = 0; index < fsentries.length; index++) {
            ...
            if (is_directoryPicker && !fsentry.is_dir) is_disabled = true;

            // if this item is not allowed because of filetype restrictions, disable it
            if (
              !window.check_fsentry_against_allowed_file_types_string(fsentry, allowed_file_types)
            )
              is_disabled = true;
            ...
            const item_path =
              fsentry.path ?? path.join($(el_window).attr('data-path'), fsentry.name);
            ...
            if (item_path !== window.trash_path && item_path !== window.appdata_path) {
              ...
              fsentry.name =
                metadata && metadata.original_name !== undefined
          ...
          if ($(el_item_container).hasClass('desktop')) {
            try {
              ...
              UIItem({
                ...
                icon: {
                  image: trash.is_empty
                    ? window.icons['trash.svg']
                    : window.icons['trash-full.svg'],
                  type: 'icon',
                },
                name: trash.name,
                is_dir: trash.is_dir,
          ...
          if (options.fadeInItems) $(el_item_container).animate({ opacity: '1' });

          // update footer item count if this is an explorer window
          if (el_window) window.update_explorer_footer_item_count(el_window);
        },
    ...
    .catch((e) => {
      ...
      // show error message
      $(error_message).html(
        'Failed to load directory' + html_encode(e && e.message ? ': ' + e.message : '')
      );
      $(error_message).show();
    });
};
