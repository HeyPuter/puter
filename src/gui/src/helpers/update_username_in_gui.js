...
const update_username_in_gui = function (new_username) {
  ...
  $(':not([data-path=""]),:not([data-item-path=""])').each((i, el) => {
    ...
    if (attr_path && attr_path !== 'null' && attr_path !== 'undefined') {
      // /[username]
      if (attr_path === '/' + window.user.username) $el.attr('data-path', '/' + new_username);
      // /[username]/...
      else if (attr_path.startsWith('/' + window.user.username + '/'))
        $el.attr(
          'data-path',
          attr_path.replace('/' + window.user.username + '/', '/' + new_username + '/')
        );
      ...
      // .window-navbar-path-input value
        ...
        // /[username]
        if (attr_path === '/' + window.user.username) $el.val('/' + new_username);
        // /[username]/...
        else if (attr_path.startsWith('/' + window.user.username + '/'))
    ...
    if (
      ...
      // every other paths
        ...
        $el.attr(
          'data-shortcut_to_path',
          attr_shortcut_to_path.replace('/' + window.user.username + '/', '/' + new_username + '/')
        );
    }
    // data-item-path
    if (attr_item_path && attr_item_path !== 'null' && attr_item_path !== 'undefined') {
      ...
      // /[username]/...
        ...
        $el.attr(
          'data-item-path',
          attr_item_path.replace('/' + window.user.username + '/', '/' + new_username + '/')
        );
    }
