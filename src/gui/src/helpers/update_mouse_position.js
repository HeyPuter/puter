...
const update_mouse_position = function (x, y) {
  ...
  if (
    (window.mouseX < 150 && window.mouseY < window.toolbar_height + 20) ||
    (window.mouseX < 20 && window.mouseY < 150)
  )
  ...
  // mouse in left edge of screen
    ...
    window.mouseX < 20 &&
    window.mouseY >= 150 &&
    window.mouseY < window.desktop_height - 150
  )
  ...
  // mouse in right edge of screen
    ...
    window.mouseX > window.desktop_width - 20 &&
    window.mouseY >= 150 &&
    window.mouseY < window.desktop_height - 150
  )
  ...
  // mouse in top-right corner of screen
    ...
    (window.mouseX > window.desktop_width - 150 && window.mouseY < window.toolbar_height + 20) ||
    (window.mouseX > window.desktop_width - 20 && window.mouseY < 150)
  )
  ...
  // mouse in top edge of screen
    ...
    window.mouseY < window.toolbar_height + 20 &&
    window.mouseX >= 150 &&
    window.mouseX < window.desktop_width - 150
  )
    window.current_active_snap_zone = 'n';
  // not in any snap zone
  else window.current_active_snap_zone = undefined;
  ...
  if (windows.length > 0) {
    ...
    for (let i = 0; i < windows.length; i++) {
      ...
      if (
        window.mouseX > rect.x &&
        window.mouseX < rect.x + rect.width &&
        window.mouseY > rect.y &&
        window.mouseY < rect.y + rect.height
      ) {
