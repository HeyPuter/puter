...
const launch_app = async (options) => {
  ...
  if (options.name === 'explorer') {
    app_info = [];
  } else if (options.app_obj) app_info = options.app_obj;
  else app_info = await puter.apps.get(options.name, { icon_size: 64 });
  ...
  // icon
  //-----------------------------------
  if (app_info.icon) icon = app_info.icon;
  else if (options.name === 'explorer') icon = window.icons['folder.svg'];
  else icon = window.icons['app-icon-' + options.name + '.svg'];

  //-----------------------------------
  // title
  //-----------------------------------
  if (app_info.title) title = app_info.title;
  else if (options.window_title) title = options.window_title;
  else if (options.name) title = options.name;
  ...
  //-----------------------------------
  if (options.file_signature) file_signature = options.file_signature;
  ...
  if (options.name === 'explorer' || options.name === 'trash') {
    ...
    else title = path.dirname(options.path);
    ...
    if (options.args?.path) {
      ...
      if (!fsentry.is_dir) {
        ...
        if (parent === options.args.path) parent = window.home_path;
        options.path = parent;
      } else {
    ...
    else if (options.path === '~') options.path = window.home_path;
  ...
  //------------------------------------
    ...
    if (file_signature) {
      ...
      iframe_url.searchParams.append(
        'puter.item.path',
        privacy_aware_path(options.file_path) || file_signature.path
      );
    ...
    if (window.api_origin) iframe_url.searchParams.append('puter.api_origin', window.api_origin);
    ...
    let window_width;
    if (
      app_info.metadata?.window_size?.width !== undefined &&
      app_info.metadata?.window_size?.width !== ''
    )
      window_width = parseFloat(app_info.metadata.window_size.width);
    if (options.maximized) window_width = '100%';

    // height
    let window_height;
    if (
      app_info.metadata?.window_size?.height !== undefined &&
      app_info.metadata?.window_size?.height !== ''
    ) {
    ...
    let top;
    if (
      app_info.metadata?.window_position?.top !== undefined &&
      app_info.metadata?.window_position?.top !== ''
    )
      top = parseFloat(app_info.metadata.window_position.top) + window.toolbar_height + 1;
    if (options.maximized) top = 0;

    // left
    let left;
    if (
      app_info.metadata?.window_position?.left !== undefined &&
      app_info.metadata?.window_position?.left !== ''
    )
      left = parseFloat(app_info.metadata.window_position.left);
    if (options.maximized) left = 0;

    // window_resizable
    let window_resizable = true;
    if (
      app_info.metadata?.window_resizable !== undefined &&
      typeof app_info.metadata.window_resizable === 'boolean'
    )
    ...
    if (
      app_info.metadata?.hide_titlebar !== undefined &&
      typeof app_info.metadata.hide_titlebar === 'boolean'
    )
    ...
    if (
      app_info.metadata?.credentialless !== undefined &&
      typeof app_info.metadata.credentialless === 'boolean'
    )
    ...
    if (options.name !== 'explorer') {
      ...
      window.launch_apps.recent = window.launch_apps.recent.slice(
        0,
        window.launch_recent_apps_count
      );
