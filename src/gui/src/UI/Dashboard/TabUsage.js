...
async function update_usage_details ($el_window) {
    const monthlyUsagePromise = puter.auth.getMonthlyUsage().then(res => {
        ...
        $('#total-usage').html(window.number_format(budget.used / 100_000_000, { decimals: 2, prefix: '$' }));
        $('#total-capacity').html(window.number_format(budget.capacity / 100_000_000, { decimals: 2, prefix: '$' }));
