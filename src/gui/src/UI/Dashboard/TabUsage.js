...
function renderUsageTable () {
    ...
    for ( const row of rowsToShow ) {
        h += `
        <tr>
            <td title="${html_encode(row.resource)}">${row.resource}</td>
            <td title="${html_encode(row.formattedUnits)}">${row.formattedUnits}</td>
            <td title="${html_encode(row.formattedCost)}">${row.formattedCost}</td>
        </tr>`;
    }
    h += '</tbody>';
    h += '</table>';
