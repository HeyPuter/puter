...
async function UIWindowDesktopBGSettings(options) {
  ...
  return new Promise(async (resolve) => {
    ...
    const default_wallpaper =
      window.gui_env === 'prod' ? '/dist/images/wallpaper.webp' : '/images/wallpaper.webp';
    ...
    $(el_window)
      .find('.desktop-bg-color-block:not(.desktop-bg-color-block-palette')
      .on('click', async function (e) {
        window.set_desktop_background({ color: $(this).attr('data-color') });
      });
    $(el_window)
      .find('.desktop-bg-color-block-palette input')
