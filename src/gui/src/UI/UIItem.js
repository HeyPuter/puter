...
function UIItem(options) {
  ...
  options.multiselectable =
    options.multiselectable === undefined || options.multiselectable === true ? true : false;
  options.shortcut_to = options.shortcut_to ?? '';
  options.shortcut_to_path = options.shortcut_to_path ?? '';
  options.immutable =
    options.immutable === false || options.immutable === 0 || options.immutable === undefined
      ? 0
      : 1;
  options.sort_container_after_append =
    options.sort_container_after_append !== undefined ? options.sort_container_after_append : false;
  const is_shared_with_me =
    options.path !== '/' + window.user.username &&
    !options.path.startsWith('/' + window.user.username + '/');
  ...
  if (
    !window.check_fsentry_against_allowed_file_types_string(
      { is_dir: options.is_dir, name: options.name, type: options.type },
      appendto_allowed_file_types
    )
  )
  ...
  if (options.is_dir) h += `<span>${i18n('folder')}</span>`;
  else h += `<span>${options.type ? html_encode(options.type) : '-'}</span>`;
  h += `</div>`;
  ...
  const is_trashed = $(el_item)
    .attr('data-path')
    .startsWith(window.trash_path + '/');

  // update parent window's explorer item count if applicable
  if (options.appendTo !== undefined) {
    let el_window = options.appendTo;
    if (!$(el_window).hasClass('.window')) el_window = $(el_window).closest('.window');
  ...
  // manual positioning
  if (
    !window.is_auto_arrange_enabled &&
    options.position &&
  ...
  $(el_item).draggable({
    ...
    start: function (event, ui) {
      ...
      $(`.item[data-is_dir="0"][data-associated_app_name=""]:not(.item-selected)`).droppable(
        'disable'
      );
    ...
    drag: function (event, ui) {
      ...
      if (
        Math.abs(ui.originalPosition.top - ui.offset.top) > 5 ||
      ...
      if (
        $(window.mouseover_item_container).closest('.window').is(window.mouseover_window) &&
          ...
          if (
            ...
            $app_iframe
              ...
              .contentWindow.postMessage(
                { msg: 'drag', x: window.mouseX - rect.left, y: window.mouseY - rect.top },
                '*'
              );
          }
        }
    ...
    stop: function (event, ui) {
      ...
      if (
        $(el_item).closest('.item-container').attr('data-path') === window.desktop_path &&
        !window.is_auto_arrange_enabled &&
        $(el_item).attr('data-path') !== window.trash_path &&
        !ui.helper.data('dropped') &&
        // Item must be dropped on the Desktop and not on the taskbar
        window.mouseover_window === undefined &&
        ui.position.top <= window.desktop_height - window.taskbar_height - 15
      ) {
        el_item.style.position = 'absolute';
  ...
  $(el_item).droppable({
    ...
    drop: async function (event, ui) {
      ...
      if (
        $(event.target).closest('.window').attr('data-id') !==
        $(window.mouseover_window).attr('data-id')
      )
        return;
      ...
      //--------------------------------------------------------
          ...
          items_to_move.forEach((item_to_move) => {
            window.create_shortcut(
              ...
              options.is_dir
                ...
                : path.dirname($(el_item).attr('data-path')),
              null,
        ...
        // Otherwise, move items
          ...
          window.move_items(
            items_to_move,
            $(el_item).attr('data-shortcut_to_path') !== ''
              ? $(el_item).attr('data-shortcut_to_path')
              : $(el_item).attr('data-path')
          );
        }
    ...
    over: function (event, ui) {
      ...
      if (
        $event_parent_win.length > 0 &&
        $event_parent_win.attr('data-id') !== $(window.mouseover_window).attr('data-id')
      )
        return;
    ...
    out: function (event, ui) {
      ...
      if (options.is_dir || options.associated_app_name) {
        $(el_item).removeClass('item-selected');
        $('.ui-draggable-dragging .item-name, .item-selected-clone .item-name').css(
          'opacity',
          'initial'
        );
  ...
  if (isMobile.phone || isMobile.tablet) {
    $(el_item).on('click', async function (e) {
      ...
      if ($(el_item).hasClass('item-disabled')) return false;

      if ($(e.target).hasClass('item-name-editor')) return false;
  ...
  $(el_item).on('mousedown', function (e) {
    ...
    if ($(e.target).hasClass('item-has-website-url-badge')) return false;
    ...
    if (
      e.which === 3 &&
      $(el_item).hasClass('item-selected') &&
      $(el_item).siblings('.item-selected').length > 0
    ) {
      $('.context-menu').remove();
      return false;
    }
    ...
    if (
      (!e.ctrlKey && !e.metaKey && !$(this).hasClass('item-selected')) ||
      ($el_parent_window.length > 0 && $el_parent_window.attr('data-multiselectable') !== 'true')
    ) {
  ...
  $(el_item).on('click', function (e) {
    ...
    if ($el_parent_window.attr('data-is_openFileDialog') === 'true') {
      ...
      else $el_parent_window.find('.openfiledialog-open-btn').addClass('disabled');
    }
    //----------------------------------------------------------------
    // On a SaveFileDialog?
    //----------------------------------------------------------------
  ...
  function rename() {
    ...
    if (
      ...
      new_name === '.' ||
      new_name === '..' ||
      !$(el_item_name_editor).hasClass('item-name-editor-active')
    ) {
      if (new_name === '.') {
        UIAlert(
          `The name "." is not allowed, because it is a reserved name. Please choose another name.`
        );
      } else if (new_name === '..') {
        UIAlert(
          `The name ".." is not allowed, because it is a reserved name. Please choose another name.`
  ...
  $(el_item_name_editor).on('keypress', function (e) {
    ...
    if (!$(el_item_name_editor).is(':visible')) return;
  ...
  $(el_item_name).on('click', function (e) {
    if (
      !skip_a_rename_click &&
      e.which !== 3 &&
      $(el_item_name).parent('.item-selected').length > 0
    ) {
      last_mousedown_ts = Date.now();
      setTimeout(() => {
        if (!skip_a_rename_click && Date.now() - last_mousedown_ts > 400) {
          if (!e.ctrlKey && !e.metaKey) window.activate_item_name_editor(el_item);
          last_mousedown_ts = 0;
        } else {
  ...
  $(el_item).bind('contextmenu taphold', async function (event) {
    ...
    if ($(event.target).hasClass('item-has-website-url-badge')) return false;
    ...
    if (event.target === el_item_name_editor) return;

    event.preventDefault();
    let menu_items;
    const $selected_items = $(el_item)
      .closest('.item-container')
      .find('.item-selected')
      .not(el_item)
      .addBack();
        ...
        menu_items.push({
          ...
          onClick: function () {
            $selected_items.each(function () {
              const ell = this;
              let metadata =
                $(ell).attr('data-metadata') === '' ? {} : JSON.parse($(ell).attr('data-metadata'));
        ...
        menu_items.push({
          ...
          onClick: async function () {
            ...
            $selected_items.each(function () {
              const ell = this;
              items.push({
                uid: $(ell).attr('data-uid'),
                path: $(ell).attr('data-path'),
                icon: $(ell).find('.item-icon img').attr('src'),
                name: $(ell).attr('data-name'),
              });
            });
            UIWindowShare(items);
          },
        ...
        menu_items.push({
          ...
          onClick: async function () {
            ...
            if (alert_resp === 'Delete') {
              ...
              if (trash.is_empty) {
                $(
                  `.item[data-path="${html_encode(window.trash_path)}" i], .item[data-shortcut_to_path="${window.trash_path}" i]`
                )
                  .find('.item-icon > img')
                  .attr('src', window.icons['trash.svg']);
                $(`.window[data-path="${html_encode(window.trash_path)}"]`)
        ...
        menu_items.push({
          ...
          onClick: async function () {
            $selected_items.each(function () {
              ...
              if (
                $(this).attr('data-path') &&
                $(this).closest('.item-container').attr('data-path') === window.desktop_path
              ) {
              ...
              window.create_shortcut(
                ...
                $(this).attr('data-shortcut_to') === ''
                  ? $(this).attr('data-uid')
                  : $(this).attr('data-shortcut_to'),
                $(this).attr('data-shortcut_to_path') === ''
                  ? $(this).attr('data-path')
                  : $(this).attr('data-shortcut_to_path')
              );
            });
    ...
    // -------------------------------------------------------
      ...
      const is_trash =
        $(el_item).attr('data-path') === window.trash_path ||
        $(el_item).attr('data-shortcut_to_path') === window.trash_path;
      menu_items = [];
      // -------------------------------------------
      // Open
      // -------------------------------------------
      if (!is_trashed) {
        ...
        // -
        // -------------------------------------------
        if (options.associated_app_name || is_trash) menu_items.push('-');
      }
      // -------------------------------------------
      // Open With
      // -------------------------------------------
      if (
        !is_trashed &&
        !is_trash &&
        (options.associated_app_name === null || options.associated_app_name === undefined)
      ) {
        let items = [];
            ...
            items.push({
              ...
              onClick: async function () {
                ...
                if (
                  ...
                  const alert_resp = await UIAlert({
                    message:
                      `${i18n('change_always_open_with')} ` +
                      html_encode(suggested_app.title) +
                      '?',
      ...
      if ($(el_item).closest('.window-body').length > 0 && options.is_dir) {
        ...
        // -- separator --
        // -------------------------------------------
        if (!is_trash && !is_trashed && options.is_dir) menu_items.push('-');
      }
      // -------------------------------------------
      // Share With…
      // -------------------------------------------
            ...
            UIWindowShare([
              {
                ...
                icon: $(el_item_icon).find('img').attr('src'),
              },
            ]);
          },
        });
      ...
      if (
        ...
        menu_items.push({
          ...
          onClick: async function () {
            ...
            else window.trigger_download([options.path]);
          },
        });
      }
      // -------------------------------------------
      ...
      if (is_trashed) {
        menu_items.push({
          ...
          onClick: async function () {
            let metadata =
              $(el_item).attr('data-metadata') === ''
                ? {}
                : JSON.parse($(el_item).attr('data-metadata'));
...
$(document).on('click', '.item-has-website-badge', async function (e) {
  puter.fs.stat({
    ...
    success: function (fsentry) {
      if (fsentry.subdomains) window.open(fsentry.subdomains[0].address, '_blank');
    },
  });
});
...
window.activate_item_name_editor = function (el_item) {
  ...
  else $(el_item_name_editor).select();
};
