  ...
  function init(opts) {
    ...
    var mouseenterRow = function (e, data) {
      ...
      mouseleaveRow = function (e) {
        ...
        if (
          !$(e.target).hasClass('has-open-context-menu-submenu') &&
    ...
    var activate = function (row, e, data) {
      if (
        mouseLocs[mouseLocs.length - 1]?.x !== undefined &&
        mouseLocs[mouseLocs.length - 1]?.y !== undefined
      ) {
    ...
    $menu.on('mouseenter', function (e, data) {
      if (
        $menu.find('.context-menu-item-active').length === 0 &&
        $menu.find('.has-open-context-menu-submenu').length === 0
      )
        activeRow = null;
    });
    /**
...
function UIContextMenu(options) {
  ...
  if (start_x + menu_width > window.innerWidth) {
    ...
    if (
      options.parent_id &&
      $(`.context-menu[data-element-id="${options.parent_id}"]`).length > 0
    ) {
  ...
  const fade_remove = (item) => {
    $(
      `#context-menu-${menu_id}, .context-menu[data-element-id="${$(item).closest('.context-menu').attr('data-parent-id')}"]`
  ...
  $(document).on(
    'click',
    `#context-menu-${menu_id} > li:not(.context-menu-item-disabled)`,
    function (e) {
      // onClick
      if (
        options.items[$(this).attr('data-action')].onClick &&
        typeof options.items[$(this).attr('data-action')].onClick === 'function'
      ) {
        let event = e;
      ...
      // "action" - onClick without un-clonable pointer event
        ...
        typeof options.items[$(this).attr('data-action')].action === 'function'
      ) {
        options.items[$(this).attr('data-action')].action();
      }
...
$(document).on('mouseenter', '.context-menu', function (e) {
  ...
  $(this)
    .find('.has-open-context-menu-submenu')
    .each(function () {
...
$(document).on('mouseenter', '.context-menu-divider', function (e) {
  // unselect all items
  $(this)
    .siblings('.context-menu-item:not(.has-open-context-menu-submenu)')
