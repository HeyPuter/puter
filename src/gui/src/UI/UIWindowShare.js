...
async function UIWindowShare(items, recipient) {
  return new Promise(async (resolve) => {
    ...
    // 1 item shared
    if (items.length === 1) h += `<img src="${items[0].icon}" style="width:70px; height:70px;">`;
    // 2 items shared
    ...
    if (items.length > 1) h += ` and ${items.length - 1} other item${items.length > 2 ? 's' : ''}`;
    h += `</h2>`;
      ...
      puter.fs
        ...
        .then((fsentry) => {
          ...
          if (!printed_users.includes(fsentry.owner.username)) {
            ...
            else perm_list += fsentry.owner.email ?? fsentry.owner.username;
            perm_list += `</div>`;
    ...
    $(el_window)
      ...
      .on('click', async function (e) {
        ...
        if (is_email(recipient_id)) recipient_email = recipient_id;
        else recipient_username = recipient_id;

        // see if the recipient is already in the list
        let recipient_already_in_list = false;
        $(el_window)
          .find('.item-perm-recipient-card')
          .each(function () {
            if (
              (recipient_username && $(this).data('recipient-username') === recipient_username) ||
              (recipient_email && $(this).data('recipient-email') === recipient_email)
            ) {
        ...
        if ($(el_window).find('.access-type').val() === 'Viewer') access_level = 'read';
        ...
        $.ajax({
          ...
          data: JSON.stringify({
            recipients: [recipient_username || recipient_email],
            shares: [
              {
                $: 'fs-share',
                path: items[0].path,
          ...
          error: function (err) {
            ...
            if (
              err.responseJSON.code === 'user_does_not_exist' ||
              err.responseJSON.code === 'shared_with_self'
            ) {
    ...
    $(el_window)
      .find('.access-recipient')
      .on('input keypress keyup keydown paste', function () {
        if ($(this).val() === '') {
          $(el_window).find('.give-access-btn').prop('disabled', true);
        } else {
          $(el_window).find('.give-access-btn').prop('disabled', false);
...
$(document).on('click', '.remove-permission-link', async function () {
  ...
  $(
    '.item-perm-recipient-card[data-recipient-username="' +
      recipient_username +
      '"][data-permission="' +
      permission +
      '"]'
