    ...
    $.ajax({
      ...
      success: function (res) {
        ...
        res.usages.forEach((entry) => {
          ...
          if (entry.name === 'ai-chat (complete)') return;

          if (entry.name.startsWith('es:subdomain')) return;

          if (entry.name.startsWith('es:app')) return;

          let name = entry.name;

          if (name === 'convert-api (convert)') name = `File Conversions`;
        ...
        const update_usage = (event) => {
          ...
          el_divContent.querySelector(
            `[data-id=${sanitize_id(event.id)}] .usage-progbar`
          ).style.width = '' + Number(event.usage_percentage) + '%';
          el_divContent.querySelector(
            `[data-id=${sanitize_id(event.id)}] .usage-progbar span`
          ...
          el_divContent.querySelector(`[data-id=${sanitize_id(event.id)}] > span`).innerText =
            used_of_str;
        };
    ...
    $.ajax({
      ...
      success: function (res) {
        ...
        $('#storage-used-percent').html(
          usage_percentage +
            '%' +
            (host_usage_percentage > 0 ? ' / ' + host_usage_percentage + '%' : '')
        );
