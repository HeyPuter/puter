  ...
  UITaskbarItem({
    ...
    onClick: async function (item) {
      ...
      if (
        ...
        window.launch_apps.recent.length === 0
      ) {
        // get launch apps
      ...
      if (window.launch_apps.recent.length > 0) {
        ...
        for (
          let index = 0;
          index < window.launch_recent_apps_count && index < window.launch_apps.recent.length;
          index++
        ) {
      ...
      if (!isMobile.phone) $(popover).find('.launch-search').focus();
  ...
  UITaskbarItem({
    ...
    onClick: function () {
      let open_window_count = parseInt(
        $(`.taskbar-item[data-app="explorer"]`).attr('data-open-windows')
      );
  ...
  if (window.user.taskbar_items && window.user.taskbar_items.length > 0) {
    for (let index = 0; index < window.user.taskbar_items.length; index++) {
      ...
      UITaskbarItem({
        ...
        onClick: function () {
          let open_window_count = parseInt(
            $(`.taskbar-item[data-app="${app_info.name}"]`).attr('data-open-windows')
          );
          if (open_window_count === 0) {
            launch_app({
              name: app_info.name,
    ...
    update: function (event, ui) {
      if (!$(ui.item).hasClass('taskbar-item')) {
        ...
        let item = UITaskbarItem({
          ...
          onClick: function () {
            let open_window_count = parseInt(
              $(`.taskbar-item[data-app="${$(ui.item).attr('data-app-name')}"]`).attr(
                'data-open-windows'
              )
            );
            if (open_window_count === 0) {
