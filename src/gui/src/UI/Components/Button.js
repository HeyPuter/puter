  ...
  class Button extends Component {
    ...
    create_template({ template }) {
      if (this.get('style') === 'link') {
        $(template).html(/*html*/ `
                <button type="submit" class="link-button" style="margin-top:10px;" disabled>${html_encode(
                  this.get('label')
                )}</button>
            `);
        return;
      ...
      $(template).html(/*html*/ `
            <button type="submit" class="button ${this.get('style') !== 'small' ? 'button-block' : ''} button-${this.get('style')}" style="margin-top:10px;" disabled>${html_encode(
              this.get('label')
            )}</button>
        `);
    }
