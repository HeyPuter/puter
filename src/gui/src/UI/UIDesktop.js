...
async function UIDesktop(options) {
  ...
  window.socket.on('trash.is_empty', async (msg) => {
    $(`.item[data-path="${html_encode(window.trash_path)}" i]`)
      .find('.item-icon > img')
      .attr('src', msg.is_empty ? window.icons['trash.svg'] : window.icons['trash-full.svg']);
  ...
  window.socket.on('app.opened', async (app) => {
    ...
    if (app.original_client_socket_id === window.socket.id) return;
  ...
  window.socket.on('item.updated', async (item) => {
    ...
    // set new item name
    $(`.item[data-uid='${html_encode(item.uid)}'] .item-name`).html(
      html_encode(truncate_filename(item.name))
    );
    ...
    $(`[data-path="${html_encode(item.old_path)}" i]`).each(function () {
      ...
      if ($(this).hasClass('window-navbar-path-dirname')) $(this).text(item.name);
    });
    ...
    $(`.item[data-uid='${item.uid}']`)
      ...
      .each(function () {
        window.sort_items(
          this,
          $(this).closest('.item-container').attr('data-sort_by'),
          $(this).closest('.item-container').attr('data-sort_order')
        );
      });
  });
  ...
  window.socket.on('item.moved', async (resp) => {
    ...
    $(`.item[data-shortcut_to_path="${html_encode(resp.old_path)}" i]`).attr(
      `data-shortcut_to_path`,
      html_encode(fsentry.path)
    );
    ...
    // update all paths of its and its descendants' open windows
      ...
      // todo this has to be case-insensitive but the `i` selector doesn't work on ^=
      $(
        `.window[data-path^="${html_encode(resp.old_path)}/"], .window[data-path="${html_encode(resp.old_path)}" i]`
      ).each(function () {
        window.update_window_path(
          this,
          $(this).attr('data-path').replace(resp.old_path, fsentry.path)
        );
      });
    }
    ...
    if (fsentry.parent_dirs_created && fsentry.parent_dirs_created.length > 0) {
      ...
      fsentry.parent_dirs_created.forEach(async (dir) => {
        ...
        window.sort_items(
          item_container,
          $(item_container).attr('data-sort_by'),
          $(item_container).attr('data-sort_order')
        );
      });
    }
    //sort each container
  ...
  window.socket.on('user.email_confirmed', (msg) => {
    // don't update if this is the original client that initiated the action
    if (msg.original_client_socket_id === window.socket.id) return;
  ...
  puter.kv.list('user_preferences.default_apps.*').then(async (default_app_keys) => {
    for (let key in default_app_keys) {
      user_preferences[default_app_keys[key].substring(17)] = await puter.kv.get(
        default_app_keys[key]
      );
    }
  ...
  $(el_desktop).droppable({
    ...
    drop: function (event, ui) {
      ...
      if (window.mouseover_window !== undefined) return;
      ...
      if (event.ctrlKey) {
        ...
        if (path.dirname($(ui.draggable).attr('data-path')) === window.trash_path) return;
  ...
  $(el_desktop).bind('contextmenu taphold', function (event) {
    ...
    if ($target.is('input') || $target.is('textarea')) return true;
    ...
    if (event.target === el_desktop) {
      ...
      UIContextMenu({
        items: [
            ...
            items: [
              {
                ...
                onClick: async function () {
                  ...
                  if (window.is_auto_arrange_enabled) {
                    window.sort_items(
                      el_desktop,
                      $(el_desktop).attr('data-sort_by'),
                      $(el_desktop).attr('data-sort_order')
                    );
                    window.set_sort_by(
                      options.desktop_fsentry.uid,
                      $(el_desktop).attr('data-sort_by'),
                      $(el_desktop).attr('data-sort_order')
                    );
          ...
          // -------------------------------------------
          {
            html: window.desktop_icons_hidden
              ? i18n('Show desktop icons')
              : i18n('Hide desktop icons'),
            onClick: function () {
              toggleDesktopIcons();
            },
          },
          ...
          // -------------------------------------------
          {
            ...
            onClick: function () {
              ...
              else if (window.clipboard_op === 'move') window.move_clipboard_items(el_desktop);
            },
          },
          // -------------------------------------------
          // Undo
  ...
  $('body').append(
    `<div class="notification-container"><div class="notifications-close-all">${i18n('close_all')}</div></div>`
  );
  ...
  if (window.url_paths[0]?.toLocaleLowerCase() === 'app' && window.url_paths[1]) {
    ...
    try {
      ...
      window.is_fullpage_mode =
        window.app_launched_from_url.metadata?.fullpage_on_landing ??
        window.is_fullpage_mode ??
        false;
    ...
    for (let [key, value] of window.url_query_params) {
      if (!key.startsWith('puter.')) window.app_query_params[key] = value;
    }
  }
  //--------------------------------------------------------------------------------------
  ...
  $(el_desktop).on('mousedown touchstart', { passive: true }, function (e) {
    ...
    if (e.type === 'taphold' && !isMobile.phone && !isMobile.tablet) return;
  ...
  if (url_paths[0]?.startsWith('@')) {
    ...
    if (!stat.is_dir) {
      ...
      const suggested_apps =
        ...
        (await window.suggest_apps_for_fsentry({
          path: item_path,
        }));
  ...
  $(document).on('mouseenter', '.toolbar', function () {
    if (window.is_fullpage_mode) $('.toolbar').focus();
  });
  ...
  $(document).on('click', function (e) {
    ...
    if (!window.has_seen_toolbar_animation) return;
...
$(document).on('contextmenu taphold', '.toolbar', function (event) {
  ...
  UIContextMenu({
    ...
    items: [
      ...
      //--------------------------------------------------
      {
        html: window.toolbar_auto_hide_enabled
          ? i18n('Disable Auto-hide')
    ...
    users_arr.forEach((l_user) => {
      items.push({
        ...
        onClick: async function (val) {
          ...
          if (l_user.username === window.user.username) return;
          // update auth data
      ...
      //--------------------------------------------------
      {
        ...
        onClick: async function () {
          ...
          if ($('.window-app').length > 0) {
            ...
            if (alert_resp === 'close_and_log_out') window.logout();
          }
          // no open windows
          else window.logout();
        },
...
$(document).on('click', '.user-options-login-btn', async function (e) {
  ...
  if (alert_resp === 'save-session') {
    ...
    if (saved) UIWindowLogin({ show_signup_button: false, reload_on_success: true });
