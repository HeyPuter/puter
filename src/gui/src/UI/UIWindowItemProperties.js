...
async function UIWindowItemProperties(item_name, item_path, item_uid, left, top, width, height) {
  ...
  $(el_window)
    ...
    .click(function (e) {
      ...
      $(el_window)
        .find(`.item-props-tab-content[data-tab="${$(this).attr('data-tab')}"]`)
        .addClass('item-props-tab-content-selected');
    });
  ...
  puter.fs.stat({
    ...
    success: function (fsentry) {
      ...
      // type
      $(el_window)
        .find('.item-prop-val-type')
        .html(fsentry.is_dir ? 'Directory' : fsentry.type === null ? '-' : fsentry.type);
      // size
      $(el_window)
        .find('.item-prop-val-size')
        .html(
          fsentry.size === null || fsentry.size === undefined
            ? '-'
            : window.byte_format(fsentry.size)
        );
      // modified
      $(el_window)
        .find('.item-prop-val-modified')
        .html(fsentry.modified === 0 ? '-' : timeago.format(fsentry.modified * 1000));
      // created
      $(el_window)
        .find('.item-prop-val-created')
        .html(fsentry.created === 0 ? '-' : timeago.format(fsentry.created * 1000));
      // subdomains
      if (fsentry.subdomains && fsentry.subdomains.length > 0) {
        fsentry.subdomains.forEach((subdomain) => {
          $(el_window)
            .find('.item-prop-val-websites')
            .append(
              `<p class="item-prop-website-entry" data-uuid="${html_encode(subdomain.uuid)}" style="margin-bottom:5px; margin-top:5px;"><a target="_blank" href="${html_encode(subdomain.address)}">${html_encode(subdomain.address)}</a> (<span class="disassociate-website-link" data-uuid="${html_encode(subdomain.uuid)}" data-subdomain="${window.extractSubdomain(subdomain.address)}">disassociate</span>)</p>`
            );
        });
      } else {
      ...
      if (fsentry.versions && fsentry.versions.length > 0) {
        fsentry.versions.reverse().forEach((version) => {
          $(el_window)
            .find('.item-props-version-list')
            .append(
              `<div class="item-prop-version-entry">${version.user ? version.user.username : ''} &bull; ${timeago.format(version.timestamp * 1000)}<p style="font-size:10px;">${version.id}</p></div>`
            );
        });
      } else {
      ...
      $(el_window)
        .find(`.disassociate-website-link`)
        .on('click', function (e) {
          puter.hosting.update($(e.target).attr('data-subdomain'), null).then(() => {
            $(el_window)
              .find(`.item-prop-website-entry[data-uuid="${$(e.target).attr('data-uuid')}"]`)
              .remove();
