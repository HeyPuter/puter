...
function UITaskbarItem(options) {
  ...
  if (options.app === 'explorer') icon = window.icons['folders.svg'];
  ...
  // active indicator
  if (options.app !== 'apps') h += `<span class="active-taskbar-indicator"></span>`;
  h += `</div>`;
  ...
  if (options.append_to_taskbar) {
    if (options.before_trash) {
      ...
      if (options.sortable) $('.taskbar-sortable').append(h);
      else {
        ...
        else $('.taskbar').find('.taskbar-sortable').after(h);
      }
    }
  } else {
    $('body').prepend(h);
  ...
  $(el_taskbar_item).on('click', function (e) {
    ...
    if (
      $(el_taskbar_item).attr('data-name') === 'Start' &&
      (isMobile.phone || isMobile.tablet) &&
      $(el_taskbar_item).hasClass('has-open-popover')
    ) {
      $('.popover').remove();
      return;
    }
    ...
    if ($(el_taskbar_item).hasClass('has-open-contextmenu')) return;
  ...
  $(el_taskbar_item).on('contextmenu taphold', function (e) {
    ...
    if (options.disable_context_menu) return;
    ...
    // divider
    // -------------------------------------------
    if (menu_items.length > 0) menu_items.push('-');
    //------------------------------------------
    // New Window
    //------------------------------------------
    ...
    if (open_windows > 0) {
      ...
      menu_items.push({
        ...
        onClick: function () {
          if (open_windows > 0) $(`.window[data-app="${options.app}"]`).hideWindow();
        },
      });
      // -------------------------------------------
      // Close All Windows
