...
async function UIWindowPublishWebsite(target_dir_uid, target_dir_name, target_dir_path) {
  ...
  $(el_window)
    ...
    .on('click', function (e) {
      ...
      puter.hosting
        .create(subdomain, target_dir_path)
        .then((res) => {
          let url = 'https://' + subdomain + '.' + window.hosting_domain + '/';
          $(el_window)
            .find('.window-publishWebsite-form')
            .hide(100, function () {
          ...
          $(`.item[data-path^="${target_dir_path}/"]`).each(function () {
            ...
            $(this).attr(
              'data-website_url',
              url + $(this).attr('data-path').substring(target_dir_path.length)
            );
          });
        ...
        .catch((err) => {
          err = err.error;
          $(el_window)
            .find('.publish-website-error-msg')
            .html(
              err.message +
