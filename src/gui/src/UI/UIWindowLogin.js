...
async function UIWindowLogin(options) {
  ...
  if (options.reload_on_success === undefined) options.reload_on_success = true;
  ...
  return new Promise(async (resolve) => {
    ...
    $(el_window)
      ...
      .on('click', function (e) {
        ...
        if (!email_username) {
          $(el_window)
            .find('.login-error-msg')
            .html(i18n('email_or_username_required') || 'Email or username is required');
        ...
        if (!password) {
          $(el_window)
            .find('.login-error-msg')
            .html(i18n('password_required') || 'Password is required');
          $(el_window).find('.login-error-msg').fadeIn();
          return;
        }
            ...
            if (data.next_step === 'otp') {
              ...
              const otp_option = new Flexer({
                children: [
                  new JustHTML({
                    html: /*html*/ `
                                        <h3 style="text-align:center; font-weight: 500; font-size: 20px;">${i18n(
                                          'login2fa_otp_title'
                                        )}</h3>
                                        <p style="text-align:center; padding: 0 20px;">${i18n(
    ...
    $(el_window)
      ...
      .on('click', function (e) {
        ...
        $(el_window)
          ...
          .attr(
            'src',
            options.show_password ? window.icons['eye-closed.svg'] : window.icons['eye-open.svg']
          );
      });
  });
}
