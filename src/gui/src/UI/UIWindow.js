...
async function UIWindow(options) {
  ...
  if (!options.dominant) window.window_counter++;
  ...
  if (
    (window.is_embedded || window.is_fullpage_mode) &&
    !options.parent_uuid &&
  ...
  options.selectable_body =
    options.selectable_body === undefined || options.selectable_body === true ? true : false;
  ...
  if (!options.dominant && !options.center) {
    options.left =
      options.left ??
      window.innerWidth / 2 - options.width / 2 + ((window.window_counter - 1) % 10) * 30 + 'px';
  ...
  } else options.left = options.left ?? window.innerWidth / 2 - options.width / 2 + 'px';

  // top
  if (!options.dominant && !options.center) {
    options.top =
      options.top ??
      window.innerHeight / 2 - options.height / 2 + ((window.window_counter - 1) % 10) * 30 + 'px';
  } else if (!options.dominant && options.center) {
    options.top = options.top ?? window.innerHeight / 2 - options.height / 2 + 'px';
  } else if (options.dominant) {
    options.top = window.innerHeight * 0.15;
  } else if (isMobile.phone) options.top = 100;
  ...
  // Window
  let zindex = options.stay_on_top
    ? 99999999 + window.last_window_zindex + 1 + ' !important'
    : window.last_window_zindex;
  ...
  if (options.params !== undefined) {
    ...
    if (user_set_url_params.length > 0) user_set_url_params = '?' + user_set_url_params.join('&');
  }
  ...
  if (options.has_head) {
    ...
    // icon
    if (options.icon) h += `<img class="window-head-icon" />`;
    // title
  ...
  if (options.is_dir && !isMobile.phone) {
    ...
    if (!window.sidebar_items) {
      ...
      for (let item of items) {
        let icon;
        if (item.path === window.home_path) icon = window.icons['sidebar-folder-home.svg'];
        else if (item.path === window.docs_path)
          icon = window.icons['sidebar-folder-documents.svg'];
        else if (item.path === window.public_path) icon = window.icons['sidebar-folder-public.svg'];
        ...
        else if (item.path === window.videos_path) icon = window.icons['sidebar-folder-videos.svg'];
        else icon = window.icons['sidebar-folder.svg'];
  ...
  if (
    options.is_dir &&
    !options.is_saveFileDialog &&
    !options.is_openFileDialog &&
    !options.is_directoryPicker
  ) {
  ...
  if (options.is_saveFileDialog) {
    ...
    if (
      options.saveFileDialog_default_filename === undefined ||
      options.saveFileDialog_default_filename === ''
    )
      h += `disabled `;
  ...
  if (options.backdrop) {
    ...
    if (taskbar_zindex === null || taskbar_zindex === undefined) backdrop_zindex = zindex;
    else {
    ...
    if (options.backdrop_covers_toolbar) backdrop_zindex = 999999;
  ...
  if (
    ...
    if ($(`.taskbar-item[data-app="${options.app}"]`).length === 0) {
      UITaskbarItem({
        ...
        before_trash: true,
        onClick: function () {
          let open_window_count = parseInt(
            $(`.taskbar-item[data-app="${options.app}"]`).attr('data-open-windows')
          );
      ...
      if (options.app) {
        $(`.taskbar-item[data-app="${options.app}"]`).attr(
          'data-open-windows',
          parseInt($(`.taskbar-item[data-app="${options.app}"]`).attr('data-open-windows')) + 1
        );
        $(`.taskbar-item[data-app="${options.app}"] .active-taskbar-indicator`).show();
      }
    }
  ...
  const el_window_head_title = document.querySelector(
    `#window-${win_id} > .window-head .window-head-title`
  );
  const el_window_head_icon = document.querySelector(
    `#window-${win_id} > .window-head .window-head-icon`
  );
  const el_window_head_scale_btn = document.querySelector(
    `#window-${win_id} > .window-head > .window-scale-btn`
  );
  const el_window_navbar_back_btn = document.querySelector(
    `#window-${win_id} .window-navbar-btn-back`
  );
  const el_window_navbar_forward_btn = document.querySelector(
    `#window-${win_id} .window-navbar-btn-forward`
  );
  ...
  const el_window_app_iframe = document.querySelector(
    `#window-${win_id} > .window-body > .window-app-iframe`
  );
  const el_savefiledialog_filename = document.querySelector(
    `#window-${win_id} .savefiledialog-filename`
  );
  const el_savefiledialog_save_btn = document.querySelector(
    `#window-${win_id} .savefiledialog-save-btn`
  );
  const el_filedialog_cancel_btn = document.querySelector(
    `#window-${win_id} .filedialog-cancel-btn`
  );
  const el_openfiledialog_open_btn = document.querySelector(
    `#window-${win_id} .openfiledialog-open-btn`
  );
  const el_directorypicker_select_btn = document.querySelector(
    `#window-${win_id} .directorypicker-select-btn`
  );
  const el_window_filedialog_upload_here = document.querySelector(
    `#window-${win_id} .window-filedialog-upload-here`
  );
  ...
  if (options.is_maximized) {
    ...
    $(el_window).attr({
      'data-left-before-maxim':
        window.innerWidth / 2 - 680 / 2 + ((window.window_counter - 1) % 10) * 30 + 'px',
  ...
  // set icon
  if (options.icon) $(el_window_head_icon).attr('src', options.icon.image ?? options.icon);

  // root folder of a shared user?
  if (
    options.is_dir &&
    options.path.split('/').length - 1 === 1 &&
    options.path !== '/' + window.user.username
  ) {
        ...
        if (options.is_visible) {
          $(el_window).show(0, function (e) {
            ...
            if (options.is_saveFileDialog) {
              ...
              else $(el_savefiledialog_filename).select();
  ...
  if (options.is_saveFileDialog) {
    ...
    $(el_savefiledialog_filename).bind('keydown change input paste', function () {
      if ($(this).val() !== '') $(el_savefiledialog_save_btn).removeClass('disabled');
      else $(el_savefiledialog_save_btn).addClass('disabled');
    });
    $(el_savefiledialog_filename).get(0).focus({ preventScroll: true });
  }
  ...
  if (options.is_openFileDialog) {
    ...
    $(el_openfiledialog_open_btn).on('click', async function (e) {
      ...
      // No item selected
      if (selected_els.length === 0) return;
      // ------------------------------------------------
      // Item(s) selected
      // ------------------------------------------------
      else {
        ...
        for (let i = 0; i < selected_els.length; i++)
          items_to_sign.push({
            uid: $(selected_els[i]).attr('data-uid'),
            action: 'write',
            path: $(selected_els[i]).attr('data-path'),
          });
      ...
      if (options.return_to_parent_window) {
        ...
        const target_iframe = $(`.window[data-element_uuid="${options.parent_uuid}"]`)
          .find('.window-app-iframe')
          .get(0);
        if (target_iframe) {
        ...
        const file_opened_event = new CustomEvent('file_opened', {
          detail: Array.isArray(selected_files) ? [...selected_files] : [selected_files],
        });

        // dispatch event to parent window
        $(`.window[data-element_uuid="${options.parent_uuid}"]`)
          .get(0)
          ?.dispatchEvent(file_opened_event);
    ...
    $(el_directorypicker_select_btn).on('click', async function (e) {
      ...
      if (selected_els.length === 0) {
        selected_dirs = await puter.fs.sign(options.initiating_app_uuid, {
  ...
  if (options.is_dir) {
    ...
    $(el_window_navbar_back_btn).on('click', function (e) {
      ...
      if ($(el_window_navbar_back_btn).hasClass('has-open-contextmenu')) return;
      // if ctrl/cmd are pressed, open in new window
      if (e.ctrlKey || e.metaKey) {
        const dirpath = window.window_nav_history[win_id].at(
          window.window_nav_history_current_position[win_id] - 1
        );
        UIWindow({
          path: dirpath,
      ...
      // ... otherwise, open in same window
        ...
        const new_path = window.window_nav_history[win_id].at(
          window.window_nav_history_current_position[win_id]
        );
        // update window path
    ...
    $(el_window_navbar_back_btn).on('taphold', function () {
      ...
      for (
        let index = window.window_nav_history_current_position[win_id] - 1;
        index >= 0;
        index--
      ) {
    ...
    $(el_window_navbar_forward_btn).on('click', function (e) {
      // if history menu is open don't continue
      if ($(el_window_navbar_forward_btn).hasClass('has-open-contextmenu')) return;
      // if ctrl/cmd are pressed, open in new window
      if (e.ctrlKey || e.metaKey) {
        const dirpath = window.window_nav_history[win_id].at(
          window.window_nav_history_current_position[win_id] + 1
      ...
      // ... otherwise, open in same window
        ...
        const target_path = window.window_nav_history[win_id].at(
          window.window_nav_history_current_position[win_id]
        );
        // update window path
    ...
    $(el_window_navbar_forward_btn).on('taphold', function () {
      ...
      for (
        let index = window.window_nav_history_current_position[win_id] + 1;
        index < window.window_nav_history[win_id].length;
        index++
      ) {
        const history_item = window.window_nav_history[win_id].at(index);
    ...
    $(el_window_navbar_up_btn).on('click', function (e) {
      ...
      // ... otherwise, open in same window
        ...
        // update history
        window.window_nav_history[win_id] = window.window_nav_history[win_id].slice(
          0,
          window.window_nav_history_current_position[win_id] + 1
        );
    ...
    $(el_window)
      .find('.window-navbar-layout-settings')
      .on('contextmenu taphold', function () {
    ...
    // --------------------------------------------------------
    //auth
    if (!window.is_auth() && !(await UIWindowLogin())) return;
        ...
        .then(function (shared_users) {
          ...
          if (shared_users && shared_users.length - 1 > 0) {
            ...
            for (let index = 0; index < shared_users.length; index++) {
              ...
              if (shared_user.name === window.user.username) continue;
          ...
          $(el_window)
            .find('.window-sidebar-item:not(.ui-droppable)')
            .droppable({
              accept: '.item',
              tolerance: 'pointer',
              drop: function (event, ui) {
                ...
                for (let i = 0; i < cloned_items.length; i++) {
                  const source_item = document.getElementById(
                    'item-' + $(cloned_items[i]).attr('data-id')
                  );
                ...
                if (event.altKey) {
                  items_to_share.forEach((item_to_move) => {
                    window.create_shortcut(
                      ...
                      $(this).attr('data-path'),
                      null,
                      $(item_to_move).attr('data-shortcut_to') === ''
                        ? $(item_to_move).attr('data-uid')
                        : $(item_to_move).attr('data-shortcut_to'),
                      $(item_to_move).attr('data-shortcut_to_path') === ''
                        ? $(item_to_move).attr('data-path')
                        : $(item_to_move).attr('data-shortcut_to_path')
                    );
                  });
                }
              ...
              over: function (event, ui) {
                ...
                if (!$(ui.draggable).hasClass('item')) return;
  ...
  $(el_window_body).on('mousedown', function (e) {
    if ($(e.target).hasClass('window-body') && !e.ctrlKey && !e.metaKey) {
      ...
      if (options.is_openFileDialog) $(el_openfiledialog_open_btn).addClass('disabled');
    }
  });
      ...
      .on(
        ...
        ({
          ...
          for (const el of removed) {
            ...
            if (selected_ctrl_items.includes(el)) $(el).addClass('item-selected');
          }
          ...
          else $(el_openfiledialog_open_btn).addClass('disabled');
        }
      )
      .on('stop', ({ store, event }) => {
        // If this is openFileDialog, enable/disable the Open button accordingly
  ...
  $(el_window_body).droppable({
    ...
    drop: async function (e, ui) {
      ...
      if ($(window.mouseover_window).attr('data-id') !== $(el_window).attr('data-id')) return;
      ...
      if (el_window_app_iframe !== null) {
        ...
        for (let i = 0; i < cloned_items.length; i++) {
          ...
          if (source_item !== null) items_to_move.push(source_item);
        }
        ...
        el_window_app_iframe.contentWindow.postMessage(
          { msg: 'drop', x: window.mouseX - rect.left, y: window.mouseY - rect.top, items: items },
          '*'
        );
      ...
      if (
        $(ui.draggable).closest('.item-container').attr('data-path') ===
      ...
      stop: function () {
        ...
        if (options.is_resizable && snap_placeholder_active && !window_is_snapped) {
          ...
          setTimeout(function () {
            ...
            if ($(el_window).width() < window.window_width_threshold_for_sidebar) {
              if (
                width_before_snap >= window.window_width_threshold_for_sidebar &&
                !sidebar_hidden
              ) {
                $(el_window_sidebar).hide();
              }
        ...
        if (
          $(el_window).position().top > window.innerHeight - window.taskbar_height - 30 &&
          !window_will_snap
        ) {
          $(el_window).animate(
            {
    ...
    $(el_window).resizable({
      ...
      resize: function (e, ui) {
        ...
        if (ui.size.width < window.window_width_threshold_for_sidebar) {
          if (
            ui.originalSize.width >= window.window_width_threshold_for_sidebar &&
            !sidebar_hidden
          ) {
            $(el_window_sidebar).hide();
          }
      ...
      stop: function () {
        ...
        $(el_window_app_iframe).css('pointer-events', 'all');
        $('.window').css('pointer-events', 'initial');
        $(el_window_sidebar).resizable(
          'option',
          'maxWidth',
          el_window.getBoundingClientRect().width / 2
        );
        $(el_window).attr({
  ...
  $(el_window_head).bind('contextmenu taphold', function (event) {
    ...
    if (event.type === 'taphold' && !isMobile.phone && !isMobile.tablet) return;

    const $target = $(event.target);

    // Cases in which native ctx menu should be preserved
    if (
      options.allow_native_ctxmenu ||
      $target.hasClass('allow-native-ctxmenu') ||
      $target.is('input') ||
      $target.is('textarea')
    )
      return true;
        ...
        UIContextMenu({
          ...
          items: [
              ...
              items: [
                {
                  ...
                  onClick: async function () {
                    ...
                    window.set_sort_by(
                      ...
                      'name',
                      $(el_window).attr('data-sort_order')
                    );
                  },
                },
                {
            ...
            // -------------------------------------------
            {
              html: i18n('paste'),
              disabled:
                window.clipboard.length === 0 || $(el_window).attr('data-path') === '/'
                  ? true
                  : false,
              onClick: function () {
            ...
            // -------------------------------------------
            {
              ...
              onClick: function () {
                window.init_upload_using_dialog(
                  el_window_body,
            ...
            // -------------------------------------------
            {
              ...
              onClick: function () {
                ...
                left -= 200;
                left =
                  left > window.innerWidth - window_width ? window.innerWidth - window_width : left;

                let top = window.mouseY;
                top =
                  top >
                  window.innerHeight -
                    (window_height + window.taskbar_height + window.toolbar_height)
                    ? window.innerHeight -
                      (window_height + window.taskbar_height + window.toolbar_height)
                    : top;
...
function delete_window_element(el_window) {
  ...
  if ($('.window').length === 0) window.window_counter = 0;
}
...
$(document).on('click', '.window-sidebar-item', async function (e) {
  ...
  // update window path only if it's a new path AND no ctrl/cmd key pressed
    ...
    window.window_nav_history[parent_win_id] = window.window_nav_history[parent_win_id].slice(
      0,
      window.window_nav_history_current_position[parent_win_id] + 1
    );
...
$(document).on('click', '.window-navbar-path', function (e) {
  if (!$(e.target).hasClass('window-navbar-path')) return;
...
$(document).on('click', '.window-navbar-path-dirname', function (e) {
  ...
  // only change dir if target is not the same as current path
    ...
    window.window_nav_history_current_position[parent_win_id] =
      window.window_nav_history[parent_win_id].length - 1;
...
$(document).on('contextmenu taphold', '.window-navbar', function (event) {
  ...
  if ($(event.target).hasClass('window-navbar-path-input')) return;
    ...
    onClick: function () {
      UIWindow({
        ...
        title:
          $(el).attr('data-path') === '/'
            ? window.root_dirname
            : path.basename($(el).attr('data-path')),
        icon: window.icons['folder.svg'],
...
window.navbar_path_droppable = (el_window) => {
  $(el_window)
    .find('.window-navbar-path-dirname')
    .droppable({
      accept: '.item',
      tolerance: 'pointer',
      drop: function (event, ui) {
...
window.navbar_path = (abs_path) => {
  ...
  if (abs_path.endsWith('/') && abs_path !== '/') abs_path = abs_path.slice(0, -1);
...
window.update_window_path = async function (el_window, target_path) {
  ...
  const is_dir =
    $(el_window).attr('data-is_dir') === '1' || $(el_window).attr('data-is_dir') === 'true';
  ...
  $(el_window)
    .find(`.window-sidebar-item[data-path="${html_encode(target_path)}"]`)
    .addClass('window-sidebar-item-active');
  ...
  if (is_dir) {
    ...
    if (
      window.window_nav_history[win_id] &&
      window.window_nav_history[win_id].length - 1 ===
        window.window_nav_history_current_position[win_id]
    )
      $(el_window_navbar_forward_btn).addClass('window-navbar-btn-disabled');
    // ... else, enable forawrd btn
    else $(el_window_navbar_forward_btn).removeClass('window-navbar-btn-disabled');
    ...
    else $(el_window_navbar_back_btn).removeClass('window-navbar-btn-disabled');

    // disabled Up button if this is root
    if (target_path === '/') $(el_window_navbar_up_btn).addClass('window-navbar-btn-disabled');
    // ... enable back btn in all other cases
    else $(el_window_navbar_up_btn).removeClass('window-navbar-btn-disabled');

    $(el_window_item_container).attr('data-path', target_path);
    $(el_window)
      .find('.window-navbar-path')
      .html(window.navbar_path(target_path, window.user.username));
    ...
    else $(el_window).find('.window-head-icon').attr('src', window.icons['folder.svg']);
  }
    ...
    try {
      puter.fs.stat(target_path, function (fsentry) {
        ...
        if (target_path === window.home_path)
          $(el_window).find('.window-head-title').text(i18n('home'));
        else $(el_window).find('.window-head-title').text(fsentry.name);
        // data-name
...
$.fn.close = async function (options) {
  ...
  $(this).each(async function () {
    ...
    if ($(this).hasClass('window')) {
      ...
      // taskbar update
      let open_window_count = parseInt(
        $(`.taskbar-item[data-app="${$(this).attr('data-app')}"]`).attr('data-open-windows')
      );
      // update open window count of corresponding taskbar item
      if (open_window_count > 0) {
        $(`.taskbar-item[data-app="${$(this).attr('data-app')}"]`).attr(
          'data-open-windows',
          open_window_count - 1
        );
      }
      ...
      if (open_window_count === 1) {
        ...
        window.remove_taskbar_item(
          $(`.taskbar-item[data-app="${$(this).attr('data-app')}"][data-keep-in-taskbar="false"]`)
        );
      }
      ...
      // FileDialog closed
      if (
        $(this).hasClass('window-filedialog') ||
        $(this).attr('data-disable_parent_window') === 'true'
      ) {
        // re-enable this FileDialog's parent window
        $(`.window[data-element_uuid="${$(this).attr('data-parent_uuid')}"]`).addClass(
          'window-active'
        );
        $(`.window[data-element_uuid="${$(this).attr('data-parent_uuid')}"]`).removeClass(
          'window-disabled'
        );
        $(`.window[data-element_uuid="${$(this).attr('data-parent_uuid')}"]`)
          .find('.window-disable-mask')
          .hide();
      ...
      // Other types of windows closed
        ...
        if (!_.isEmpty(window.window_stack)) {
          const $last_window_in_stack = $(
            `.window[data-id="${window.window_stack[window.window_stack.length - 1]}"]`
          );
          // check if previous window is not minimized
          if (
            $last_window_in_stack !== null &&
            $last_window_in_stack.attr('data-is_minimized') !== '1' &&
            $last_window_in_stack.attr('data-is_minimized') !== 'true'
          ) {
            $(
              `.window[data-id="${window.window_stack[window.window_stack.length - 1]}"]`
            ).focusWindow();
          ...
          // if it's explore
          if (
            $last_window_in_stack.attr('data-app') &&
            $last_window_in_stack.attr('data-app').toLowerCase() === 'explorer'
          ) {
            window.history.replaceState(null, document.title, '/');
            document.title = 'Puter';
          }
...
window.update_explorer_footer_item_count = function (el_window) {
  ...
  $(el_window)
    .find('.explorer-footer .explorer-footer-item-count')
    .html(
      item_count +
        ` ${i18n('item')}` +
...
window.update_explorer_footer_selected_items_count = function (el_window) {
  ...
  if (item_count > 0) {
    ...
    $(el_window)
      .find('.explorer-footer .explorer-footer-selected-items-count')
      .html(
        item_count +
          ` ${i18n('item')}` +
          (item_count == 0 || item_count > 1 ? `${i18n('plural_suffix')}` : '') +
...
window.update_window_layout = function (el_window, layout) {
  ...
  if (layout === 'icons') {
    ...
    $(el_window)
      .find('.window-navbar-layout-settings')
      .attr('src', window.icons['layout-icons.svg']);
    ...
    $(el_window)
      .find('.window-navbar-layout-settings')
      .attr('src', window.icons['layout-list.svg']);
    $(el_window).attr('data-layout', layout);
    ...
    $(el_window)
      .find('.window-navbar-layout-settings')
      .attr('src', window.icons['layout-details.svg']);
    $(el_window).attr('data-layout', layout);
  }
};
...
window.toggle_empty_folder_message = function (el_item_container) {
  ...
  if ($(el_item_container).hasClass('desktop')) return;
...
$.fn.focusWindow = function (event) {
  if (this.hasClass('window')) {
    ...
    if ($(this).attr('data-parent_uuid') !== 'null') {
      $(`.window[data-element_uuid="${$(this).attr('data-parent_uuid')}"]`).css(
        'z-index',
        window.last_window_zindex
      );
    }
    ...
    if ($(this).attr('data-element_uuid') !== 'null') {
      $(`.window[data-parent_uuid="${$(this).attr('data-element_uuid')}"]`).css(
        'z-index',
        ++window.last_window_zindex
      );
    }
    ...
    if (
      $(event?.target).hasClass('window-menubar') ||
      $(event?.target).closest('.window-menubar').length > 0
    ) {
      ...
      if (
        ...
        $app_iframe
          ...
          .contentWindow.postMessage(
            { msg: 'click', x: window.mouseX - rect.left, y: window.mouseY - rect.top },
            '*'
          );
      }
    }
    ...
    $('.item-container')
      .not(window.active_item_container)
      .find('.item-selected')
      .addClass('item-blurred');
    // update window-stack
    ...
    if (update_window_url === 'true' || update_window_url === null) {
      window.history.replaceState(
        ...
        '',
        '/app/' + url_app_name + $(this).attr('data-user_set_url_params')
      );
      document.title = $(this).attr('data-name');
    }
    $(`.taskbar .taskbar-item[data-app="${$(this).attr('data-app')}"]`).addClass(
      'taskbar-item-active'
    );
  } else {
...
$.fn.hideWindow = async function (options) {
  $(this).each(async function () {
    if ($(this).hasClass('window')) {
      ...
      let taskbar_item_pos = $(
        `.taskbar .taskbar-item[data-app="${$(this).attr('data-app')}"]`
      ).position();

      // taskbar position is center of window minus half of taskbar item width
      taskbar_item_pos.left =
        taskbar_item_pos.left + $(window).width() / 2 - $(`.taskbar`).width() / 2;
...
$(document).on('click', '.explore-table-headers-th', function (e) {
  ...
  $(e.target)
    .closest('.window')
    .find('.explore-table-headers-th')
    .removeClass('explore-table-headers-th-active');
  ...
  $(e.target)
    .closest('.window')
    .find('.explore-table-headers-th-active > .header-sort-icon')
    .html(sort_icon);
...
document.addEventListener(
  ...
  function (event) {
    if (
      $(event.target).hasClass('window-app') ||
      $(event.target).hasClass('window-app-iframe') ||
      $(event.target?.activeElement).hasClass('window-app-iframe')
    ) {
      setTimeout(function () {
