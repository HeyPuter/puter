...
function UIPopover(options) {
  ...
  if (options.parent_element && $(options.parent_element).hasClass('has-open-popover')) return;
  ...
  function position_popover() {
    ...
    if (options.center_horizontally) {
      ...
      else x_pos = options.left ?? $(options.snapToElement).offset().left + 5;
    }
    ...
    if (options.center_horizontally) {
      ...
      y_pos =
        options.top ??
        $(options.snapToElement).offset().top + $(options.snapToElement).height() + 5;
    }
