...
window.initgui = async function (options) {
  ...
  //--------------------------------------------------------------------------------------
  if (
    window.url_query_params.has('embedded_in_popup') &&
    (window.url_query_params.get('embedded_in_popup') === 'true' ||
      window.url_query_params.get('embedded_in_popup') === '1')
  ) {
  ...
  if (window.embedded_in_popup && window.openerOrigin) {
    ...
    if (
      ...
      (!window.userAppToken || window.url_query_params.get('request_auth'))
    ) {
      picked_a_user_for_sdk_login = await UIWindowSessionList({
        reload_on_success: false,
        draggable_body: false,
  ...
  const bad_session_logout = async () => {
    try {
      ...
      // TODO: i18n
      await UIAlert({
        message: 'Session is invalid and logout failed; ' + 'please clear local storage manually.',
      });
    }
  };
  ...
  // -------------------------------------------------------------------------------------
    ...
    if (window.embedded_in_popup) {
      puter.ui.showSpinner(
        'Setting up your <a href="https://puter.com" target="_blank">Puter</a> account for secure AI and Cloud features...'
      );
    }
  ...
  if (window.feature_flags.prompt_user_when_navigation_away_from_puter) {
    window.onbeforeunload = function () {
      if ($(`.window:not(.window[data-app="explorer"])`).length > 0) return true;
    };
  }
