  ...
  if (e.which === 37 || e.which === 38 || e.which === 39 || e.which === 40) {
      ...
      // if search input is focused and left or right arrow is pressed, return false
        ...
        if (e.which === 38) {
          ...
          if (selected_item_row === 0) {
            ...
            // unselect all items
            $('.launch-popover .start-app-card.launch-app-selected').removeClass(
              'launch-app-selected'
            );
          ...
          // if this item is not in the first row, move the selection up
            ...
            if (new_selected_item_row < 0) new_selected_item_row = selected_item_row_count - 1;
          }
        }
        // if down arrow is pressed
        else if (e.which === 40) {
          new_selected_item_row = selected_item_row + 1;
          if (new_selected_item_row >= selected_item_row_count) new_selected_item_row = 0;
        }
        // if left arrow is pressed
        else if (e.which === 37) {
          new_selected_item_col = selected_item_col - 1;
          if (new_selected_item_col < 0) new_selected_item_col = selected_item_col_count - 1;
        }
        // if right arrow is pressed
        else if (e.which === 39) {
          new_selected_item_col = selected_item_col + 1;
          if (new_selected_item_col >= selected_item_col_count) new_selected_item_col = 0;
        }
        new_selected_item_index =
          new_selected_item_row * selected_item_col_count + new_selected_item_col;
    ...
    // ----------------------------------------------
      ...
      // if an item is selected and down arrow is pressed, select the next enabled item
        ...
        let new_selected_item = $('.context-menu-active .context-menu-item').get(
          new_selected_item_index
        );
        while (
          $(new_selected_item).hasClass('context-menu-item-disabled') ||
          $(new_selected_item).hasClass('context-menu-divider')
        ) {
          new_selected_item_index = new_selected_item_index + 1;
          new_selected_item = $('.context-menu-active .context-menu-item').get(
            new_selected_item_index
          );
        }
      ...
      // if right arrow is pressed, open the submenu by triggering a mouseover event
        ...
        if ($(selected_item).hasClass('context-menu-item-submenu') === true) {
          ...
          window.select_ctxmenu_item(
            $('.context-menu[data-is-submenu="true"] .context-menu-item').get(0)
          );
        }
        return false;
      }
    ...
    // ----------------------------------------------
      ...
      function findNeighbor(current, direction) {
        ...
        candidates = candidates.filter((i) => {
          ...
          if (isVertical) {
            return (
              Math.abs(i.left - ox) < axisThreshold &&
              (direction === 'up' ? irect.top < oy : irect.top > oy)
            );
          } else {
            return (
              Math.abs(i.top - oy) < axisThreshold &&
        ...
        if (candidates.length === 0) {
          ...
          if (isVertical) {
            ...
            candidates.sort((a, b) => (direction === 'up' ? b.top - a.top : a.top - b.top));
          } else {
            candidates = candidates.filter((i) => Math.abs(i.top - oy) < axisThreshold);
            candidates.sort((a, b) => (direction === 'left' ? b.left - a.left : a.left - b.left));
          }
          return candidates[0];
        }
    ...
    // ----------------------------------------------
      ...
      let selected_item_index = selected_item
        ? $('.window-search .search-result').index(selected_item)
        : -1;
      let new_selected_item_index = selected_item_index;
      let new_selected_item;
...
$(document).bind('keydown', async function (e) {
  ...
  if ((e.keyCode === 46 && e.shiftKey) || (e.altKey && e.metaKey && e.keyCode === 8)) {
    let $selected_items = $(window.active_element)
  ...
  if (e.keyCode === 46 || (e.keyCode === 8 && (e.ctrlKey || e.metaKey))) {
    ...
    let $selected_items = $(window.active_element)
      .closest(`.item-container`)
      .find(`.item-selected[data-path^="${window.trash_path + '/'}"]`);
    if ($selected_items.length > 0) {
      const alert_resp = await UIAlert({
        message: i18n('confirm_delete_multiple_items'),
        buttons: [
  ...
  if (
    ...
    let matches = [];
    const selected_items = $(window.active_item_container)
      .find(`.item-selected`)
      .not('.item-disabled')
      .first();
    ...
    if (
      ...
      $(selected_items).attr('data-name').toLowerCase().startsWith(window.keypress_item_seach_term)
    ) {
      return false;
    }
    ...
    for (let j = 0; j < haystack.length; j++) {
      if (
        $(haystack[j]).attr('data-name').toLowerCase().startsWith(window.keypress_item_seach_term)
      ) {
    ...
    if (matches.length > 0) {
      ...
      window.update_explorer_footer_selected_items_count(
        $(window.active_element).closest('.window')
      );
    }
  ...
  //-----------------------------------------------------------------------
    ...
    if (
      selected_items.length === 1 &&
      $(selected_items).text().toLowerCase().startsWith(window.keypress_item_seach_term)
    ) {
      return false;
    }
...
$(document).bind('keyup keydown', async function (e) {
  ...
  if (
    ...
    if ($parent_container.attr('data-multiselectable') === 'false') return false;
  ...
  if (e.ctrlKey && e.which === 87) {
    ...
    if ($parent_window.length === 0) $parent_window = $(window.active_element).find('.window');
  ...
  if (
    ...
    !$(focused_el).is('input') &&
    !$(focused_el).is('textarea') &&
    Date.now() - window.last_enter_pressed_to_rename_ts > 200 &&
    // prevent firing twice, because this will be fired on both keyup and keydown
    e.type === 'keydown'
  ) {
    let $selected_items;
  ...
  if (
    ...
    if (window.clipboard.length === 0) return;
    ...
    if (parent_container) {
      ...
      if (
        (target_path === window.trash_path || target_path.startsWith(window.trash_path + '/')) &&
        window.clipboard_op !== 'move'
      )
        return;
      // execute clipboard operation
      if (window.clipboard_op === 'copy') window.copy_clipboard_items(target_path);
      else if (window.clipboard_op === 'move') window.move_clipboard_items(target_el, target_path);
    }
    return false;
  }
  //-----------------------------------------------------------------------------
