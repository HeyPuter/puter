...
const ipc_listener = async (event, handled) => {
  ...
  if (app_env !== 'app') return handled.resolve(false);

  // --------------------------------------------------------
  // A response to a GUI message received from the app.
  // --------------------------------------------------------
  if (
    typeof event.data.original_msg_id !== 'undefined' &&
    typeof window.appCallbackFunctions[event.data.original_msg_id] !== 'undefined'
  ) {
    // Execute callback
  ...
  //--------------------------------------------------------
    ...
    // Auth
    if (!window.is_auth() && !(await UIWindowSignup({ referrer: app_name }))) return;
  ...
  //--------------------------------------------------------
    ...
    // app window
    else el_window = window.window_for_app_instance(event.data.appInstanceID);

    // window not found
    if (!el_window || el_window.length === 0) return;
    ...
    const sanitize_items = (items) => {
      return items.map((item) => {
        ...
        return {
          ...
          icon: item.icon
            ? `<img style="width: 15px; height: 15px; position: absolute; top: 4px; left: 6px;" src="${html_encode(item.icon)}" />`
            : undefined,
          icon_active: item.icon_active
            ? `<img style="width: 15px; height: 15px; position: absolute; top: 4px; left: 6px;" src="${html_encode(item.icon_active)}" />`
            : undefined,
  ...
  // --------------------------------------------------------
    ...
    set_menu_item_prop(
      window.menubars[event.data.appInstanceID],
      event.data.value.id,
      'disabled',
      true
    );
  ...
  //--------------------------------------------------------
    ...
    if (event.data.width < 200) event.data.width = 200;
    // set window width
  ...
  //--------------------------------------------------------
    ...
    // must be at least 200
    if (event.data.height < 200) event.data.height = 200;
  ...
  //--------------------------------------------------------
    ...
    event.data.msg === 'setWindowSize' &&
    (event.data.width !== undefined || event.data.height !== undefined)
  ) {
    let el_window;
    // specific window
    if (event.data.window_id)
  ...
  //--------------------------------------------------------
    ...
    event.data.msg === 'setWindowPosition' &&
    ...
    if (event.data.x !== undefined) {
      ...
      if (event.data.x < 0) event.data.x = 0;
      // we don't want the window to go off the right edge of the screen
      if (event.data.x > window.innerWidth - 100) event.data.x = window.innerWidth - 100;
      // set window left
    ...
    if (event.data.y !== undefined) {
      ...
      if (event.data.y < window.taskbar_height) event.data.y = window.taskbar_height;
      // we don't want the window to go off the bottom edge of the screen
      if (event.data.y > window.innerHeight - 100) event.data.y = window.innerHeight - 100;
      // set window top
  ...
  //--------------------------------------------------------
    ...
    if (!window.watchItems[event.data.item_uid]) window.watchItems[event.data.item_uid] = [];
  ...
  //--------------------------------------------------------
    ...
    // No options?
    if (!event.data.options) event.data.options = {};
  ...
  //--------------------------------------------------------
    ...
    const tell_caller_and_update_views = async ({ target_path, el_filedialog_window, res }) => {
      ...
      // sort each window
      $(`.item-container[data-path="${html_encode(path.dirname(target_path))}" i]`).each(
        function () {
    ...
    const handle_move_save = async ({ source_path, target_path, el_filedialog_window }) => {
      ...
      if (!stat_info.appdata_app) {
        await puter.ui.alert(
          `the app ${app_uuid} attempted to ` + `move data owned by the user illegaly`
        );
        return;
      }
      ...
      if (stat_info.appdata_app !== app_uuid) {
        await puter.ui.alert(
          `the app ${app_uuid} attempted to ` + `move data owned by ${stat_info.appdata_app}`
        );
        return;
      }
  ...
  //--------------------------------------------------------
    ...
    event.data.msg === 'saveToPictures' ||
    event.data.msg === 'saveToDesktop' ||
    event.data.msg === 'saveToAppData' ||
    event.data.msg === 'saveToDocuments' ||
    event.data.msg === 'saveToVideos' ||
    event.data.msg === 'saveToAudio'
  ) {
    let target_path;
    let create_missing_ancestors = false;

    console.warn(
      `The method ${event.data.msg} is deprecated - see docs.puter.com for more information.`
    );
    event.data.filename = path.normalize(event.data.filename).replace(/(\.+\/|\.+\\)/g, '');
