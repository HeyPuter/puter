...
const commandIcon =
  '<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" class="lucide lucide-command-icon lucide-command"><path d="M15 6v12a3 3 0 1 0 3-3H6a3 3 0 1 0 3 3V6a3 3 0 1 0-3 3h12a3 3 0 1 0-3-3"/></svg>';
...
function performSearch (query) {
    ...
    const results = fuse.search(query).slice(0, 20).map(r => {
        ...
        if ( exactIndex !== -1 ) {
            const matchedText = item.text.substr(exactIndex, query.length);
            textFragment = generateTextFragment(matchedText);
        }
