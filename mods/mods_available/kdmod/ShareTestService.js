...
const { Actor, UserActorType } = require('../../../src/backend/src/services/auth/Actor.js');
...
class ShareTestService extends use.Service {
  ...
  async run_scenario_(scenario) {
    ...
    for (const step of scenario.sequence) {
      ...
      const report = await Context.get()
        .sub({ user, actor })
        .arun(async () => {
  ...
  // API for scenarios
  async ['__scenario:create-example-file']({ actor, user }, { name, contents }) {
    const svc_fs = this.services.get('filesystem');
    const parent = await svc_fs.node(new NodePathSelector(`/${user.username}/Desktop`));
    console.log('test -> create-example-file', user, name, contents);
  ...
  async ['__scenario:assert-no-access']({ actor, user }, { path }) {
    const svc_fs = this.services.get('filesystem');
  ...
  async ['__scenario:grant']({ actor, user }, { to, permission }) {
    const svc_permission = this.services.get('permission');
    await svc_permission.grant_user_user_permission(actor, to, permission, {}, {});
  }
  async ['__scenario:assert-access']({ actor, user }, { path, level }) {
    const svc_fs = this.services.get('filesystem');
