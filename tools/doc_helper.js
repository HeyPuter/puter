...
function checkForDuplicateEvent(eventId, filePath, seenEvents) {
  if (seenEvents.has(eventId)) {
    ...
    if (existing.fromManualOverride) {
      warnings.push(
        `Event ${eventId} found in ${filePath} but already defined in manual overrides. Using manual override.`
      );
    } else {
      warnings.push(
        `Duplicate event ${eventId} found in ${filePath}. First seen in ${existing.filename}.`
      );
    }
    return true;
  }
...
function extractEventsFromFile(filePath, seenEvents, debugMode) {
  ...
  while ((match = regex.exec(content)) !== null) {
    ...
    const hasAllowEffect =
      content.includes('event.allow') ||
...
function generateTestExtension(events) {
  ...
  events.forEach((event) => {
    ...
    const eventName = event.event
      ...
      : eventId.split('.').slice(1).join('.').toUpperCase();
...
function main() {
  ...
  if (args.length < 1) {
    console.error(
      'Usage: node doc_helper.js <directory> [output_file] [--generate-test] [--test-dir=<directory>] [--debug]'
    );
  ...
  manualOverrides.forEach((override) => {
    ...
    if (!event || !event.fromManualOverride) {
      warnings.push(
        `Manual override for ${override.id} exists but no matching event was found in the codebase.`
      );
    }
  });
