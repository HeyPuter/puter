...
areyousure('This script will delete all data in the database. Are you sure you want to proceed?', {
  crit: true,
});
...
const launch_puter = (args) => {
  const ret = spawnSync('node', ['tools/run-selfhosted.js', ...args], {
    stdio: 'inherit',
    env: {
      ...process.env,
      NO_VAR_RUNTIME: '1',
...

{
  ...
  if (!launch_puter(['--quit-on-alarm', `--boot-script=${boot_script_path}`]).ok) {
