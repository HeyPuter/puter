...
const StringStream = (str, { state_ } = {}) => {
  ...
  return {
    ...
    async debug() {
      ...
      return `[stream : "${clean(str.slice(0, Math.min(6, l1)))}"... |${state.pos}| ..."${clean(
        str.slice(state.pos, state.pos + Math.min(6, l2))
      )}"]`;
    },
  };
};

const LinesCommentParser = ({ prefix }) => {
  return {
    parse: async (stream) => {
...
const BlockCommentParser = ({ start, end, ignore_line_prefix }) => {
  return {
    parse: async (stream) => {
      stream.skip_whitespace();
  ...
  const get_language_by_filename = ({ filename }) => {
    const { language } = (({ filename }) => {
      const { language_id } = (({ filename }) => {
        ...
        if (!language_id) {
          throw new Error(`unrecognized language id: ` + language_id);
        }
        return { language_id };
      })({ filename });
