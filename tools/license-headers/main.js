 ...
 */
const LicenseChecker = ({ comment_parser, desired_header }) => {
  const supports = ({ filename }) => {
    return comment_parser.supports({ filename });
  };
  const compare = async ({ filename, source }) => {
    const headers = await comment_parser.extract_top_comments({ filename, source });
    ...
    diff_info.range = [headers[top].range[0], headers[bottom - 1].range[1]];
...
const license_check_test = async ({ options }) => {
  ...
  const license_checker = LicenseChecker({
    comment_parser,
    desired_header: fs.readFileSync(path_.join(__dirname, '../../doc/license_header.txt'), 'utf-8'),
  });
...
const cmd_check_fn = async () => {
  ...
  if (counts.error > 0) {
    ...
    console.log('');
    t.addRow({ situation: 'error', count: counts.error }, { color: 'red' });
  }
  ...
  t.addRow({ situation: 'ok', count: counts.ok }, { color: 'green' });
  t.addRow({ situation: 'missing', count: counts.missing }, { color: 'yellow' });
  t.addRow({ situation: 'conflict', count: counts.conflict }, { color: 'red' });
  ...
  for await (const value of walk_iterator) {
    ...
    if (diff_info) {
      if (diff_info.distance !== 0) {
        ...
        await cut_header();
        const cut_range = [diff_info.range[1], diff_info.range[1]];
        ...
        if (cut_range[0] !== cut_range[1]) {
          ...
          const action = await prompt.run();
          if (action === 'skip') continue;
          const new_source = source.slice(0, cut_range[0]) + source.slice(cut_range[1]);
          fs.writeFileSync(value.path, new_source);
        }
        counts.ok++;
        process.stdout.write(`\x1B[32;1mOK\x1B[0m\n`);
