...
const _bitBooleans = ['immutable', 'is_shortcut', 'is_symlink', 'is_dir'];

const _integers = ['created', 'accessed', 'modified'];

const _strings = ['id', 'uid', 'parent_id', 'name'];
...
const verify_fsentry = async (t, o) => {
  await t.case('fsentry is valid', async () => {
    ...
    await t.case('symlink_path is null or string', () => {
      expect(o.symlink_path === null || typeof o.symlink_path === 'string').true;
    });
