...
module.exports = TestFactory.cartesian('Cartesian Test for /write', write, {
  each: async (t, state, i) => {
    ...
    const contents = new Blob([`case ${i}\n`], { type: 'text/plain' });
    ...
    (fd.append('size', contents.size), fd.append('file', contents, 'uploaded_name.txt'));
    ...
    if (state['conditions.destinationIsFile'] && state.name === 'default' && !state.overwrite) {
      error_expected = {
