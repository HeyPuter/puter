...
module.exports = TestFactory.cartesian('Cartesian Test for /copy', copy, {
  each: async (t, state, i) => {
    ...
    const data = {};
    data.source = state['source.format'] === 'uid' ? srcUID : `${dir}${pathOfThingToCopy}`;
    data.destination = state['destination.format'] === 'uid' ? dstUID : `${dir}/b`;
    ...
    if (state['conditions.destinationIsFile'] && state.name === 'specified') {
      error_expected = {
        code: 'dest_is_not_a_directory',
        message: `Destination must be a directory.`,
      };
    } else if (state['conditions.destinationIsFile'] && !state.overwrite && !state.dedupe_name) {
