...
async function checkTranslationRegistrations() {
  ...
  for (const fileName of files) {
    ...
    if (!translation) {
      reportError(
        `Translation '${translationName}' is not listed in translations.js, please add it!`
      );
      continue;
    }
    ...
    if (!translation.code) {
      ...
      reportError(
        `Translation '${translationName}' has code '${translation.code}', which should be '${translationName}'!`
      );
    }
    if (typeof translation.dictionary !== 'object') {
      reportError(
        `Translation '${translationName}' is missing a translations dictionary! Should be an object.`
      );
    }
  }
}
...
function checkTranslationKeys() {
  ...
  for (const translation of Object.values(translations)) {
    ...
    for (const [key, value] of Object.entries(translation.dictionary)) {
      if (!enDictionary[key]) {
        reportError(
          `Translation '${translation.code}' has key '${key}' that doesn't exist in 'en'!`
        );
      }
    }
  }
...
async function checkTranslationUsage() {
  ...
  const sourceDirectories = ['./src/gui/src/helpers', './src/gui/src/UI'];
